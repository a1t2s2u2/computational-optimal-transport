...
$\supp(\mu)$ は閉集合である．
実際，補集合 $\supp(\mu)^c = \{x \in X \mid x \in U,\; \mu(U) = 0 \text{ なる開集合 } U \text{ が存在する}\}$
が開集合であることを示せばよい．
...
\begin{theorem}{Prokhorov の定理}{prokhorov}
  ...
  \begin{enumerate}
    \item $\Pi$ は緊密である．
    \item $\Pi$ は\textbf{相対コンパクト}である．
      すなわち，$\Pi$ の任意の列が弱収束する部分列を持つ．
  \end{enumerate}
\end{theorem}

\begin{remark}{}{prokhorov-remark}
  ポーランド空間 $\X, \Y$ 上の確率測度
  $\alpha, \beta$ に対するカップリングの集合 $\Couplings(\alpha, \beta)$
  （\ref{sec:kantorovich-overview}~節で定義）は
  緊密かつ弱位相で閉であることが示される．
  したがって Prokhorov の定理からコンパクトとなり，
  Kantorovich 問題の最適解の存在が保証される．
\end{remark}
