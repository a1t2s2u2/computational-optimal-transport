...
$-\Hb$ は凸関数（定義~\ref{def:convex-function}）であり，
そのヘッセ行列は $\partial^2 (-\Hb)(\mathbf{P}) = \diag(1/P_{i,j})$ である．
したがって $-\varepsilon\Hb$ を加えた目的関数のヘッセ行列は
$\mathbf{C}$ の項（線形なので $0$）に $\varepsilon \diag(1/P_{i,j})$ が加わり，
$\mathbf{P} > 0$ の範囲で正定値となる．
...
\begin{definition}{Kullback--Leibler ダイバージェンス}{kl-divergence}
  ...
  \[
    ...
    \right)
  \]
  で定義する．$\KLD(\mathbf{P} \| \mathbf{K}) \geq 0$ であり，
  等号は $\mathbf{P} = \mathbf{K}$ のときかつそのときに限る．
\end{definition}
