...
$\mu_i=\mathcal{N}(m_i,M_i)$（$i=0,1$）を
$\R^n$ 上の Gaussian 測度とする．ここで
$m_i\in\R^n$ は平均ベクトル，
$M_i\in\R^{n\times n}$ は対称半正定値な共分散行列である．
...
\begin{lemma}{最適結合は Gaussian に選べる}{gs-gaussian-coupling}
  ...
  中心 Gaussian 測度
  $\mu_0=\mathcal{N}(0,M_0)$ と
  $\mu_1=\mathcal{N}(0,M_1)$ の間には，
...
\begin{proof}[定理~\ref{thm:gs-gaussian-formula}の証明]
  ...
  共分散行列が特異な場合は
  $M_i+\varepsilon I$ に対する公式を用いる．
  独立な $Z\sim\mathcal{N}(0,I)$ を加える結合から
  \[
    \Wass_2\!\left(
      \mathcal{N}(m_i,M_i+\varepsilon I),
      \mathcal{N}(m_i,M_i)
    \right)
...
\begin{proof}
  ...
  \[
    AM_0A^\top=M_1
  \]
  を満たす．したがって $X\sim\mathcal{N}(m_0,M_0)$ なら
  $T(X)\sim\mathcal{N}(m_1,M_1)$ である．
  この結合のコストを展開すると
...
\begin{proposition}{可換な共分散行列}{gs-commuting-covariances}
  ...
  \[
    ...
    \norm{m_0-m_1}^2
    +
    \norm{M_0^{1/2}-M_1^{1/2}}_{\mathrm{F}}^2,
  \]
...
\begin{example}{二次元の中心 Gaussian}{gs-two-dimensional-gaussian}
  $m_0=m_1=0$ かつ $M_0,M_1\in\R^{2\times2}$ とする．
