...
\begin{proposition}{Dirac 測度の押し出し}{sem-dirac-pushforward}
  ボレル可測写像 $T \colon \X \to \Y$ と点 $x \in \X$ に対して，任意の
  $A \in \Bb(\Y)$ で
  \[
    \delta_x\bigl(T^{-1}(A)\bigr) = \delta_{T(x)}(A),
...
\begin{proof}
  $T(x) \in A \iff x \in T^{-1}(A)$ から，指示関数として
  $\mathbf{1}_{\{x \in T^{-1}(A)\}} = \mathbf{1}_{\{T(x) \in A\}}$
  が成り立ち，左辺は $\delta_x(T^{-1}(A))$，右辺は $\delta_{T(x)}(A)$ に等しい．
\end{proof}
