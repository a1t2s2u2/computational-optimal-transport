...
\begin{definition}{可測空間}{sem-measurable-space}
  集合 $\X$ と $\X$ 上の $\sigma$-代数 $\mathcal{F}$ の組
  $(\X, \mathcal{F})$ を\textbf{可測空間}（measurable space）という．
...
\begin{definition}{ボレル $\sigma$-代数}{sem-borel}
  ...
  \textbf{ボレル $\sigma$-代数}といい，$\Bb(\X)$ と記す．
\end{definition}

\begin{definition}{確率測度}{sem-probability-measure}
  可測空間 $(\X, \mathcal{F})$ 上の\textbf{確率測度}（probability measure）とは，
  ...
  \end{enumerate}
  を満たすものをいう．
  $\X$ 上の確率測度の全体を $\Mm_+^1(\X)$ と記す．
\end{definition}

\begin{definition}{可測写像}{sem-measurable-map}
  可測空間 $(\X, \mathcal{F})$ から $(\Y, \mathcal{G})$ への写像
  $T \colon \X \to \Y$ が\textbf{可測}であるとは，
  $T^{-1}(B) \in \mathcal{F}$ が
  すべての $B \in \mathcal{G}$ に対して成り立つことをいう．
\end{definition}
