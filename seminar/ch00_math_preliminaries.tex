...
\begin{remark}{$\Bb(\Omega)$ のイメージ}{sem-borel-intuition}
  $\Bb(\Omega)$ は開集合から出発して補集合・可算和を繰り返して得られる集合族であり，
  位相から自然に作られる．一方で $2^\Omega$ 全体にわたって整合的に測度を定めることは
  できない（Vitali 集合などが障害となる）ため，測度の定義域として
  $\Bb(\Omega)$ を採用する．日常の解析・確率で書き下せる集合は事実上すべて
  $\Bb(\Omega)$ に属する．連続写像 $f$ は $f^{-1}(\text{開集合})$ が開集合なので
  自動的にボレル可測である．
\end{remark}

\begin{remark}{積ボレル $\sigma$-代数}{sem-product-borel}
  可分な距離空間 $\X, \Y$ に対しては，積距離から定まる位相のボレル
  $\sigma$-代数は積 $\sigma$-代数と一致する：
  $\Bb(\X \times \Y) = \Bb(\X) \otimes \Bb(\Y)$．
  後章のカップリング $\pi \in \Mm_+^1(\X \times \Y)$ はこの可測構造の下で扱う．
\end{remark}
...
\begin{remark}{本稿における可測構造の取り決め}{sem-borel-convention}
  以降，距離空間 $\X$ 上の可測構造はボレル $\sigma$-代数
  $\Bb(\X)$ を既定とし，$(\X, \Bb(\X))$ を既定の可測空間とする（$\Y$ も同様）．
  本章の定義はいずれも一般の距離空間で成立するが，第~\ref{ch:seminar-01}章以降は
  連続理論（disintegration, tightness, Wasserstein 幾何）を扱う都合上，
  $\X, \Y$ に Polish 性（定義~\ref{def:sem-polish}）を一様に課す．
\end{remark}
...
\begin{proposition}{離散測度の押し出し}{sem-dirac-pushforward}
  ボレル可測写像 $T \colon \X \to \Y$，点 $x_1, \ldots, x_n \in \X$，
  係数 $a_1, \ldots, a_n \geq 0$ に対して，
  \[
    T\pushforward \Bigl(\sum_{i=1}^{n} a_i\, \delta_{x_i}\Bigr)
    = \sum_{i=1}^{n} a_i\, \delta_{T(x_i)}.
  \]
  特に $T\pushforward \delta_x = \delta_{T(x)}$．
\end{proposition}

\begin{proof}
  $x \in T^{-1}(A) \iff T(x) \in A$ と Dirac 測度の $\{0, 1\}$ 値性から，
  任意の $A \in \Bb(\Y)$ で
  $\delta_{x}(T^{-1}(A)) = \delta_{T(x)}(A)$．
  押し出しの定義より任意の $A \in \Bb(\Y)$ で
  \[
