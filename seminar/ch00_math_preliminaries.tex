...
\begin{definition}{置換（permutation）}{sem-permutation}
  $\range{n} \defeq \{1, \ldots, n\}$ 上の\textbf{置換}とは，
  $\range{n}$ から $\range{n}$ への全単射をいう．
  $\range{n}$ 上のすべての置換の集合を $\Perm(n)$ と記す．
  $|\Perm(n)| = n!$ である．
\end{definition}

\begin{definition}{置換行列（permutation matrix）}{sem-permutation-matrix}
...
\begin{proposition}{離散測度の押し出し}{sem-dirac-pushforward}
  可測写像 $T \colon \X \to \Y$，点 $x_1, \ldots, x_n \in \X$，
