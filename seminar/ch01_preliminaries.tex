...
\begin{claim}{離散測度の標準形}{sem-discrete-measure-form}
  $\X$ 上のボレル確率測度 $\mu \in \Mm_+^1(\X)$ について以下は同値．
  \begin{enumerate}
...
\begin{proof}
  ...
  Polish 空間は Hausdorff なので有限集合 $\{x_1, \ldots, x_n\}$ は閉．
  ...
  $\X \setminus \supp(\mu)$ の各点 $y$ はある開球 $B_{r_y}(y)$ で $\mu(B_{r_y}(y)) = 0$ をみたす．
  Polish 空間は第二可算ゆえ $\X \setminus \supp(\mu)$ はそのような開球の可算和で覆え，
  測度の劣加法性から $\mu(\X \setminus \supp(\mu)) = 0$．したがって
  \[
