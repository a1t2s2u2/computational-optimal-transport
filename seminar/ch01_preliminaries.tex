...
\begin{definition}{Hausdorff 性}{sem-hausdorff}
  位相空間 $(\Omega, \mathcal{O})$ が\textbf{Hausdorff 空間}であるとは，
  相異なる任意の 2 点 $x, y \in \Omega$ に対し，
  $x \in U$, $y \in V$, $U \cap V = \emptyset$ をみたす
  開集合 $U, V \in \mathcal{O}$ が存在することをいう．
\end{definition}
...
\begin{definition}{第二可算}{sem-second-countable}
  位相空間 $(\Omega, \mathcal{O})$ が\textbf{第二可算}であるとは，
  可算な開集合族 $\mathcal{N} = \{U_k\}_{k \geq 1} \subset \mathcal{O}$ が存在して，
  任意の開集合 $U \in \mathcal{O}$ がある部分族
  $\mathcal{N}' \subset \mathcal{N}$ の和集合
  $U = \bigcup_{V \in \mathcal{N}'} V$ として書けることをいう．
  この $\mathcal{N}$ を\textbf{可算開基}という．
\end{definition}

\begin{definition}{Lindelöf 性}{sem-lindelof}
  位相空間 $(\Omega, \mathcal{O})$ が\textbf{Lindelöf 空間}であるとは，
  $\Omega$ の任意の開部分集合 $U$ の任意の開被覆
  $\{V_\lambda\}_{\lambda \in \Lambda}$
  （$U \subset \bigcup_\lambda V_\lambda$，各 $V_\lambda \in \mathcal{O}$）が
  可算な部分被覆を持つことをいう．
\end{definition}
...
\begin{claim}{距離空間は位相空間（距離位相）}{sem-metric-induces-topology}
  距離空間 $(\Omega, d)$ に対し，
  \[
    \mathcal{O}_d \defeq
    \{U \subset \Omega \mid
  ...
  \]
  は $\Omega$ 上の位相（定義~\ref{def:sem-topological-space}）である．
  $(\Omega, \mathcal{O}_d)$ を $(\Omega, d)$ から定まる
  \textbf{距離位相空間}という．
  さらに $D \subset \Omega$ について，
  $D$ が $(\Omega, \mathcal{O}_d)$ で稠密であること
  （定義~\ref{def:sem-dense}）と，
  「任意の $x \in \Omega$ と $\varepsilon > 0$ に対し
  $d(x, y) < \varepsilon$ をみたす $y \in D$ が存在する」ことは同値．
\end{claim}

\begin{proof}
  \textbf{位相の3条件．}
  (i) $\emptyset \in \mathcal{O}_d$ は条件が空に成立．
  $\Omega \in \mathcal{O}_d$ も $B_r(x) \subset \Omega$ より自明．
  (ii) $U_1, \ldots, U_n \in \mathcal{O}_d$ と $x \in \bigcap_k U_k$ に対し，
  各 $k$ で $B_{r_k}(x) \subset U_k$ なる $r_k > 0$ がとれ，
  $r \defeq \min_k r_k > 0$ とおけば $B_r(x) \subset \bigcap_k U_k$．
  (iii) $\{U_\lambda\} \subset \mathcal{O}_d$ と
  $x \in \bigcup_\lambda U_\lambda$ に対し，
  ある $\lambda_0$ で $x \in U_{\lambda_0}$，
  $B_r(x) \subset U_{\lambda_0} \subset \bigcup_\lambda U_\lambda$
  なる $r > 0$ がとれる．

  \textbf{稠密性の同値．}
  ($\Rightarrow$) 任意の $x \in \Omega$ と $\varepsilon > 0$ に対し
  $B_\varepsilon(x) \in \mathcal{O}_d$ かつ非空ゆえ
  $D \cap B_\varepsilon(x) \neq \emptyset$．
  ($\Leftarrow$) 任意の空でない $U \in \mathcal{O}_d$ と $x \in U$ に対し，
  ある $r > 0$ で $B_r(x) \subset U$ がとれ，仮定より
  $D \cap B_r(x) \neq \emptyset$，ゆえに $D \cap U \neq \emptyset$．
\end{proof}

\begin{claim}{距離空間は Hausdorff}{sem-metric-hausdorff}
  距離空間 $(\Omega, d)$ から定まる距離位相空間 $(\Omega, \mathcal{O}_d)$ は
  Hausdorff（定義~\ref{def:sem-hausdorff}）である．
...
\begin{proof}
  ...
  $B_r(x), B_r(y) \in \mathcal{O}_d$ ゆえ Hausdorff．
...
\begin{claim}{可分距離空間は第二可算かつ Lindelöf}{sem-separable-second-countable-lindelof}
  距離空間 $(\Omega, d)$ が距離位相について可分（定義~\ref{def:sem-separable}）であれば，
  第二可算（定義~\ref{def:sem-second-countable}）であり，かつ
  Lindelöf（定義~\ref{def:sem-lindelof}）である．
\end{claim}
...
\begin{definition}{測度の台}{sem-support}
...
\begin{claim}{離散測度の標準形と一意性}{sem-discrete-measure-form}
  $\mu \in \Mm_+^1(\X)$ について以下は同値：
  \begin{enumerate}
    \item[(i)] $\mu$ は離散測度（定義~\ref{def:sem-discrete-measure}）．
    \item[(ii)] $\supp(\mu)$ は有限集合．
  \end{enumerate}
  さらに離散測度 $\mu = \sum_{i=1}^{n} a_i\, \delta_{x_i}$ の表示において
  $\supp(\mu) = \{x_1, \ldots, x_n\}$，$a_i = \mu(\{x_i\})$ が成り立ち，
  $n$ および対 $\{(x_i, a_i)\}_{i=1}^n$ は添字の入れ替えを除いて一意に定まる．
\end{claim}

\begin{proof}
  \textbf{(i) $\Rightarrow$ (ii)，および $\supp(\mu) = \{x_1, \ldots, x_n\}$．}
  ...
  \textbf{表示の一意性．}
  上で示した $\supp(\mu) = \{x_1, \ldots, x_n\}$ により点集合は $\mu$ から一意に定まり，
  $a_i = \mu(\{x_i\})$ により重みも一意に定まる．したがって
  添字の入れ替えを除いて表示は一意．
\end{proof}
