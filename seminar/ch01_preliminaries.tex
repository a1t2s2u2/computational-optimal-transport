...
\begin{claim}{有限集合の最小元}{sem-finite-min}
  $n \in \N$ と $S_n \in \{S \subset \R \mid |S| = n\}$ に対して，
  $S_n$ には最小元が存在する：
  \[
    \exists\, s^* \in S_n \;\;\text{s.t.}\;\; \forall\, s \in S_n,\; s^* \leq s.
  \]
\end{claim}
...
\begin{proof}
  ...
  \textbf{$n = 1$ の時．} $S_1$ の唯一の元がそのまま最小元．

  \textbf{$n$ の時に成り立つと仮定して $n + 1$ の時を示す．}
  $S_{n+1}$ から任意に $a$ を取り $S_n \defeq S_{n+1} \setminus \{a\}$ とおく．
  帰納法の仮定により $S_n$ は最小元 $b \in S_n$ を持つ．
  ...
  \[
    ...
    \end{cases}
  \]
  とおけば $s^* \in S_{n+1}$ かつ任意の $s \in S_{n+1}$ について $s^* \leq s$．
\end{proof}
