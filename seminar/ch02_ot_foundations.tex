...
\begin{remark}{本章の設定}{sem-ch2-setup}
  ...
  \begin{enumerate}
    \item[(i)] 距離から $\X, \Y$ の位相・ボレル $\sigma$-代数・可測関数・
      測度の弱収束を定める．
    \item[(ii)] Polish 性は有限ボレル測度のタイト性（\textbf{Ulam}）・
      条件付き測度の取り出し（\textbf{disintegration}）・
      弱コンパクト性（\textbf{Prokhorov}）を保証し，
      Kantorovich 問題の解の存在と双対定理の基盤となる．
