...
\begin{proposition}{連続版との同値性}{sem-discrete-kantorovich-equiv}
  定義~\ref{def:sem-discrete-kantorovich} の設定のもとで，
  \[
    \MK_c(\alpha, \beta) = \MKD_{\mathbf{C}}(\mathbf{a}, \mathbf{b}).
  \]
  すなわち，Kantorovich 問題（定義~\ref{def:sem-kantorovich}）は
  離散 Kantorovich 問題（定義~\ref{def:sem-discrete-kantorovich}），
  ひいては線形計画問題（定義~\ref{def:sem-lp}）に帰着される．
\end{proposition}
