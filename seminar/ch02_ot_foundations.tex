...
\begin{remark}{本章の設定}{sem-ch2-setup}
  ...
  \end{enumerate}
  離散版では確率測度の台 $S \defeq \{x_1, \ldots, x_n\} \subset \X$ が
...
\begin{claim}{離散 Monge 問題（discrete Monge problem）}{sem-discrete-monge}
  $\alpha, \beta$ を上記の表示の離散測度とする．
  台上の写像 $T \colon \{x_1, \ldots, x_n\} \to \Y$ に対する押し出しを
...
\begin{proof}
  目的関数 (a) と押し出し制約 (b) が $T$ の台上の値 $T(x_1), \ldots, T(x_n)$
  のみに依存することを示し，さらに台上の任意の値の組
  $(y_1, \ldots, y_n) \in \Y^n$ がある可測写像 $\widetilde T \colon \X \to \Y$ の
  台上の値として実現されること (c) を示す．
  ...
  \]
  であり，等式 $T\pushforward \alpha = \beta$ もまた台上の値のみに依存する．

  以上 (a)(b) より制限写像 $T \mapsto T|_{\{x_1, \ldots, x_n\}}$ は，
  連続版の実行可能写像を離散版の実行可能写像に目的関数値を保存して写す．
  残るはこの制限写像が全射であること，
  すなわち台上の任意の値の組が可測写像から実現されることである．
