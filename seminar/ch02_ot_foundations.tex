...
\begin{remark}{本章の設定}{sem-ch2-setup}
  ...
  距離と Polish 性は本章で次のように使う：
  \begin{enumerate}
    \item[(i)] 距離から $\X, \Y$ の位相（定義~\ref{def:sem-metric-topology}）・
      ボレル $\sigma$-代数（定義~\ref{def:sem-borel}）・可測関数
      （定義~\ref{def:sem-measurable-function}）・測度の弱収束
      （定義~\ref{def:sem-weak-convergence}）を定める．
    \item[(ii)] Polish 性は有限ボレル測度のタイト性
      （命題~\ref{prop:sem-ulam}）・条件付き測度の取り出し
      （\textbf{disintegration}，命題~\ref{prop:sem-disintegration}）・
      弱コンパクト性（\textbf{Prokhorov}，命題~\ref{prop:sem-prokhorov}）を
      保証し，Kantorovich 問題の解の存在と双対定理の基盤となる．
    \item[(iii)] Wasserstein 距離の輸送コスト
