...
\end{align*}
と書き換えられる．これと押し出し制約
$T\pushforward \alpha = \sum_{i=1}^n a_i\, \delta_{T(x_i)} = \beta$
（命題~\ref{prop:sem-dirac-pushforward} (ii)）はいずれも
$T$ の台 $\{x_1, \ldots, x_n\}$ 上の値 $T(x_1), \ldots, T(x_n) \in \Y$ にしか依存しない．
逆に任意の値の組 $(y_1, \ldots, y_n) \in \Y^n$ は，$y_\ast \in \Y$ を一つ固定して
台上で $y_i$，補集合上で $y_\ast$ をとる単純関数を作れば
（Polish $\Rightarrow$ Hausdorff より各 $\{x_i\} \in \Bb(\X)$ ゆえボレル可測）
ボレル可測写像 $\X \to \Y$ に持ち上がる．
したがって一般 Monge 問題における可測写像 $T \colon \X \to \Y$ の選択は
$T \colon \{x_1, \ldots, x_n\} \to \Y$ の選択と等価で，\textbf{離散 Monge 問題}は
\[
  \min_{T}
  \left\{
  \sum_{i=1}^{n} a_i \, c(x_i, T(x_i))
  ...
  \right\}
\]
と書ける（ここで $T\pushforward \alpha \defeq \sum_{i=1}^{n} a_i\, \delta_{T(x_i)}$）．
