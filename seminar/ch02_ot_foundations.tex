...
\begin{definition}{Monge 問題（離散版）}{sem-monge-discrete}
  点 $x_1, \ldots, x_n \in \X$ に重み $a_i > 0$（$\sum_i a_i = 1$）を，
  点 $y_1, \ldots, y_m \in \Y$ に重み $b_j > 0$（$\sum_j b_j = 1$）を持たせた
  離散測度
  $\alpha = \sum_{i=1}^n a_i \delta_{x_i}$，
  $\beta = \sum_{j=1}^m b_j \delta_{y_j}$
  とコスト関数 $c \colon \X \times \Y \to \R_+$ が与えられたとする．
  写像 $T \colon \{x_1, \ldots, x_n\} \to \{y_1, \ldots, y_m\}$ が
  質量保存条件
  \[
    \forall\, j \in \range{m}, \quad
    b_j = \sum_{i \,:\, T(x_i) = y_j} a_i
  \]
  を満たすとき，$T\pushforward \alpha = \beta$ と書く．
