...
\begin{definition}{カップリング}{sem-coupling}
  $\X, \Y$ 上の確率測度
  $\alpha \in \Mm_+^1(\X)$, $\beta \in \Mm_+^1(\Y)$ に対し，
  射影
  \[
    P_\X \colon \X \times \Y \to \X,
    \qquad
    P_\Y \colon \X \times \Y \to \Y
  \]
  を用いて
  \[
...
\begin{definition}{Kantorovich 問題}{sem-kantorovich}
  ...
  可測関数 $c \colon \X \times \Y \to \R_+$ に対して，
  $\alpha, \beta$ のカップリング
  $\pi \in \Couplings(\alpha, \beta)$（定義~\ref{def:sem-coupling}）にわたって
  輸送コストを最小化する問題
  \[
...
コスト関数を離散化した行列
$\mathbf{C} \in \R_+^{n \times m}$，$C_{i,j} \defeq c(x_i, y_j)$ を定めると，
Kantorovich 問題の各要素は次のように有限次元へ帰着される：
\begin{center}
  \begin{tabular}{l|ll}
    & 連続版 & 離散版 \\ \hline
    カップリング集合
      & $\Couplings(\alpha, \beta) \subset \Mm_+^1(\X \times \Y)$
      & $\CouplingsD(\mathbf{a}, \mathbf{b}) \subset \R_+^{n \times m}$ \\
    輸送コスト
      & $\displaystyle\int_{\X \times \Y} c\, \d\pi$
...
\subsubsection*{離散カップリング集合}

\begin{definition}{離散カップリング集合（輸送多面体）}{sem-discrete-coupling}
  \[
  ...
  \]
  を $\mathbf{a}$ と $\mathbf{b}$ の\textbf{離散カップリング集合}（\textbf{輸送多面体}）という．
...
\subsubsection*{輸送コストの離散化}

Claim~\ref{clm:sem-discrete-coupling-form} の全単射 $\Phi$ で
$\pi = \Phi(\mathbf{P})$ と書くと，輸送コストはフロベニウス内積に帰着される：

\begin{claim}{輸送コストの行列表示}{sem-cost-matrix-form}
  \[
...
\begin{proof}
  $\pi = \sum_{i,j} P_{i,j} \delta_{(x_i, y_j)}$ に対し，
...
\subsubsection*{離散 Kantorovich 問題}

$\Phi$ の全単射性から，$\pi \in \Couplings(\alpha, \beta)$ にわたる下限は
