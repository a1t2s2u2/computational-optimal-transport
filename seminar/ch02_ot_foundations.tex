...
\begin{remark}{本章の設定}{sem-ch2-setup}
  ...
  \begin{itemize}
    \item \textbf{距離空間}．距離関数
      ...
      \end{align*}
      ここで $\sigma(\mathcal{O}_d)$ は $\mathcal{O}_d$ を含む最小の
      $\sigma$-代数を表す．写像 $T \colon \X \to \Y$ が\textbf{可測}とは，
      任意の $B \in \Bb(\Y)$ に対し $T^{-1}(B) \in \Bb(\X)$ となることをいう
      （定義~\ref{def:sem-measurable-map}）．
