...
素朴には，集合 $\Perm(n)$ 内のすべての置換を用いてコスト関数を評価できる．
しかし $|\Perm(n)| = n!$ であり，
小さな $n$ に対しても巨大である．
例えば $n$ がわずか $70$ のとき，$n! > 10^{100}$ となる．
したがって，置換の集合上でコスト関数を効率的に最適化する
アルゴリズムが必要であり，これは第3章で扱う．
...
に対する Monge 問題は，
$T\pushforward \alpha = \beta$ を満たす写像 $T$ のもとで
$\sum_i c(x_i, T(x_i))$ を最小化する問題となる．
特に $n = m$ かつすべての重みが一様（$a_i = b_j = 1/n$）のとき，
質量保存の制約から $T$ は全単射となり，
