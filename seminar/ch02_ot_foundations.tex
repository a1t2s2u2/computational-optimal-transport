...
$\alpha, \beta$ がともに離散測度の場合，
カップリングは $\X \times \Y$ の有限グリッド $\{x_i\} \times \{y_j\}$ 上に集中する
非負行列で特徴付けられる：

\begin{claim}{離散カップリングの行列表示}{sem-discrete-coupling-form}
  $\alpha = \sum_{i=1}^n a_i \delta_{x_i}$，$\beta = \sum_{j=1}^m b_j \delta_{y_j}$ を
  §\ref{sec:sem-monge}~冒頭で定めた標準形の離散測度とする．
  カップリング $\pi \in \Couplings(\alpha, \beta)$ は，ある
  $\mathbf{P} \in \R_+^{n \times m}$ により
  \[
    \pi = \sum_{i=1}^n \sum_{j=1}^m P_{i,j}\, \delta_{(x_i, y_j)}
  \]
  と一意に表される．対応 $\pi \leftrightarrow \mathbf{P}$ は
  $\Couplings(\alpha, \beta)$ と
  \[
...
\begin{proof}
  \textbf{(a) $\pi$ がグリッド上に集中する．}
  $\alpha = \sum_i a_i \delta_{x_i}$ より
  $\alpha(\X \setminus \{x_1, \ldots, x_n\}) = 0$．
  周辺条件 $(P_\X)\pushforward \pi = \alpha$ から
  \[
    \pi\bigl( (\X \setminus \{x_1, \ldots, x_n\}) \times \Y \bigr)
    = \alpha(\X \setminus \{x_1, \ldots, x_n\}) = 0.
  \]
  同様に $\pi\bigl( \X \times (\Y \setminus \{y_1, \ldots, y_m\}) \bigr) = 0$．
  したがって $\pi$ は有限集合
  $\{(x_i, y_j) \mid 1 \leq i \leq n,\; 1 \leq j \leq m\}$ 上に集中し，
  ...
  \begin{align*}
    a_k = \alpha(\{x_k\})
    = \bigl((P_\X)\pushforward \pi\bigr)(\{x_k\})
    &= \pi(\{x_k\} \times \Y) \\
    &= \sum_{j=1}^m \pi(\{(x_k, y_j)\}) = \sum_{j=1}^m P_{k,j},
  \end{align*}
  すなわち $\mathbf{P}\ones_m = \mathbf{a}$．同様に $\mathbf{P}^\top \ones_n = \mathbf{b}$．

  \textbf{(c) 逆方向の対応．}
  $\mathbf{P} \in \CouplingsD(\mathbf{a}, \mathbf{b})$ から
  $\pi \defeq \sum_{i,j} P_{i,j} \delta_{(x_i, y_j)}$ と定めれば
  $\pi(\X \times \Y) = \sum_{i,j} P_{i,j}
  = \sum_i \sum_j P_{i,j} = \sum_i a_i = 1$ ゆえ
  $\pi \in \Mm_+^1(\X \times \Y)$．
  (b) の計算を逆にたどれば周辺は $\alpha, \beta$ となり，
  対応は全単射．
\end{proof}
...
離散測度の場合，Kantorovich 問題は離散カップリング行列上の
有限次元線形計画に帰着する：

\begin{claim}{離散 Kantorovich 問題}{sem-discrete-kantorovich}
  $\alpha, \beta$ を§\ref{sec:sem-monge}~冒頭の標準形の離散測度，
  $C_{i,j} \defeq c(x_i, y_j)$ とおく．
  Claim~\ref{clm:sem-discrete-coupling-form} の対応で
  $\pi \in \Couplings(\alpha, \beta)$ と
  $\mathbf{P} \in \CouplingsD(\mathbf{a}, \mathbf{b})$ を結ぶと
  \[
    \int_{\X \times \Y} c(x, y) \, \d\pi(x, y) = \inner{\mathbf{C}}{\mathbf{P}}
  \]
  が成り立ち，連続版 Kantorovich 問題（定義~\ref{def:sem-kantorovich}）の
  下限は\textbf{離散 Kantorovich 問題}
  \[
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \sum_{i,j} C_{i,j} P_{i,j}
      = \inner{\mathbf{C}}{\mathbf{P}}.
  \end{align*}
  Claim~\ref{clm:sem-discrete-coupling-form} の全単射対応により
  $\pi \in \Couplings(\alpha, \beta)$ にわたる下限と
...
凸多面体（主張~\ref{clm:sem-coupling-polytope}）をなす．
