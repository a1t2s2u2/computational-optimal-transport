...
最適輸送の最も単純な場合として，まず\textbf{最適割当問題}を導入する．

$n$ 人の作業者と $n$ 個の仕事があり，
作業者 $i$ を仕事 $j$ に配属するコストが $C_{i,j}$ で与えられているとする．
作業者と仕事を1対1に対応させるとき，
総コストを最小にする割当を求めるのが最適割当問題である．

\begin{definition}{置換}{sem-permutation}
  有限集合 $S$ 上の\textbf{置換}（permutation）とは，
  $S$ から $S$ への全単射 $\sigma \colon S \to S$ をいう．
  特に $S = \range{n} \defeq \{1, \ldots, n\}$ のとき，
...
\begin{definition}{置換行列}{sem-permutation-matrix}
  ...
  各成分は $0$ か $1$ であり，$\sigma$ が全単射であることから
  \textbf{各行・各列にちょうど1つの $1$} を持つ．
  置換行列はベクトルの成分を $\sigma$ に従って並び替える行列であり，
  $(\mathbf{P}_\sigma \mathbf{v})_i = v_{\sigma(i)}$ が成り立つ．
\end{definition}
...
\begin{example}{置換行列}{sem-permutation-matrix-example}
  ...
  \[
    ...
    =
    \begin{pmatrix} \sigma(1) \\ \sigma(2) \\ \sigma(3) \end{pmatrix}.
  \]
\end{example}
...
\begin{definition}{最適割当問題}{sem-assignment}
  ...
  \[
    \min_{\sigma \in \Perm(n)} \sum_{i=1}^{n} C_{i, \sigma(i)}
  \]
  を求める問題である．
\end{definition}

置換行列を用いると，割当のコストはフロベニウス内積で表せる．

\begin{proposition}{フロベニウス内積による表現}{sem-frobenius}
  コスト行列 $\mathbf{C} \in \R^{n \times n}$ と置換 $\sigma \in \Perm(n)$ に対し，
  \[
    \sum_{i=1}^{n} C_{i,\sigma(i)} = \inner{\mathbf{C}}{\mathbf{P}_\sigma}.
  \]
  したがって最適割当問題は
...
\begin{proof}
  $(\mathbf{P}_\sigma)_{i,j} = 1$ となるのは $j = \sigma(i)$ のときのみなので，
  \[
    \inner{\mathbf{C}}{\mathbf{P}_\sigma}
    = \sum_{i,j} C_{i,j} (\mathbf{P}_\sigma)_{i,j}
    = \sum_{i=1}^{n} C_{i,\sigma(i)}.  \qedhere
  \]
\end{proof}
...
\begin{example}{最適割当}{sem-assignment-cost-example}
  ...
  \[
    \inner{\mathbf{C}}{\mathbf{P}_\sigma}
    = C_{1,2} + C_{2,3} + C_{3,1}
    = 2 + 1 + 3 = 6.
  \]
  $3! = 6$ 通りの割当すべてのコストを列挙すると，
  \begin{center}
  \begin{tabular}{ccc}
    $\sigma$ & 割当 & コスト $\sum_i C_{i,\sigma(i)}$ \\\hline
    $(1,2,3)$ & $1{\to}1,\; 2{\to}2,\; 3{\to}3$ & $5 + 4 + 6 = 15$ \\
    $(1,3,2)$ & $1{\to}1,\; 2{\to}3,\; 3{\to}2$ & $5 + 1 + 9 = 15$ \\
    $(2,1,3)$ & $1{\to}2,\; 2{\to}1,\; 3{\to}3$ & $2 + 7 + 6 = 15$ \\
    $(2,3,1)$ & $1{\to}2,\; 2{\to}3,\; 3{\to}1$ & $2 + 1 + 3 = \mathbf{6}$ \\
    $(3,1,2)$ & $1{\to}3,\; 2{\to}1,\; 3{\to}2$ & $8 + 7 + 9 = 24$ \\
    $(3,2,1)$ & $1{\to}3,\; 2{\to}2,\; 3{\to}1$ & $8 + 4 + 3 = 15$ \\
  \end{tabular}
  \end{center}
  よって $\sigma = (2, 3, 1)$ が最小コスト $6$ を達成し，最適である．
\end{example}
