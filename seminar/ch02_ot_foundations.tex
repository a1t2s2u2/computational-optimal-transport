...
定義により，
ある $x_1, \ldots, x_n \in \X$，$y_1, \ldots, y_m \in \Y$，
...
\begin{claim}{離散 Monge 問題（discrete Monge problem）}{sem-discrete-monge}
  ...
  \[
    ...
    \right\}
  \]
  により定める．このとき離散 Monge 問題の最小値は連続版 Monge 問題
  （定義~\ref{def:sem-monge}）の下限と一致する：
  連続版の可測写像 $T \colon \X \to \Y$ から制限 $T|_{\{x_1, \ldots, x_n\}}$ により
  離散版の写像が得られ，この対応の下で目的関数値・押し出し制約が保存される．
