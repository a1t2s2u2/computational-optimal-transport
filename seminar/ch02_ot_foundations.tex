...
\begin{remark}{本章の設定}{sem-ch2-setup}
  ...
  Polish 性は自動で満たされる．実際，$\X$ にどの距離 $d$ を入れたとしても
  次が成り立つ．
  \begin{itemize}
    \item[] \textbf{完備性．} 相異なる 2 点間の距離の最小値を
      $\delta \defeq \min_{i \neq j} d(x_i, x_j)$ とおく．
      これは有限個の正数の最小値なので $\delta > 0$．
      任意の Cauchy 列 $(y_k)_{k \geq 1} \subseteq \X$ に対し，
      Cauchy 条件（定義~\ref{def:sem-complete-metric}）で
      $\varepsilon \defeq \delta / 2$ を取ると，ある $N \in \N$ が存在して
      $k, l \geq N \Longrightarrow d(y_k, y_l) < \delta / 2 < \delta$．
      一方 $\X$ の相異なる 2 点の距離は $\delta$ 以上だから，
      $d(y_k, y_l) < \delta$ は $y_k = y_l$ を強制する．
      ゆえに列 $(y_k)$ は $N$ 以降すべて同一の点となり，
      その点に収束する．よって完備．
    \item[] \textbf{可分性．} $\X$ 自身が有限（特に可算）集合であり，
      任意の $x \in \X$ について $x$ 自身が $\X$ に属するから，
      $\X$ は $\X$ 自身の中で稠密（定義~\ref{def:sem-separable}）．
      よって $\X$ は $\X$ という可算稠密部分集合を持ち可分．
  \end{itemize}
  以上より $(\X, d)$ は Polish 空間となるから，
  離散版に対する追加の制約はない．
\end{remark}
