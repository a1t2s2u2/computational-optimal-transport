...
\begin{example}{最適割当}{sem-assignment-cost-example}
  2人の作業者と2つの仕事場が直線上に配置されている状況を考える．
  作業者の自宅 $x_1, x_2$ と仕事場 $y_1, y_2$ の位置はそれぞれ
  \[
    x_1 = 0, \quad x_2 = 3, \quad y_1 = 1, \quad y_2 = 4
  \]
  であり，通勤コストは距離 $C_{i,j} = |x_i - y_j|$ で与えられる：
  \[
    \mathbf{C} =
    \begin{pmatrix}
      |0-1| & |0-4| \\
  ...
  $2! = 2$ 通りの割当を直線上に図示する．
  青丸（{\color{blue!60}$\bullet$}）が作業者の自宅，
  赤四角（{\color{red!60}$\blacksquare$}）が仕事場であり，
...
%% Monge 問題（§2.2 後半）
%% ============================================================
\begin{setting}{以降の節の舞台}{sem-ch2-setup}
  以降の節では議論の舞台となる空間構造を以下に固定する（§\ref{sec:sem-metric-space}--\ref{sec:sem-measurable} の取り決めに従う）．
  \begin{enumerate}
    \item[(i)] $\X, \Y$ は Polish 空間（定義~\ref{def:sem-polish}）とし，
      可測空間 $(\X, \Bb(\X))$, $(\Y, \Bb(\Y))$ として考える．
    \item[(ii)] 確率測度は $\alpha \in \Mm_+^1(\X)$, $\beta \in \Mm_+^1(\Y)$ とする．
    \item[(iii)] コスト関数 $c \colon \X \times \Y \to \R_+$ は連続とする（したがってボレル可測）．
  \end{enumerate}
\end{setting}
...
一般測度での gluing には正則条件付き確率（disintegration theorem）の存在が必要であり，
本章の舞台設定で課した $\X$ の Polish 性が本質的に効く．

\begin{proposition}{測度上の $p$-Wasserstein 距離}{sem-wasserstein-measure}
  $\X = \Y$ とし，$\dist$ を $\X$ 上の距離，
