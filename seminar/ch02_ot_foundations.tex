...
$T_1, T_2 \in \mathcal{T}(\alpha,\beta)$ の
凸結合 $\bar{T} = t T_1 + (1-t)T_2$ が再び
...
\begin{example}{実行可能集合の非凸性}{sem-nonconvex}
  ...
  $T_1(x) = x$（恒等写像），$T_2(x) = -x$（符号反転）とする．
  いずれも ${T_i}\pushforward \alpha = \beta$ を満たすから
  $T_1, T_2 \in \mathcal{T}(\alpha, \beta)$ である．

  しかし凸結合 $\bar{T} = \frac{1}{2}T_1 + \frac{1}{2}T_2$ は
  $\bar{T}(x) = \frac{1}{2}x + \frac{1}{2}(-x) = 0$（定値写像）であるから，
  \[
...
この例が示すように，写像の凸結合は各点の行き先を補間するが，
押し出しは行き先の\textbf{分布}で決まるため，
