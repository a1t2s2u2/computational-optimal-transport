...
\begin{tikzpicture}[scale=1.0]
  ...
  % --- π を表す 2 つの集中領域（紫の濃淡）---
  \fill[purple!20, opacity=0.7] (1.0, 1.0) ellipse (0.7 and 0.5);
  \fill[purple!45, opacity=0.7] (1.0, 1.0) ellipse (0.45 and 0.3);
  \fill[purple!75, opacity=0.8] (1.0, 1.0) ellipse (0.22 and 0.15);
  \fill[purple!20, opacity=0.7] (3.0, 3.0) ellipse (0.7 and 0.5);
  \fill[purple!45, opacity=0.7] (3.0, 3.0) ellipse (0.45 and 0.3);
  \fill[purple!75, opacity=0.8] (3.0, 3.0) ellipse (0.22 and 0.15);
  \node[font=\small] at (2.0, 2.0) {$\pi$};

  % --- α: X への射影（下方，2 山の密度曲線） ---
  \draw[blue, thick, smooth, samples=80, domain=0:4]
    plot (\x, {-1 + 0.55*exp(-(\x-1.0)*(\x-1.0)*4)
                + 0.55*exp(-(\x-3.0)*(\x-3.0)*4)});
  \node[blue, font=\small, anchor=north west] at (3.6, -0.3) {$\alpha$};
  \node[font=\footnotesize, gray, anchor=west] at (-0.2, -1.3)
    {$(P_\X)\pushforward \pi = \alpha$};

  % --- β: Y への射影（左方，2 山の密度曲線） ---
  \draw[red, thick, smooth, samples=80, domain=0:4, variable=\y]
    plot ({-1 + 0.55*exp(-(\y-1.0)*(\y-1.0)*4)
            + 0.55*exp(-(\y-3.0)*(\y-3.0)*4)}, \y);
