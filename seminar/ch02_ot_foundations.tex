...
\begin{example}{実行可能集合の非凸性}{sem-nonconvex}
  ...
  \]
  をとる．以下では Dirac 測度に関する次の等式を用いる：
  任意のボレル可測写像 $T$ と任意の点 $x$，$A \in \Bb(\Y)$ に対して
  \[
    \delta_{T(x)}(A)
    = \begin{cases} 1 & (T(x) \in A)\\ 0 & (\text{otherwise})\end{cases}
    = \begin{cases} 1 & (x \in T^{-1}(A))\\ 0 & (\text{otherwise})\end{cases}
  ...
  \begin{tikzpicture}[scale=0.9]
    %% ケース 1: T(x) ∈ A（両辺 = 1）
    \begin{scope}
      \node[font=\footnotesize\bfseries, anchor=west] at (-3.5, 2.2)
        {ケース 1: $T(x) \in A$ \ $\Rightarrow$ \
         $\delta_x(T^{-1}(A)) = \delta_{T(x)}(A) = 1$};
      % X 箱
      \draw[thick, rounded corners] (-3.5, -0.3) rectangle (-0.8, 1.5);
      \node[font=\small] at (-3.2, 1.3) {$\X$};
      \fill[green!30, rounded corners=3pt] (-3.1, 0.2) rectangle (-1.5, 1.0);
      \node[green!45!black, font=\scriptsize] at (-2.3, 0.6) {$T^{-1}(A)$};
      \fill (-2.3, 0.35) circle (1.6pt);
      \node[below, font=\scriptsize] at (-2.3, 0.3) {$x$};
      % Y 箱
      \begin{scope}[xshift=5cm]
        \draw[thick, rounded corners] (-0.5, -0.3) rectangle (2.2, 1.5);
        \node[font=\small] at (1.9, 1.3) {$\Y$};
        \fill[orange!45, rounded corners=3pt] (-0.2, 0.2) rectangle (1.4, 1.0);
        \node[orange!55!black, font=\scriptsize] at (0.6, 0.6) {$A$};
        \fill (0.6, 0.35) circle (1.6pt);
        \node[below, font=\scriptsize] at (0.6, 0.3) {$T(x)$};
      \end{scope}
      % 矢印 T
      \draw[->, thick] (-2.3, 0.35) to[bend left=20] (4.1, 0.35);
      \node[font=\scriptsize] at (2.3, 1.35) {$T$};
    \end{scope}
    ...
    \begin{scope}[yshift=-3cm]
      \node[font=\footnotesize\bfseries, anchor=west] at (-3.5, 2.2)
        {ケース 2: $T(x) \notin A$ \ $\Rightarrow$ \
         $\delta_x(T^{-1}(A)) = \delta_{T(x)}(A) = 0$};
      % X 箱
      \draw[thick, rounded corners] (-3.5, -0.3) rectangle (-0.8, 1.5);
      \node[font=\small] at (-3.2, 1.3) {$\X$};
      \fill[green!30, rounded corners=3pt] (-3.1, 0.2) rectangle (-2.0, 1.0);
      \node[green!45!black, font=\scriptsize] at (-2.55, 0.6) {$T^{-1}(A)$};
      \fill (-1.3, 0.35) circle (1.6pt);
      \node[below, font=\scriptsize] at (-1.3, 0.3) {$x$};
      % Y 箱
      \begin{scope}[xshift=5cm]
        \draw[thick, rounded corners] (-0.5, -0.3) rectangle (2.2, 1.5);
        \node[font=\small] at (1.9, 1.3) {$\Y$};
        \fill[orange!45, rounded corners=3pt] (-0.2, 0.2) rectangle (0.8, 1.0);
        \node[orange!55!black, font=\scriptsize] at (0.3, 0.6) {$A$};
        \fill (1.7, 0.35) circle (1.6pt);
        \node[below, font=\scriptsize] at (1.7, 0.3) {$T(x)$};
      \end{scope}
      % 矢印 T
      \draw[->, thick] (-1.3, 0.35) to[bend left=20] (6.2, 0.35);
      \node[font=\scriptsize] at (2.3, 1.35) {$T$};
    \end{scope}
  \end{tikzpicture}
  \\[0.3em]
  \parbox{0.92\linewidth}{\footnotesize
    \textit{図：} $x \in T^{-1}(A)$（緑領域に入る）と
    $T(x) \in A$（オレンジ領域に入る）は同値．
    したがって Dirac 測度の値 $\delta_x(T^{-1}(A))$ と $\delta_{T(x)}(A)$ は一致する．}
  \end{center}
