...
標準形（主張~\ref{clm:sem-discrete-measure-form}）により，
...
\[
  ...
  \right\}
\]
と書ける．目的関数および押し出し制約は $T$ の台 $\{x_1, \ldots, x_n\}$ 上の値
$T(x_1), \ldots, T(x_n) \in \Y$ にしか依存しないが，
定義~\ref{def:sem-monge} から継承される可測性条件は，
注釈~\ref{rem:sem-ch2-setup} の Polish 性と同様，離散の場合は自動成立する．

\begin{claim}{離散始域上の値割当の可測延長}{sem-discrete-measurable-extension}
  Polish 空間 $\X, \Y$ と相異なる $x_1, \ldots, x_n \in \X$ を固定する．
  任意の値の組 $(y_1, \ldots, y_n) \in \Y^n$ に対し，
  $T(x_i) = y_i$ ($i = 1, \ldots, n$) をみたす可測写像 $T \colon \X \to \Y$ が存在する．
\end{claim}

\begin{proof}
  $y_\ast \in \Y$ を一つ任意に固定し，
  \[
    T(x) \defeq
    \begin{cases}
    ...
    \end{cases}
  \]
  と定める．Polish 空間は Hausdorff ゆえ各 $\{x_i\}$ は閉集合，
  したがって $\Bb(\X)$ に属する．
  $T$ の値域は有限集合 $V \defeq \{y_\ast, y_1, \ldots, y_n\}$ であり，
  各 $y \in V$ に対し $T^{-1}(\{y\})$ は
  $\{x_1, \ldots, x_n\}$ の部分集合（有限点集合）または
  それと $\X \setminus \{x_1, \ldots, x_n\}$ との和集合に一致するから，
  いずれもボレル集合．任意の $B \in \Bb(\Y)$ に対し
  $T^{-1}(B) = \bigcup_{y \in B \cap V} T^{-1}(\{y\})$
  はボレル集合の有限和ゆえボレル．したがって $T$ は可測．
\end{proof}

特に離散 Monge 問題において $T$ に課された可測性条件は，
$T(x_1), \ldots, T(x_n) \in \Y$ をどう選んでも常に満たすことができ，
実行可能領域は実質的に押し出し制約 $T\pushforward \alpha = \beta$ のみで定まる．
