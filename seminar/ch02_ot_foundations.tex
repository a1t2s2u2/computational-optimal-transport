...
\begin{remark}{本章の設定}{sem-ch2-setup}
  $\X, \Y$ は\textbf{完備可分な距離空間}（Polish 空間，
  ...
  \begin{itemize}
    ...
    \item \textbf{完備性}．
      ...
      $n, m \geq N \Longrightarrow d(x_n, x_m) < \varepsilon$ が成り立つことをいう．
      $(\X, d)$ が完備とは，任意の Cauchy 列 $(x_n) \subset \X$ が
    ...
    \item \textbf{可分性}．
      ...
      $\exists q \in D,\; d(x, q) < \varepsilon$」と同値．
      $(\X, d)$ が可分とは可算な稠密部分集合 $D \subset \X$ が
...
\begin{claim}{最適解集合は輸送多面体の面をなす}{sem-optimal-face}
  最適解の集合
  \[
    S^*
    \defeq
...
\begin{proof}
  ...
  主張~\ref{clm:sem-coupling-polytope} の凸性から
  ...
  $t \in (0,1)$）とする．
  $\inner{\mathbf{C}}{\mathbf{P}},\, \inner{\mathbf{C}}{\mathbf{Q}}
  \geq \MKD_{\mathbf{C}}(\mathbf{a}, \mathbf{b})$
  かつ
  $t\inner{\mathbf{C}}{\mathbf{P}} + (1-t)\inner{\mathbf{C}}{\mathbf{Q}}
  = \MKD_{\mathbf{C}}(\mathbf{a}, \mathbf{b})$
  が成り立つには等号
...
\begin{claim}{頂点での最適解の達成}{sem-vertex-optimal}
  $S^*$ は $\CouplingsD(\mathbf{a}, \mathbf{b})$ の
  頂点を少なくとも1つ含む．
\end{claim}

\begin{proof}
  主張~\ref{clm:sem-optimal-face} より
...
\begin{remark}{最適輸送計画のスパース性}{sem-sparse-optimal}
  $\CouplingsD(\mathbf{a}, \mathbf{b})$ の頂点は
  非零成分を高々 $n + m - 1$ 個しか持たない
  （第~\ref{ch:seminar-02}~章
  命題~\ref{prop:sem-extremal-tree}）．
  全成分数 $nm$ と比べ，
  主張~\ref{clm:sem-vertex-optimal} が保証する
  頂点最適解は大幅にスパースである．
\end{remark}
...
\begin{remark}{エントロピー正則化による一意性の回復}{sem-entropic-uniqueness-motivation}
  離散 Kantorovich 問題の非一意性は
  目的関数 $\inner{\mathbf{C}}{\mathbf{P}}$ の線形性に由来する．
  第~\ref{ch:seminar-03-entropic}~章で導入する
  エントロピー正則化
  \[
  ...
  狭義凸（定義~\ref{def:sem-convex-function}）となるため，
  最適解は常に一意に定まる
...
\begin{remark}{Polish 空間上の Kantorovich 問題の解の存在}{sem-continuous-existence}
  $\X, \Y$ が Polish 空間で
  コスト関数 $c \colon \X \times \Y \to [0, +\infty]$ が
  下半連続であるとき，
  Kantorovich 問題（定義~\ref{def:sem-kantorovich}）の下限は
  最小値として達成される：
  \[
    \MK_c(\alpha, \beta)
  ...
  証明は以下の4段階による：
  \begin{enumerate}
    \item $\Couplings(\alpha, \beta)$ は確率測度の
          弱収束の位相で\textbf{緊密}（tight）である．
          周辺 $\alpha, \beta$ がそれぞれ Polish 空間上で緊密であることから，
          任意の $\varepsilon > 0$ に対して
          コンパクト集合 $K_\X \times K_\Y \subset \X \times \Y$ で
          $\sup_{\pi \in \Couplings(\alpha, \beta)}
          \pi((\X \times \Y) \setminus (K_\X \times K_\Y)) < \varepsilon$
          とできる．
    \item \textbf{Prokhorov の定理}により，
          Polish 空間上の緊密な確率測度族は弱位相で相対コンパクトである．
    \item $\Couplings(\alpha, \beta)$ は弱位相で閉集合であるから
          （弱収束の極限は周辺条件を保存するため），
          相対コンパクトかつ閉 = コンパクト．
    \item 下半連続コスト汎関数
          $\pi \mapsto \int c \, \d\pi$ は弱収束に関して下半連続であり，
          空でないコンパクト集合上で最小値を達成する．
  \end{enumerate}
  詳細は C.~Villani,
  \textit{Optimal Transport: Old and New}, Springer, 2009, Chapter~4 を参照．
\end{remark}
...
\begin{definition}{$c$-変換}{sem-c-transform}
  ...
  \[
    ...
    \qquad (x \in \X)
  \]
  を\textbf{$\bar c$-変換}という．
\end{definition}

\begin{remark}{連続版双対の形}{sem-continuous-dual-shape}
  連続版でも，双対制約は
  \[
    f(x) + g(y) \leq c(x,y)
