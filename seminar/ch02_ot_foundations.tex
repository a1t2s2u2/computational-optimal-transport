...
\begin{proof}
  ...
  単点 $\{x_k\}$ について
  \[
    \bigl((P_X)\pushforward \Phi(\mathbf{P})\bigr)(\{x_k\})
  ...
  $(P_X)\pushforward \Phi(\mathbf{P}) = \alpha$．同様に $\beta$ について．
...
\begin{proof}
  ...
  主問題は $\CouplingsD(\mathbf{a}, \mathbf{b})$ が空でなく有界（主張~\ref{clm:sem-discrete-kantorovich-existence}）なので
  定義~\ref{def:sem-lp} の LP として実行可能かつ有界であり，
