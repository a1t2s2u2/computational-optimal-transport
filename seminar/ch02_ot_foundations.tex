...
\begin{remark}{本章の設定}{sem-ch2-setup}
  ...
  本章では $\X, \Y$ を\textbf{完備可分な距離空間}
  （Polish 空間，定義~\ref{def:sem-polish}）とする．
  距離関数をそれぞれ $d_\X, d_\Y$ と書き，混同の恐れがない場合は
  単に $d$ と略す．$\X$ にはボレル $\sigma$-代数 $\Bb(\X)$
  （定義~\ref{def:sem-borel}）を入れて可測空間 $(\X, \Bb(\X))$ とみなし，
  $\X$ 上の確率測度全体を $\Mm_+^1(\X)$
  （定義~\ref{def:sem-probability-measure}）と記す．$\Y$ についても同様．
