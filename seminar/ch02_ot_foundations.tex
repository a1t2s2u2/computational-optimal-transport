...
\begin{example}{最適割当}{sem-assignment-cost-example}
  ...
  \begin{tikzpicture}[
    ...
    % σ=(1,2): 1→1, 2→2（最適）
    \draw[optarr] (w1) -- node[costlbl, midway] {$1$} (t1);
    \draw[optarr] (w2) -- node[costlbl, midway] {$1$} (t2);
    % σ=(2,1): 1→2, 2→1
    \draw[arr] (w1) -- node[costlbl, pos=0.25] {$4$} (t2);
    \draw[arr] (w2) -- node[costlbl, pos=0.25] {$2$} (t1);
  \end{tikzpicture}
  \end{center}
