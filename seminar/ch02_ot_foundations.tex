...
\begin{example}{Monge 写像が存在しない場合}{sem-monge-noexist}
  ...
  \]
  を考える（$\alpha$ は点 $0$ に質量 $1$ を集中させた Dirac 測度，
  $\beta$ は点 $-1$ と $1$ に質量 $\tfrac{1}{2}$ ずつ分散させた離散測度）．
  この $\alpha, \beta$ に対して $\mathcal{T}(\alpha, \beta) = \emptyset$，
  すなわち押し出し制約をみたす可測写像が存在しないことを示す．

  任意の可測写像 $T \colon \R \to \R$ に対し
  命題~\ref{prop:sem-dirac-pushforward} (i) より
  \[
    T\pushforward \alpha = T\pushforward \delta_0 = \delta_{T(0)}
  \]
  となる．これは点 $T(0) \in \R$ に質量 $1$ を集中させた Dirac 測度．

  押し出し制約 $T\pushforward \alpha = \beta$ が成立するためには
  $\delta_{T(0)} = \beta$ が必要．これを単点集合 $\{-1\}$ の上で
  測度として評価すると，
  \[
    \delta_{T(0)}(\{-1\}) =
    \begin{cases}
    ...
    \beta(\{-1\}) = \tfrac{1}{2}.
  \]
  どんな $T(0) \in \R$ を選んでも左辺は $\{0, 1\}$ のいずれかにしかならず，
  右辺の $\tfrac{1}{2}$ と一致しない．
  したがって押し出し制約を満たす $T$ は存在せず，
