...
\begin{tikzpicture}[scale=1.0]
  ...
  % --- π を表す同心楕円（紫の濃淡で同時分布の集中を表現）---
  \fill[purple!20, opacity=0.7] (1.5, 2.0) ellipse (1.4 and 0.9);
  \fill[purple!40, opacity=0.7] (1.5, 2.0) ellipse (0.9 and 0.55);
  \fill[purple!70, opacity=0.8] (1.5, 2.0) ellipse (0.45 and 0.27);
  \node[font=\small] at (1.5, 0.55) {$\pi$};

  % --- α: X への射影（下方の Gaussian 風曲線） ---
  \draw[blue, thick, smooth, samples=60, domain=0:4]
    plot (\x, {-1 + 0.7*exp(-(\x-1.5)*(\x-1.5)*1.5)});
  \node[blue, font=\small, anchor=north west] at (3.6, -0.3) {$\alpha$};
  \node[font=\footnotesize, gray, anchor=west] at (-0.2, -1.3)
    {$(P_\X)\pushforward \pi = \alpha$};

  % --- β: Y への射影（左方の Gaussian 風曲線） ---
  \draw[red, thick, smooth, samples=60, domain=0:4, variable=\y]
    plot ({-1 + 0.7*exp(-(\y-2.0)*(\y-2.0)*1.5)}, \y);
