...
\begin{definition}{Kantorovich 問題}{sem-kantorovich}
  ...
  $\alpha$ と $\beta$ の\textbf{カップリング}という．
\end{definition}

\begin{remark}{周辺条件の意味}{sem-marginal-condition}
  カップリングの条件 $(P_\X)\pushforward \pi = \alpha$ を具体的に展開する．
  任意のボレル集合 $A \in \Bb(\X)$ をとる．
  押し出しの定義（定義~\ref{def:sem-pushforward}）により
  \[
...
\begin{definition}{線形計画問題}{sem-lp}
  ...
  $\mathcal{F} \neq \emptyset$ のとき LP は\textbf{実行可能}，
  $\inf_{\mathbf{x} \in \mathcal{F}} \mathbf{c}^\top \mathbf{x} > -\infty$ のとき\textbf{有界}であるという．
\end{definition}
...
\]
（相異なる $x_1, \ldots, x_n$，$y_1, \ldots, y_m$，
$\mathbf{a} \in \R_{++}^n$，$\mathbf{b} \in \R_{++}^m$，
$\sum_i a_i = \sum_j b_j = 1$）の場合を考える．
コスト関数を離散化した行列
$\mathbf{C} \in \R_+^{n \times m}$，$C_{i,j} \defeq c(x_i, y_j)$ を定める．

\begin{proposition}{離散 Kantorovich 問題}{sem-discrete-kantorovich}
  Kantorovich 問題（定義~\ref{def:sem-kantorovich}）は
  \textbf{離散カップリング集合}
  \[
  ...
  この有限次元問題を\textbf{離散 Kantorovich 問題}という．
\end{proposition}
...
\begin{proof}
  ...
  したがって $\pi$ は $\{(x_i, y_j)\}_{i, j}$ 上のみ質量を持つ．
  $P_{i,j} \defeq \pi(\{(x_i, y_j)\}) \geq 0$ とおくと，
  任意のボレル集合 $A \in \Bb(\X \times \Y)$ に対して $\sigma$-加法性より
  \[
    \pi(A) = \sum_{(x_i,y_j) \in A} P_{i,j}
           = \Bigl(\sum_{i,j} P_{i,j}\,\delta_{(x_i,y_j)}\Bigr)(A),
  \]
  すなわち $\pi = \sum_{i,j} P_{i,j} \delta_{(x_i, y_j)}$．
  単点 $\{x_k\}$ における周辺条件
...
双対の構造の理解が Sinkhorn の導出の鍵になる（定理~\ref{thm:sem-sinkhorn-scaling-form}）．
...
\begin{remark}{双対変数と Sinkhorn アルゴリズムの接続}{sem-dual-sinkhorn-connection}
  ...
  中心的な役割を果たす．
  正則化版の最適解は双対変数を通じて陽に表現でき（定理~\ref{thm:sem-sinkhorn-scaling-form}），
