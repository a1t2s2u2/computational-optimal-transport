...
$n$ 箇所にそれぞれ質量 $1/n$ の砂山があり，
別の $n$ 箇所に同量ずつ運んで積み上げたい．
各砂山は丸ごと 1 つの行き先へ運ぶものとする（1 対 1 対応）．
砂山 $i$ を行き先 $j$ へ運ぶコストが $C_{i,j}$ で与えられているとき，
総コストを最小にする対応づけを求めたい．

\begin{definition}{最適割当問題（optimal assignment problem）}{sem-assignment}
  置換 $\sigma = (\sigma(1), \sigma(2), \ldots, \sigma(n)) \in \Perm(n)$ と
  行列 $\mathbf{C} \in \R_+^{n \times n}$
  （成分 $C_{i,j}$ を添字 $i$ から $j$ への\textbf{輸送コスト}と解釈する）
  に対して，\textbf{最適割当問題}とは，
  \[
    \min_{\sigma \in \Perm(n)} \frac{1}{n} \sum_{i=1}^{n} C_{i, \sigma(i)}
  \]
  を達成する $\sigma$ を求める問題である．
\end{definition}

\begin{example}{最適割当}{sem-assignment-cost-example}
  $n = 2$ とし，置換 $\sigma = (\sigma(1), \sigma(2)) \in \Perm(2)$ と
  行列 $\mathbf{C} \in \R_+^{2 \times 2}$ を考える．
  直線上の砂山の位置 $x_1 = 0, x_2 = 3$ と行き先の位置 $y_1 = 1, y_2 = 4$ から，
  運搬コストを距離 $C_{i,j} \defeq |x_i - y_j|$ で与えると，
  \[
    \mathbf{C} =
    \begin{pmatrix}
      |0-1| & |0-4| \\
    ...
    \end{pmatrix}.
  \]
  $\Perm(2) = \{(1,2),\, (2,1)\}$ の $2! = 2$ 通りの割当を直線上に図示する．
  青丸（{\color{blue!60}$\bullet$}）が砂山，
  赤四角（{\color{red!60}$\blacksquare$}）が行き先であり，
...
\begin{proposition}{最適解の存在}{sem-assignment-existence}
  任意の $\mathbf{C} \in \R_+^{n \times n}$ に対して，
  最適割当問題の最小値は達成される．すなわち
  \[
    \frac{1}{n} \sum_{i=1}^{n} C_{i, \sigma^*(i)}
    = \min_{\sigma \in \Perm(n)} \frac{1}{n} \sum_{i=1}^{n} C_{i, \sigma(i)}
  \]
  を満たす $\sigma^* \in \Perm(n)$ が存在する．
\end{proposition}

\begin{proof}
  $\Perm(n)$ は $|\Perm(n)| = n!$ の有限集合なので，
  $\sigma$ を動かして得られる値の集合
  \[
    \left\{ \frac{1}{n}\sum_{i=1}^{n} C_{i, \sigma(i)} \;\middle|\; \sigma \in \Perm(n) \right\}
  \]
  は高々 $n!$ 個の実数からなる有限集合である．
  実数の空でない有限集合は最小元を持つから，
  これを達成する $\sigma^* \in \Perm(n)$ が存在する．
\end{proof}
...
最適割当問題では，同数の砂山を等しい質量で 1 対 1 に運んでいた．
これを次のように一般化したい：
\begin{itemize}
  \item[(i)] 砂山の数 $n$ と行き先の数 $m$ は等しくなくてよい．
  \item[(ii)] 各砂山の質量と各行き先の必要量は不均一でよい．
  \item[(iii)] 1 つの行き先に複数の砂山の砂が集まるのは許すが，
    1 つの砂山は丸ごと 1 つの行き先へ運ぶ（分割しない）．
\end{itemize}

1781 年に Monge が「砂山を最小の労力で積み替える問題」として提唱したこの問題は，
砂山の分布を測度 $\alpha$，行き先の分布を測度 $\beta$，
どの砂山をどこへ運ぶかを写像 $T$ で表すことで，次のように定式化される．
...
\begin{definition}{Monge 問題（Monge problem）}{sem-monge}
  ...
  （$c(x, y)$ を点 $x$ から点 $y$ への\textbf{輸送コスト}と解釈する）に対して，
  \textbf{Monge 問題}とは，
  \[
    \inf_{T}
    \left\{
...
確率単体の元 $\mathbf{a} = (a_1, \ldots, a_n) \in \simplex_n$，
$\mathbf{b} = (b_1, \ldots, b_m) \in \simplex_m$ はそれぞれ各砂山の質量・各行き先の必要量を表す．
...
このとき離散測度の押し出し（命題~\ref{prop:sem-dirac-pushforward}）より
\[
  T\pushforward \alpha
