...
\begin{remark}{本章の設定}{sem-ch2-setup}
  Cuturi の論文では $\X, \Y$ を単に距離空間としているが，
  本稿では $\X, \Y$ を\textbf{完備可分な距離空間}（Polish 空間，
  定義~\ref{def:sem-polish}）とし，
  ボレル $\sigma$-代数 $\Bb(\X), \Bb(\Y)$（定義~\ref{def:sem-borel}）を備えた
  可測空間として扱う．
  距離 $d$ そのものが式に現れる場面は少ないが，
  距離（および Polish 性）を仮定するのは次の理由による：
  \begin{enumerate}
    \item[(i)] 距離から位相（定義~\ref{def:sem-metric-topology}）・
      ボレル $\sigma$-代数（定義~\ref{def:sem-borel}）・
      可測関数（定義~\ref{def:sem-measurable-function}）・
      測度の弱収束（定義~\ref{def:sem-weak-convergence}）が一括で定まる．
    \item[(ii)] Polish 性により，有限ボレル測度のタイト性
      （命題~\ref{prop:sem-ulam}）・条件付き測度の取り出し
      （\textbf{disintegration}，命題~\ref{prop:sem-disintegration}）・
      弱コンパクト性（\textbf{Prokhorov}，命題~\ref{prop:sem-prokhorov}）が使える．
      これらは Kantorovich 問題の解の存在および双対定理の基盤となる．
    \item[(iii)] Wasserstein 距離では輸送コストを $c(x, y) = d(x, y)^p$ に取るため，
      $\X$ 上の距離（\textbf{ground 距離}）そのものが式に現れる．
  \end{enumerate}
  離散版（$\X = \{x_1, \ldots, x_n\}$ が有限集合）では，
  任意の距離 $d$ のもとで $(\X, d)$ は Polish 空間となる
  （命題~\ref{prop:sem-finite-polish}）．
  したがって本章の設定は離散版に対して追加の制約を課さない．
\end{remark}

\begin{proposition}{有限集合は任意の距離で Polish}{sem-finite-polish}
  $n \in \N$ とし，相異なる $n$ 点からなる有限集合
  $\X = \{x_1, \ldots, x_n\}$ に任意の距離 $d$
  （定義~\ref{def:sem-metric-space}）を入れた距離空間 $(\X, d)$ は，
  Polish 空間（定義~\ref{def:sem-polish}）である．
\end{proposition}
...
\begin{proof}
  ...
  \textbf{完備性．}
  $n = 1$ のとき $\X$ 上の任意の列は $x_1$ の定値列であり $x_1 \in \X$
  に収束するから，以下 $n \geq 2$ とする．相異なる 2 点間の距離の最小値を
  \[
    \delta \defeq \min_{1 \leq i < j \leq n} d(x_i, x_j)
  \]
  と置く．距離の同一性（定義~\ref{def:sem-metric-space}(i)）より
  $i \neq j$ ならば $d(x_i, x_j) > 0$ であり，
  これは有限個の正実数の最小値だから $\delta > 0$．

  Cauchy 列 $(y_k)_{k \geq 1} \subseteq \X$ をとる．
  Cauchy 条件（定義~\ref{def:sem-complete-metric}）を
  $\varepsilon \defeq \delta / 2$ に対して適用すると，
  ある $N \in \N$ が存在して
  \[
    k, l \geq N \;\Longrightarrow\; d(y_k, y_l) < \delta / 2 < \delta.
  \]
  一方，$\X$ の相異なる 2 点間の距離は $\delta$ 以上だから，
  $d(y_k, y_l) < \delta$ は $y_k = y_l$ を強制する．
  したがって $k \geq N$ である $y_k$ はすべて同一の点
  $y_\infty \in \X$ であり，$d(y_k, y_\infty) = 0$ ($k \geq N$) が成り立つ．
  ゆえに $\lim_{k \to \infty} d(y_k, y_\infty) = 0$，
  すなわち $(y_k)$ は $y_\infty \in \X$ に収束する．
  これで $(\X, d)$ は完備．

  \textbf{可分性．}
  $\X$ 自身が有限集合，特に可算集合である．
  任意の $x \in \X$ と任意の $\varepsilon > 0$ に対し，
  $x \in \X$ 自身が $d(x, x) = 0 < \varepsilon$ を満たすから，
  $\X$ は $\X$ の中で稠密（定義~\ref{def:sem-separable}）．
  したがって $\X$ は可算稠密部分集合 $\X$ を持ち，可分．

  以上より $(\X, d)$ は完備かつ可分な距離空間，
  すなわち Polish 空間（定義~\ref{def:sem-polish}）である．
\end{proof}
...
質量 $1/n$ の砂山 $n$ 個を同数の行き先に 1 対 1 で運ぶ．
コスト行列 $\mathbf{C} \in \R_+^{n \times n}$ のもとで総コストを最小化する割当を求める．

\begin{definition}{最適割当問題（optimal assignment problem）}{sem-assignment}
  $n \in \N$，置換 $\sigma = (\sigma(1), \ldots, \sigma(n)) \in \Perm(n)$，
  および行列 $\mathbf{C} \in \R_+^{n \times n}$
  （$C_{i,j}$ は添字 $i$ から $j$ への\textbf{輸送コスト}）に対して，
  \[
...
Monge 問題では次の設定を扱う：
\begin{itemize}
  \item[(i)] 砂山の数と行き先の数は等しくなくてよい．
  \item[(ii)] 各砂山の質量と各行き先で積み上げる量は不均一でよい．
  \item[(iii)] 複数の砂山が同じ行き先に運ばれてもよい．
  \item[(iv)] 1 つの砂山は丸ごと 1 つの行き先へ運ぶ（\textbf{分割しない}）．
\end{itemize}

Monge (1781) はこれを次のように定式化した．

\begin{definition}{Monge 問題（Monge problem）}{sem-monge}
  可測空間 $\X, \Y$ 上の確率測度 $\alpha \in \Mm_+^1(\X)$, $\beta \in \Mm_+^1(\Y)$ と
...
実際，$\mathcal{T}(\alpha,\beta)$ が凸であるためには，
任意の $T_1, T_2 \in \mathcal{T}(\alpha,\beta)$ と
任意の $t \in [0,1]$ に対して
$\bar{T} \defeq t T_1 + (1-t)T_2$ が再び
...
最適割当問題は $n = m$ かつ一様な重みの場合しか扱えず，
Monge 問題はこれを拡張するものの，組合せ的な分割不可制約を残す．
Kantorovich (1942) はこれを，質量の\textbf{分割}を許す確率的輸送に置き換え，
$\X \times \Y$ 上の結合測度（\textbf{カップリング}）として定式化した．

\subsection{カップリング}

\begin{definition}{カップリング（coupling）}{sem-coupling}
  可測空間 $\X, \Y$ 上の確率測度 $\alpha \in \Mm_+^1(\X)$, $\beta \in \Mm_+^1(\Y)$
