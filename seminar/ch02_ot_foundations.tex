...
\begin{remark}{本章の設定}{sem-ch2-setup}
  ...
  Polish 空間の各性質が果たす役割：
  \begin{itemize}
    \item \textbf{距離空間}．開集合・ボレル $\sigma$-代数を定め，
      可測写像 $T \colon \X \to \Y$ を意味付ける．
    \item \textbf{完備性}．Cauchy 列が収束することから，
      連続版 OT の解の存在（Prokhorov の定理によるカップリング集合の
      弱コンパクト性）の基盤となる．本章では離散版を扱うため
      陽には用いないが，連続版への接続には不可欠．
    \item \textbf{可分性}．可算稠密部分集合の存在は，
      ボレル $\sigma$-代数が可算生成可能であることを保証し，
      確率測度空間 $\Mm_+^1(\X)$ の距離化を可能にする．
  \end{itemize}
