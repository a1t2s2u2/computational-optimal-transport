...
\begin{remark}{本章の設定}{sem-ch2-setup}
  ...
  \begin{itemize}
    \item \textbf{距離空間}．距離関数
      $d \colon \X \times \X \to [0, \infty)$ が
      同一性・対称性・三角不等式（定義~\ref{def:sem-metric-space}）を満たす．
      開球を
      \[
        B_r(x) \defeq \{y \in \X \mid d(x, y) < r\}
      \]
      で定める（定義~\ref{def:sem-open-ball}）．
      $U \subset \X$ が\textbf{開集合}とは，
      任意の $x \in U$ に対し $B_r(x) \subset U$ をみたす $r > 0$ が
      とれることをいう．$\X$ のすべての開集合を含む最小の $\sigma$-代数を
      \textbf{ボレル $\sigma$-代数} $\Bb(\X)$ という
      （定義~\ref{def:sem-borel}）．
      写像 $T \colon \X \to \Y$ が\textbf{可測}とは，
      任意の $B \in \Bb(\Y)$ に対し $T^{-1}(B) \in \Bb(\X)$ が
      成立することをいう（定義~\ref{def:sem-measurable-map}）．
  ...
  $S$ にどの距離を入れた距離空間 $(S, d)$ も自動で Polish 空間になる：
  \begin{itemize}
