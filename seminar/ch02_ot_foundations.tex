...
質量 $1/n$ の砂山が $n$ 個と，同数の行き先がある．
各砂山を丸ごと 1 つの行き先へ運ぶ（1 対 1 対応）ものとし，
コスト行列 $\mathbf{C} \in \R_+^{n \times n}$ のもとで総コストを最小にする割当を求めたい．
...
Monge (1781) はこれを，砂山の分布 $\alpha$，行き先の分布 $\beta$，
輸送写像 $T$ によって次のように定式化した．
...
Monge 問題は\textbf{実行可能集合の非凸性}（困難 1）と
\textbf{解の非存在}（困難 2）の 2 つの困難を抱える．順に見ていく．
...
\begin{memo*}
背景：非凸性の根本原因は，写像から測度への対応 $T \longmapsto T\pushforward\alpha$ が
\textbf{非線形}であること．実際「押し出してから平均」すると
$\tfrac{1}{2}{T_1}\pushforward\alpha + \tfrac{1}{2}{T_2}\pushforward\alpha = \beta$ だが，
「平均してから押し出す」と $\bar{T}\pushforward\alpha = \delta_0 \ne \beta$．一般に
$(tT_1 + (1-t)T_2)\pushforward \alpha \neq
