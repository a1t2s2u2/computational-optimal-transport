...
\begin{claim}{摂動つき最小化の極限}{sem-perturbation-selection}
  $X$ をコンパクト集合，$L, H \colon X \to \R$ を連続関数とし，
  各 $\varepsilon > 0$ に対して
  \[
    x_\varepsilon
    \in
    \argmin_{x \in X}\bigl\{L(x) - \varepsilon H(x)\bigr\}
  \]
  とおく．また
  \[
    L^* \defeq \min_{x \in X} L(x), \qquad
    X^* \defeq \argmin_{x \in X} L(x)
  \]
  とする．このとき，$\varepsilon \to 0$ における $x_\varepsilon$ の
  任意の集積点は $X^*$ の中で $H$ を最大化する点である．
  特に最大化点が一意なら
  \[
    x_\varepsilon \to \argmax_{x \in X^*} H(x)
    \qquad (\varepsilon \to 0).
  \]
\end{claim}

\begin{proof}
  任意の $q \in X^*$ に対して，$x_\varepsilon$ の最適性より
  \[
    L(x_\varepsilon) - \varepsilon H(x_\varepsilon)
    \leq
    L(q) - \varepsilon H(q)
    = L^* - \varepsilon H(q).
  \]
  したがって
  \[
    0 \leq L(x_\varepsilon) - L^*
    \leq \varepsilon\bigl(H(x_\varepsilon) - H(q)\bigr).
  \]
  $H$ はコンパクト集合 $X$ 上で有界なので，$L(x_\varepsilon) \to L^*$ が従う．
  また左辺の非負性から $H(x_\varepsilon) \geq H(q)$ でもある．

  収束部分列 $x_{\varepsilon_k} \to \bar{x}$ をとると，連続性より
  \[
    L(\bar{x}) = L^*, \qquad H(\bar{x}) \geq H(q).
  \]
  これは任意の $q \in X^*$ について成り立つので，
  $\bar{x}$ は $X^*$ 上で $H$ を最大化する．
  最大化点が一意なら任意の集積点はその点に一致するから，
  全体の収束が従う（主張~\ref{clm:sem-subseq-limit-unique}）．
\end{proof}
...
\begin{proof}
  ...
  $\mathbf{P}_\varepsilon
  = \argmin_{\mathbf{P} \in \CouplingsD(\mathbf{a},\mathbf{b})}
  \bigl\{\inner{\mathbf{C}}{\mathbf{P}} - \varepsilon\Hb(\mathbf{P})\bigr\}$
  である．
  $\CouplingsD(\mathbf{a},\mathbf{b})$ はコンパクト，
  $\inner{\mathbf{C}}{\cdot}$ と $\Hb$ は連続だから，
  主張~\ref{clm:sem-perturbation-selection} を
  $X = \CouplingsD(\mathbf{a},\mathbf{b})$，$L = \inner{\mathbf{C}}{\cdot}$，$H = \Hb$
  として適用できる．

  $S^*$ は凸かつコンパクトであり，
  $-\Hb$ は連続で狭義凸（主張~\ref{clm:sem-neg-entropy-strict-convex}）だから，
  $S^*$ 上のエントロピー最大化点
  $\mathbf{P}^\star \defeq \argmax_{\mathbf{P} \in S^*}\Hb(\mathbf{P})$
  は一意である
  （Weierstrass の定理（定理~\ref{thm:sem-weierstrass}）と
  命題~\ref{prop:sem-strict-convex-unique-min}）．
  よって $\mathbf{P}_\varepsilon \to \mathbf{P}^\star$（$\varepsilon \to 0$）を得る．
  ...
  \]
  の $\eta \to 0$ の極限を調べることになる．
  これは $\Hb$ の最大化に小さい摂動 $-\eta\inner{\mathbf{C}}{\cdot}$ を加えたものである．
  主張~\ref{clm:sem-perturbation-selection} を
  $X = \CouplingsD(\mathbf{a},\mathbf{b})$，$L = -\Hb$，$H = -\inner{\mathbf{C}}{\cdot}$
  として適用する．
  主張~\ref{clm:sem-max-entropy-independent} より，
  $\CouplingsD(\mathbf{a},\mathbf{b})$ 上で $\Hb$ を最大化する唯一の元は
  $\mathbf{a}\mathbf{b}^\top$ だから，$X^* = \{\mathbf{a}\mathbf{b}^\top\}$ は一点集合であり，
