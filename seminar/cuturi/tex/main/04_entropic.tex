...
\begin{proof}
  ...
  $\eta_k \to 0$ を任意にとり，コンパクト性より，
  必要なら部分列をとって
  $\widetilde{\mathbf{P}}_{\eta_k} \to \mathbf{P}^\infty$ とする．
  任意の $\mathbf{Q} \in \CouplingsD(\mathbf{a},\mathbf{b})$ に対して，
  $\widetilde{\mathbf{P}}_{\eta_k}$ の最大性より
  \[
    \Hb(\widetilde{\mathbf{P}}_{\eta_k})
    - \eta_k\inner{\mathbf{C}}{\widetilde{\mathbf{P}}_{\eta_k}}
    \geq
    \Hb(\mathbf{Q})
    - \eta_k\inner{\mathbf{C}}{\mathbf{Q}}.
  \]
  $\inner{\mathbf{C}}{\cdot}$ はコンパクト集合上で有界であり，
  $\eta_k \to 0$ だから，極限をとると
  $\Hb(\mathbf{P}^\infty) \geq \Hb(\mathbf{Q})$ を得る．
  これは任意の $\mathbf{Q}$ について成り立つので，
  $\mathbf{P}^\infty$ は $\CouplingsD(\mathbf{a},\mathbf{b})$ 上で $\Hb$ を最大化する．
  主張~\ref{clm:sem-max-entropy-independent} より
  $\mathbf{P}^\infty = \mathbf{a}\mathbf{b}^\top$．

  任意の収束部分列の極限がすべて $\mathbf{a}\mathbf{b}^\top$ に一致するから
