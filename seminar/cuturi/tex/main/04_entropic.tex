...
\begin{proof}
  ...
  $\inner{\mathbf{C}}{\mathbf{P}_\varepsilon} \to \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$
  である．
  したがって $\mathbf{P}_\varepsilon$ は $S^*$ に近づく．
  実際，もし $S^*$ から一定以上離れた点が残るなら，
  コンパクト性と $\inner{\mathbf{C}}{\cdot}$ の連続性より，
  その点ではコストが $\MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$ より
  正の幅だけ大きくなり，上の収束に反する．

  さらに，(i) の証明で得た不等式より
  $\Hb(\mathbf{P}_\varepsilon) \geq \Hb(\widehat{\mathbf{P}})$
  が成り立つ．
  ここで $\widehat{\mathbf{P}}$ は $S^*$ 上の一意なエントロピー最大解だから，
  $S^*$ の中で $\widehat{\mathbf{P}}$ から離れた点では
  エントロピーは $\Hb(\widehat{\mathbf{P}})$ より真に小さい．
  一方 $\mathbf{P}_\varepsilon$ は $S^*$ に近づき，
  かつ $\Hb(\mathbf{P}_\varepsilon) \geq \Hb(\widehat{\mathbf{P}})$ をみたすから，
  $\mathbf{P}_\varepsilon$ は $\widehat{\mathbf{P}}$ から離れることができない．
  ゆえに $\mathbf{P}_\varepsilon \to \widehat{\mathbf{P}}$（$\varepsilon \to 0$）を得る．
