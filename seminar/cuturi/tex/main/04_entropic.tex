...
\begin{proof}
  ...
  はただ一つに定まる：$S^*$ は凸かつコンパクトであり，
  $-\Hb$ は連続で狭義凸（主張~\ref{clm:sem-neg-entropy-strict-convex}）だから，
  Weierstrass の定理（定理~\ref{thm:sem-weierstrass}）より
  $-\Hb$ は $S^*$ 上で最小値を達成し，
  その最小点は命題~\ref{prop:sem-strict-convex-unique-min} より一意である．

  $0$ に収束する正の列 $(\varepsilon_k)$ を任意にとり，
  $\mathbf{P}_{\varepsilon_k} \to \mathbf{P}^\star$ を示す．
  主張~\ref{clm:sem-subseq-limit-unique} により，
  収束する部分列の極限がすべて $\mathbf{P}^\star$ に等しいことを示せば十分である．
  収束部分列 $\mathbf{P}_{\varepsilon_{k_j}} \to \mathbf{Q} \in \CouplingsD(\mathbf{a},\mathbf{b})$ をとる．
  内積の連続性と (i) より
  ...
  だから $\mathbf{Q} \in S^*$．
  また上の不等式と $\Hb$ の連続性より
  $\Hb(\mathbf{Q}) \geq \Hb(\mathbf{P}^*)$（$\forall \mathbf{P}^* \in S^*$）
  となるから，$\mathbf{Q}$ は $S^*$ 上で $\Hb$ を最大化し，
  一意性より $\mathbf{Q} = \mathbf{P}^\star$．
  列 $(\varepsilon_k)$ は任意だったから
  $\mathbf{P}_\varepsilon \to \mathbf{P}^\star$（$\varepsilon \to 0$）を得る．
