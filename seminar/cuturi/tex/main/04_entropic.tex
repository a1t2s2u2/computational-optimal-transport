...
\begin{proof}
  ...
  \textbf{(ii).}
  $S^*$ はコンパクト凸集合であり，$\Hb$ は狭義凹かつ連続だから，
  $\widehat{\mathbf{P}} \defeq \argmax_{\mathbf{P} \in S^*}\Hb(\mathbf{P})$
  は一意に存在する．
  (i) の証明より
  $\inner{\mathbf{C}}{\mathbf{P}_\varepsilon} \to \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$
  であり，また
  $\Hb(\mathbf{P}_\varepsilon) \geq \Hb(\widehat{\mathbf{P}})$
  が成り立つ．
  以下 $\mathbf{P}_\varepsilon \to \widehat{\mathbf{P}}$ を示す．

  任意の $\delta > 0$ をとり，
  \[
    A_\delta
    \defeq
    \bigl\{\, \mathbf{P} \in S^* \bigm|
      \|\mathbf{P} - \widehat{\mathbf{P}}\| \geq \delta/2
    \,\bigr\}
  \]
  とおく．$A_\delta$ はコンパクトであり $\widehat{\mathbf{P}} \notin A_\delta$ だから，
  $\widehat{\mathbf{P}}$ が $S^*$ 上の $\Hb$ の一意な最大点であることより
  \[
    \gamma_\delta
    \defeq
    \Hb(\widehat{\mathbf{P}}) - \max_{\mathbf{P} \in A_\delta}\Hb(\mathbf{P})
    > 0.
  \]

  $\Hb$ はコンパクト集合 $\CouplingsD(\mathbf{a},\mathbf{b})$ 上で一様連続だから，
  ある $\rho_\delta \in (0, \delta/2)$ が存在して
  \[
    \|\mathbf{P} - \mathbf{Q}\| < \rho_\delta
    \quad\Longrightarrow\quad
    |\Hb(\mathbf{P}) - \Hb(\mathbf{Q})| < \gamma_\delta/2.
  \]

  次に
  $B_\delta
  \defeq
  \bigl\{\, \mathbf{P} \in \CouplingsD(\mathbf{a},\mathbf{b}) \bigm|
    \operatorname{dist}(\mathbf{P}, S^*) \geq \rho_\delta
  \,\bigr\}$
  とおく．$B_\delta$ はコンパクトで $B_\delta \cap S^* = \varnothing$ だから
  \[
    \eta_\delta
    \defeq
    \min_{\mathbf{P} \in B_\delta}
    \bigl(\inner{\mathbf{C}}{\mathbf{P}} - \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})\bigr)
    > 0.
  \]
  $\inner{\mathbf{C}}{\mathbf{P}_\varepsilon} \to \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$
  だから，十分小さい $\varepsilon > 0$ について
  $\inner{\mathbf{C}}{\mathbf{P}_\varepsilon} - \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b}) < \eta_\delta$
  となり，$\mathbf{P}_\varepsilon \notin B_\delta$．
  よって $\operatorname{dist}(\mathbf{P}_\varepsilon, S^*) < \rho_\delta$ であり，
  ある $\mathbf{Q}_\varepsilon \in S^*$ が存在して
  $\|\mathbf{P}_\varepsilon - \mathbf{Q}_\varepsilon\| < \rho_\delta$ となる．

  ここで $\|\mathbf{P}_\varepsilon - \widehat{\mathbf{P}}\| \geq \delta$ と仮定すると，
  $\rho_\delta < \delta/2$ より
  \[
    \|\mathbf{Q}_\varepsilon - \widehat{\mathbf{P}}\|
    \geq
    \|\mathbf{P}_\varepsilon - \widehat{\mathbf{P}}\|
    - \|\mathbf{P}_\varepsilon - \mathbf{Q}_\varepsilon\|
    > \delta/2
  \]
  だから $\mathbf{Q}_\varepsilon \in A_\delta$ であり，
  $\Hb(\mathbf{Q}_\varepsilon) \leq \Hb(\widehat{\mathbf{P}}) - \gamma_\delta$．
  一様連続性と $\|\mathbf{P}_\varepsilon - \mathbf{Q}_\varepsilon\| < \rho_\delta$ より
  \[
    \Hb(\mathbf{P}_\varepsilon)
    \leq \Hb(\mathbf{Q}_\varepsilon) + \gamma_\delta/2
    \leq \Hb(\widehat{\mathbf{P}}) - \gamma_\delta/2
  \]
  となり，(i) で得た
  $\Hb(\mathbf{P}_\varepsilon) \geq \Hb(\widehat{\mathbf{P}})$ に反する．

  したがって十分小さい $\varepsilon > 0$ について
  $\|\mathbf{P}_\varepsilon - \widehat{\mathbf{P}}\| < \delta$ であり，
  $\delta > 0$ は任意だったから
  $\mathbf{P}_\varepsilon \to \widehat{\mathbf{P}}$（$\varepsilon \to 0$）を得る．

  \textbf{(iii).}
  目的関数を $\varepsilon$ で割ると，
  $\inner{\mathbf{C}}{\mathbf{P}} - \varepsilon\Hb(\mathbf{P})$ の最小化は
  $\Hb(\mathbf{P}) - \tfrac{1}{\varepsilon}\inner{\mathbf{C}}{\mathbf{P}}$ の最大化と同値である．
  $\eta \defeq 1/\varepsilon$ とおけば $\varepsilon \to +\infty$ は $\eta \to 0$ に対応し，
  $\mathbf{P}_\eta$ は
  \[
    \argmax_{\mathbf{P} \in \CouplingsD(\mathbf{a},\mathbf{b})}
    \bigl\{\Hb(\mathbf{P}) - \eta\inner{\mathbf{C}}{\mathbf{P}}\bigr\}
  \]
  の元である．
  $+\infty$ に発散する正の列 $(\varepsilon_k)$ を任意にとり，
  $\eta_k \defeq 1/\varepsilon_k \to 0$ とする．
  収束部分列 $\mathbf{P}_{\eta_{k_j}} \to \mathbf{P}^\infty \in \CouplingsD(\mathbf{a},\mathbf{b})$ をとる．
  任意の $\mathbf{Q} \in \CouplingsD(\mathbf{a},\mathbf{b})$ に対して最大性から
  \[
    \Hb(\mathbf{P}_{\eta_{k_j}}) - \eta_{k_j}\inner{\mathbf{C}}{\mathbf{P}_{\eta_{k_j}}}
    \geq
    \Hb(\mathbf{Q}) - \eta_{k_j}\inner{\mathbf{C}}{\mathbf{Q}}.
  \]
  $\inner{\mathbf{C}}{\cdot}$ はコンパクト集合上で有界だから，
  $\eta_{k_j} \to 0$ で両辺の $\eta$ の項は消えて
  $\Hb(\mathbf{P}^\infty) \geq \Hb(\mathbf{Q})$ を得る．
  これがすべての $\mathbf{Q}$ について成り立つから，
  $\mathbf{P}^\infty$ は $\CouplingsD(\mathbf{a},\mathbf{b})$ 上で $\Hb$ を最大化し，
  主張~\ref{clm:sem-max-entropy-independent} より
  $\mathbf{P}^\infty = \mathbf{a}\mathbf{b}^\top$．
  収束する部分列の極限がすべて
  $\mathbf{a}\mathbf{b}^\top$ に等しいから（主張~\ref{clm:sem-subseq-limit-unique}），
