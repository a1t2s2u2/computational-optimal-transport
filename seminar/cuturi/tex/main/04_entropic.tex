...
\begin{proof}
  ...
  (i) の証明より
  $\inner{\mathbf{C}}{\mathbf{P}_\varepsilon} \to \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$
  であり，また同じく (i) の証明で得た評価より，
  任意の $\mathbf{R} \in S^*$ に対して
  $\Hb(\mathbf{P}_\varepsilon) \geq \Hb(\mathbf{R})$ である．
  特に $\Hb(\mathbf{P}_\varepsilon) \geq \Hb(\widehat{\mathbf{P}})$ が成り立つ．
  ...
  \textbf{(iii).}
  $\eta \defeq 1/\varepsilon$ とおく．
  $\varepsilon \to +\infty$ は $\eta \to 0$ に対応する．
  また，$\inner{\mathbf{C}}{\mathbf{P}} - \varepsilon\Hb(\mathbf{P})$ を最小化することは，
  $\Hb(\mathbf{P}) - \eta\inner{\mathbf{C}}{\mathbf{P}}$ を最大化することと同値である．
  そこで $\widetilde{\mathbf{P}}_\eta \defeq \mathbf{P}_{1/\eta}$ と書けば，
  \[
    \widetilde{\mathbf{P}}_\eta
    \in
    \argmax_{\mathbf{P} \in \CouplingsD(\mathbf{a},\mathbf{b})}
    \bigl\{\Hb(\mathbf{P}) - \eta\inner{\mathbf{C}}{\mathbf{P}}\bigr\}.
  \]

  $\eta_k \to 0$ となる任意の正の列 $(\eta_k)_k$ をとる．
  $\CouplingsD(\mathbf{a},\mathbf{b})$ はコンパクトであり，
  $\widetilde{\mathbf{P}}_{\eta_k} \in \CouplingsD(\mathbf{a},\mathbf{b})$ だから，
  ある部分列 $(\eta_{k_j})_j$ と
  $\mathbf{P}^\infty \in \CouplingsD(\mathbf{a},\mathbf{b})$ が存在して
  $\widetilde{\mathbf{P}}_{\eta_{k_j}} \to \mathbf{P}^\infty$ となる．

  任意の $\mathbf{Q} \in \CouplingsD(\mathbf{a},\mathbf{b})$ をとる．
  $\widetilde{\mathbf{P}}_{\eta_{k_j}}$ の最大性より
  \[
    \Hb(\widetilde{\mathbf{P}}_{\eta_{k_j}})
    - \eta_{k_j}\inner{\mathbf{C}}{\widetilde{\mathbf{P}}_{\eta_{k_j}}}
    \geq
    \Hb(\mathbf{Q})
    - \eta_{k_j}\inner{\mathbf{C}}{\mathbf{Q}}.
  \]
  ここで $\inner{\mathbf{C}}{\cdot}$ は
  コンパクト集合 $\CouplingsD(\mathbf{a},\mathbf{b})$ 上で有界であり，
  $\eta_{k_j} \to 0$ だから，
  \[
    \eta_{k_j}\inner{\mathbf{C}}{\widetilde{\mathbf{P}}_{\eta_{k_j}}} \to 0,
    \qquad
    \eta_{k_j}\inner{\mathbf{C}}{\mathbf{Q}} \to 0.
  \]
  また $\Hb$ は連続なので，極限をとると
  ...
  以上より，$\eta_k \to 0$ となる任意の列に対して，
  その任意の収束部分列の極限は $\mathbf{a}\mathbf{b}^\top$ に等しい．
  したがって
  $\widetilde{\mathbf{P}}_\eta \to \mathbf{a}\mathbf{b}^\top$（$\eta \to 0$）である．
  $\widetilde{\mathbf{P}}_\eta = \mathbf{P}_{1/\eta}$ だから，
