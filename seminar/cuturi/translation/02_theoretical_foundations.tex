...
\begin{definition}{非負実数の空間}{nonneg-reals}
  $\R_{\geq 0} \defeq \{x \in \R \mid x \geq 0\}$ と記す．
  $\R_{\geq 0}^n$ は $n$ 次元ベクトルで全成分が非負であるものの集合を表す．
\end{definition}
...
\begin{definition}{確率単体}{simplex}
  ...
  \[
    \simplex_n \defeq
    \left\{ \mathbf{a} \in \R_{\geq 0}^n \;\middle|\; \sum_{i=1}^n a_i = 1 \right\}
  \]
...
\begin{example}{低次元の確率単体}{simplex-low-dim}
  \begin{itemize}
    \item $n = 2$ のとき，$\simplex_2 = \{(a_1, a_2) \in \R_{\geq 0}^2 \mid a_1 + a_2 = 1\}$
...
\begin{definition}{コスト関数}{cost-function}
  ...
  可測関数
  \[
    c \colon \X \times \Y \to \R_{\geq 0} \cup \{+\infty\}
  \]
...
\begin{setting}{Monge 問題}{monge}
  ...
  およびコスト関数 $c \colon \X \times \Y \to \R_{\geq 0}$ が与えられたとき，
...
Monge の決定論的な輸送写像 $T$ を\textbf{輸送計画}に置き換えることである．
すなわち，各点 $x$ の質量を\textbf{1つの行き先に集中させる}代わりに，
\textbf{複数の行き先に分割して}送ることを許す．
この輸送計画は，$\X \times \Y$ 上の結合測度（カップリング）として定式化される．
...
\begin{setting}{Kantorovich 問題}{kantorovich}
  ...
  およびコスト関数 $c \colon \X \times \Y \to \R_{\geq 0}$ に対して，
  ...
  コスト行列 $\mathbf{C} \in \R_{\geq 0}^{n \times m}$ に対する離散版は
  \[
...
\begin{theorem}{Kantorovich 双対定理（連続版）}{kantorovich-duality-continuous}
  ...
  下半連続なコスト関数 $c \colon \X \times \Y \to \R_{\geq 0} \cup \{+\infty\}$ に対して，
  \[
    \MK_c(\alpha, \beta)
...
\begin{remark}{Monge--Amp\`{e}re 方程式}{monge-ampere}
  ...
  \]
  を満たす．ここで $\partial^2 \varphi(x) \in \R^{d \times d}$ は
  $\varphi$ の二階偏導関数を成分とする行列
  $(\partial^2 \varphi)_{k,l} = \frac{\partial^2 \varphi}{\partial x_k \partial x_l}$
  である．
