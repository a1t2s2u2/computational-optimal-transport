...
\begin{definition}{離散エントロピー}{discrete-entropy}
  行列 $\mathbf{P} \in \R_{\geq 0}^{n \times m}$ の\textbf{離散エントロピー}（discrete entropy）を
  \[
...
\begin{setting}{正則化された最適輸送}{regularized-ot}
  ...
  コスト行列 $\mathbf{C} \in \R_{\geq 0}^{n \times m}$，
...
\begin{definition}{Kullback--Leibler ダイバージェンス}{kl-divergence}
  非負ベクトル $\mathbf{p}, \mathbf{q} \in \R_{\geq 0}^{d}$ に対して，
  ...
  行列 $\mathbf{P}, \mathbf{K} \in \R_{\geq 0}^{n \times m}$ に対しては，
...
\begin{definition}{Hilbert 射影距離}{hilbert-metric}
  ...
  これは射影空間 $\R_{>0}^n / {\sim}$ 上の距離となる．
\end{definition}

\begin{theorem}{Birkhoff の縮小定理}{birkhoff-contraction}
  正行列 $\mathbf{K} \in \R_{>0}^{n \times m}$ に対して，
  \[
