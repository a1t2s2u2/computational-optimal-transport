...
\begin{lemma}{Coupling の貼り合わせ}{gs-gluing}
...
\begin{proof}
  ...
  \textbf{(4) 三角不等式．}
  $\mu_1,\mu_2,\mu_3\in\Pp{p}(X)$ とし，
  $\pi_{12},\pi_{23}$ をそれぞれの最適 coupling とする．
  補題~\ref{lem:gs-gluing}より，
  $X^3$ 上の確率測度 $\boldsymbol{\pi}$ で
  $(x_1,x_2)$ 周辺が $\pi_{12}$，
  $(x_2,x_3)$ 周辺が $\pi_{23}$ であるものが存在する．
  $\boldsymbol{\pi}$ の $(x_1,x_3)$ 周辺分布を
  $\pi_{13}$ とおくと $\pi_{13}\in\Couplings(\mu_1,\mu_3)$ だから，
  \[
    \Wass_p(\mu_1,\mu_3)
    \leq \norm{d(x_1,x_3)}_{L^p(\pi_{13})}
    = \norm{d(x_1,x_3)}_{L^p(\boldsymbol{\pi})}.
  \]
  距離 $d$ の三角不等式 $d(x_1,x_3)\leq d(x_1,x_2)+d(x_2,x_3)$ と
  $L^p$ ノルムの単調性から
  \[
    \norm{d(x_1,x_3)}_{L^p(\boldsymbol{\pi})}
  ...
  \]
  Minkowski の不等式を適用し，
  周辺分布が $\pi_{12},\pi_{23}$ であることから
  \[
    \norm{d(x_1,x_2)+d(x_2,x_3)}_{L^p(\boldsymbol{\pi})}
    \leq \norm{d(x_1,x_2)}_{L^p(\pi_{12})}
    + \norm{d(x_2,x_3)}_{L^p(\pi_{23})}
