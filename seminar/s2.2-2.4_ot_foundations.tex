...
1対1対応（全単射）を見つけ，総コストを最小化する問題である．
...
\begin{example}{置換行列と割当コスト}{sem-assignment-cost-example}
  ...
  \[
    ...
    \end{pmatrix}.
  \]
  置換行列の役割は，ベクトルの成分を $\sigma$ に従って並び替えることである：
  \[
    \mathbf{P}_\sigma
  ...
  \medskip
  また，置換行列を使うと割当のコストをフロベニウス内積で表せる．
  $\mathbf{P}_\sigma$ の成分は $\sigma(i) = j$ の位置だけが $1$ なので，
  ...
  \[
    ...
    \end{pmatrix}
  \]
  のとき，$\mathbf{P}_\sigma$ の $1$ がある位置（太字）のコストだけが拾われ，
  \[
    \inner{\mathbf{C}}{\mathbf{P}_\sigma}
  ...
  \]
  すなわち，最適割当問題は
  $\min_{\sigma \in \Perm(n)} \frac{1}{n}\inner{\mathbf{C}}{\mathbf{P}_\sigma}$
  とも書ける．
\end{example}
