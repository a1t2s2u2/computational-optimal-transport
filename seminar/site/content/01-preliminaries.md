...
集合 \(A, B\) の間の写像 \(f \colon A \to B\) について：

- \(f\) が**単射**とは，\(f(x) = f(y) \Rightarrow x = y\) が成り立つこと．
- \(f\) が**全射**とは，任意の \(y \in B\) に対してある \(x \in A\) が存在して \(f(x) = y\) が成り立つこと．
- \(f\) が**全単射**（または**同型写像**）とは，単射かつ全射であること．

全単射 \(f \colon A \xrightarrow{\sim} B\) が存在するとき \(A\) と \(B\) は**同型**であるという．
:::
