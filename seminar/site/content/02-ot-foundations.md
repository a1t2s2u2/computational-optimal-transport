...
[ref:命題: 離散測度に対する最適値の一致|離散測度に対する最適値の一致] の証明における \(P_{i,j} \defeq \pi(\{(x_i, y_j)\})\) の関係から
...
[ref:定義: 離散 Kantorovich 問題|離散 Kantorovich 問題] の \(\alpha, \beta, \mathbf{C}\) に対して，
...
（[ref:定理: Weierstrass の最大値の定理|Weierstrass の最大値の定理]）より，
...
Kantorovich 問題は線形計画問題（[ref:定義: 線形計画問題|線形計画問題]）に帰着される．
LP は双対問題を持ち，実行可能かつ有界な場合に主問題と双対問題の最適値が一致する
（強双対性，[ref:定理: LP 強双対性|LP 強双対性]）．
本節ではまず LP 双対の一般論を整備し，離散 Kantorovich 双対をその特殊ケースとして導出する．

双対問題を論じる主な意義は次の2点にある．
第一に，強双対性（[ref:定理: LP 強双対性|LP 強双対性]）より主問題の最適値は
...
弱双対性（[ref:主張: 弱双対性|弱双対性]）より \(d^* \leq p^*\) が成立する（\(\leq\) 方向）．
...
上の系は解を持たないから，Farkas の補題（[ref:主張: Farkas の補題|Farkas の補題]）を
...
である．\(\CouplingsD(\mathbf{a}, \mathbf{b})\) は空でなく有界（[ref:主張: 離散 Kantorovich 問題の解の存在|離散 Kantorovich 問題の解の存在]）であるから，
主問題は[ref:定義: 線形計画問題|線形計画問題] の LP として実行可能かつ有界である．
[ref:定理: LP 強双対性|LP 強双対性] の強双対性より主問題と双対問題の最適値は一致する．
:::
:::
...
\((f^*, g^*) \in \PotentialsD(\mathbf{C})\)（[ref:定義: 離散双対実行可能集合|離散双対実行可能集合]）から，
