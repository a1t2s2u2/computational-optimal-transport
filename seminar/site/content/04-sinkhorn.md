...
最適解の構造を取り出す鍵は，目的関数\(F(\mathbf{P}) = \inner{\mathbf{C}}{\mathbf{P}} - \varepsilon \Hb(\mathbf{P})\)の勾配と，周辺制約が定める**アフィン部分空間**の幾何である．
...
:::details-embedded 証明
\(\mathbf{P}_\varepsilon\) は，アフィン集合\(\mathcal{A} \defeq \{\mathbf{P} : \mathbf{P}\ones_m = \mathbf{a},\, \mathbf{P}^\top \ones_n = \mathbf{b}\}\)上で \(F\) を最小化する点である．しかも \(\mathbf{P}_\varepsilon\) は**内点**，すなわち全成分が正である（[ref:Prop: 正則化解の正値性|正則化解の正値性]）．

**一次の最適性条件．**任意の \(\mathbf{Q} \in \mathcal{T}\) をとる．\(\mathbf{P}_\varepsilon\) の成分はすべて正なので，十分小さい \(|t|\) に対して\(\mathbf{P}_\varepsilon + t\mathbf{Q} \in \mathcal{A}\) かつ全成分が正であり，実行可能である．\(t \mapsto F(\mathbf{P}_\varepsilon + t\mathbf{Q})\) は \(t = 0\) で最小値をとるから，
...
**勾配の計算とスケーリング形式．**\(-\Hb(\mathbf{P}) = \sum_{i,j}\varphi(P_{i,j})\)（\(\varphi(x) = x\log x - x\)，\(\varphi'(x) = \log x\)）であったから（備考[ref:Rem: 通常の Shannon エントロピーとの違い|通常の Shannon エントロピーとの違い]），
