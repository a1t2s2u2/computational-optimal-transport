...
\blockmeta{space=separable; lean=TopologicalSpace.SeparableSpace}
...
\blockmeta{space=metric; lean=MetricSpace}
...
\blockmeta{space=normed; lean=NormedAddCommGroup}
...
\begin{definition}{開球}{sem-open-ball}
  距離空間 $(\Omega, d)$ 上の点 $x \in \Omega$ と半径 $r \in \R_{>0}$ に対して，
...
$B_r(x) \subset U$ をみたす $r \in \R_{>0}$ が存在することをいう．
...
\blockmeta{space=complete_metric; lean=CompleteSpace}
...
\blockmeta{space=polish; lean=PolishSpace}
...
\begin{definition}{単関数}{sem-simple-function}
  可測空間 $(\Omega, \mathcal{F})$ 上の関数 $s \colon \Omega \to \R_{\geq 0}$ が
