...
\begin{remark}{周辺条件の行列表現}{sem-marginal-matrix}
  連続版のカップリング集合（定義~\ref{def:sem-kantorovich}）の条件
  $\pi(A \times \Y) = \alpha(A)$（$\forall A \in \Bb(\X)$）を
  離散測度の単点 $A = \{x_k\}$ に適用すると，
  命題~\ref{prop:sem-discrete-kantorovich-equiv} の証明における $P_{i,j} \defeq \pi(\{(x_i, y_j)\})$ の関係から
  \[
  ...
  \begin{center}
  \begin{tabular}{ccc}
    連続版 & $\longleftrightarrow$ & 離散版（行列表記） \\[4pt]
    $\pi(A \times \Y) = \alpha(A)$ & & $\mathbf{P} \ones_m = \mathbf{a}$ \\[2pt]
