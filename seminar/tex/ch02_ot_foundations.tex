...
\begin{remark}{連続版の周辺条件との対応}{sem-marginal-matrix}
  連続版（定義~\ref{def:sem-kantorovich}）の周辺条件を
  ...
  \begin{tabular}{ccc}
    連続版 & $\longleftrightarrow$ & 離散版 \\
    \hline
    $\pi(A \times \Y) = \alpha(A)$ & & $\displaystyle\sum_{j=1}^m P_{i,j} = a_i$ \\
    $\pi(\X \times B) = \beta(B)$ & & $\displaystyle\sum_{i=1}^n P_{i,j} = b_j$
  \end{tabular}
  \end{center}
\end{remark}
