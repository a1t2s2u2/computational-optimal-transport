...
\begin{proof}
  ...
  凸結合 $t\mathbf{P}+(1-t)\mathbf{Q}$ は
  $\CouplingsD(\mathbf{a}, \mathbf{b})$ の凸性（主張~\ref{clm:sem-coupling-convex}）より
  ふたたび $\CouplingsD(\mathbf{a}, \mathbf{b})$ に属する．
...
\begin{proof}
  $\MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$ の最適解 $\mathbf{P}^*$ を一つとる．
  ...
  $\CouplingsD(\mathbf{a}, \mathbf{b})$ の点，すなわち $\MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$ の最適解であり，(ii) を得る．

  \textbf{(iii).}
  上の不等式の左側 $0 \leq \varepsilon\bigl(\Hb(\mathbf{P}_\varepsilon)-\Hb(\mathbf{P}^*)\bigr)$
  を $\varepsilon > 0$ で割ると
  $\Hb(\mathbf{P}_\varepsilon) \geq \Hb(\mathbf{P}^*)$ が，$\MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$ の\emph{任意の}最適解 $\mathbf{P}^*$ について成り立つ．
  ...
  が $\MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$ の任意の最適解 $\mathbf{P}^*$ で成り立ち，
  かつ (ii) より $\mathbf{P}^0$ も $\MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$ の最適解だから，
  $\mathbf{P}^0$ は $\MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$ の最適解の中で $\Hb$ を最大化する．
  $\MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})$ の最適解全体は凸集合であり（主張~\ref{clm:sem-optimal-face}），
  $\Hb$ の最大化は $-\Hb$ の最小化に等しく，$-\Hb$ はその上で狭義凸
  （主張~\ref{clm:sem-neg-entropy-strict-convex}）だから，
  命題~\ref{prop:sem-strict-convex-unique-min} より
  最適解の中で $\Hb$ を最大化する元は一意である．これを $\widehat{\mathbf{P}}$ とおく．
