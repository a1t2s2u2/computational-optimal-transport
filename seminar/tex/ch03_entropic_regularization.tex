...
\begin{proof}
  ...
  一方 $\mathbf{P}_\varepsilon$ も非正則化問題の実行可能点であり，
  $\mathbf{P}^*$ がその最適解だから
  ...
  ゆえに $\mathbf{P}^0$ は最適値を達成する実行可能点，すなわち非正則化問題の最適解であり，(ii) を得る．

  \textbf{(iii).}
  上の不等式の左側 $0 \leq \varepsilon\bigl(\Hb(\mathbf{P}_\varepsilon)-\Hb(\mathbf{P}^*)\bigr)$
  を $\varepsilon > 0$ で割ると
  $\Hb(\mathbf{P}_\varepsilon) \geq \Hb(\mathbf{P}^*)$ が，\emph{任意の}非正則化最適解
  $\mathbf{P}^* \in S^*$ について成り立つ．
