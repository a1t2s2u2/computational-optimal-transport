\chapter{動的定式化：Benamou--Brenier}
\label{ch:seminar-08-benamou-brenier}

第~\ref{ch:seminar-07-geodesics}章では，最適輸送写像 $T=\nabla\phi$ に沿って
質量を一定速度で動かす\textbf{変位補間}が，$(\Pp_2(\R^d),\Wass_2)$ の測地線を
与えることを見た．本章では同じ測地線を，写像ではなく\textbf{流れ}として
捉え直す．すなわち各時刻の質量分布 $\rho_t$ と速度場 $v_t$ の対 $(\rho_t,v_t)$ を
未知関数とし，$\Wass_2^2$ を
「端点をつなぐ流れの\textbf{運動エネルギー}の最小値」として特徴づける．
これが Benamou と Brenier による\textbf{動的定式化}である．

この視点には二つの利点がある．第一に，静的な Kantorovich 問題（質量の対応づけ）
では見えにくい\textbf{時間発展}が前面に出る．第二に，最小化すべき作用が
$\Wass_2$ そのものを定義に含まないため，$\Wass_2$ を
\textbf{偏微分方程式の制約つき最適化}として計算・離散化できる．
...
%% ============================================================
\section{連続の方程式}
\label{sec:sem-continuity-equation}
...
\begin{definition}{連続の方程式}{sem-continuity-equation}
...
\begin{remark}{弱形式の意味}{sem-ce-weak-meaning}
  上の弱形式は，発散定理（部分積分）
...
連続側の議論では，第~\ref{ch:seminar-07-geodesics}章の備考~\ref{rem:sem-w2-is-metric}で
触れた測度の\textbf{分解}と\textbf{弱コンパクト性}を繰り返し用いる．
これらは測度論の重い定理であり，本セミナーでは主張のみを引用する．

\begin{claim}{分解と弱コンパクト性}{sem-disintegration}
  $(1)$（分解，disintegration）
...
%% 8.2 動的定式化（非凸形式）
%% ============================================================
\section{動的定式化と運動エネルギー}
\label{sec:sem-bb-dynamic}

端点 $\alpha,\beta$ をつなぐ流れの\textbf{運動エネルギー}を最小化する問題を立てる．

\begin{definition}{Benamou--Brenier 作用（非凸形式）}{sem-bb-action}
  $\alpha,\beta \in \Pp_2(\R^d)$ に対し，連続の方程式を満たし
  端点条件 $\rho_0=\alpha$，$\rho_1=\beta$ をもつ流れ $(\rho_t,v_t)$ 全体を
  $\mathrm{CE}(\alpha,\beta)$ と書く．流れの\textbf{作用}（運動エネルギーの時間積分）を
  \[
    \mathcal{A}(\rho,v)
    \defeq
...
\begin{remark}{なぜ運動エネルギーか}{sem-why-kinetic}
  各粒子が時刻 $t$ に速度 $v_t$ をもつとき，
  $\int_0^1 \norm{v_t}^2\,\d t$ はその軌道の
  \textbf{エネルギー}（長さの 2 乗のスケール）である．
  Cauchy--Schwarz より，端点を固定した軌道のなかでエネルギーを最小にするのは
  \textbf{等速直線運動}であり，その値は移動距離の 2 乗に等しい．
  $\mathcal{A}$ はこれを質量について平均したものなので，
  $\mathcal{B}(\alpha,\beta)$ が $\Wass_2^2(\alpha,\beta)$ に一致することが期待される
  （定理~\ref{thm:sem-benamou-brenier}）．
\end{remark}

問題~\ref{def:sem-bb-action}は，被積分量 $\norm{v_t}^2\rho_t$ が
$(\rho,v)$ について\textbf{凸でない}（積 $\rho v$ を含む）という難点をもつ．
そこで変数を取り替えて凸化する．
...
%% ============================================================
\section{運動量による凸化}
\label{sec:sem-bb-convexification}

速度 $v_t$ の代わりに\textbf{運動量} $m_t \defeq \rho_t v_t$ を未知関数にとる．
連続の方程式は $m$ について線形
...
\begin{definition}{運動エネルギー密度関数}{sem-theta}
...
\begin{proposition}{$\theta$ は凸かつ下半連続}{sem-theta-convex}
  $\theta$ は次の\textbf{支持関数表現}をもつ：
  \[
    \theta(a,b)
...
\begin{proof}
  ...
  \textbf{$a>0$ のとき．} $q$ を固定すると，$K$ の制約 $p\le-\tfrac14\norm{q}^2$ と
  ...
  \textbf{$(a,b)=(0,\mathbf 0)$ のとき．} 任意の $(p,q)\in K$ で $pa+\inner qb=0$，
  ゆえ $\theta^\circ=0=\theta$．
...
\begin{definition}{Benamou--Brenier 問題（凸形式）}{sem-bb-convex}
  ...
  これは\textbf{凸最適化問題}である．
  $m_t=\rho_t v_t$ の対応により
  $\widetilde{\mathcal{B}}(\alpha,\beta)=\mathcal{B}(\alpha,\beta)$ である．
\end{definition}

%% ============================================================
%% 8.4 Benamou--Brenier の定理
%% ============================================================
\section{Benamou--Brenier の定理}
\label{sec:sem-bb-theorem}

動的問題の最小値が静的な $\Wass_2^2$ に一致し，
その最小化解が前章の測地線（変位補間）であることを述べる．

\begin{theorem}{Benamou--Brenier}{sem-benamou-brenier}
  $\alpha,\beta\in\Pp_2(\R^d)$ に対し，
  \[
    \Wass_2^2(\alpha,\beta)
    = \mathcal{B}(\alpha,\beta)
  ...
  変位補間（定義~\ref{def:sem-mccann-interpolation}）
  $\mu_t=\bigl((1-t)\Id+t\nabla\phi\bigr)_\sharp\alpha$ と，
  それを運ぶ速度場により達成される．
\end{theorem}

最小値の達成（下半連続性と存在）は主張~\ref{clm:sem-disintegration}の
弱コンパクト性に依拠する解析的事実であり，証明は文献に譲る．
ここでは等式の二つの向きを与え，とくに測地線が最小化解であることを
形式計算で確認する．

\begin{claim}{下限の達成}{sem-bb-existence}
  作用 $\mathcal{A}$ は弱収束に関して下半連続であり，
  $\mathrm{CE}(\alpha,\beta)$ 上で最小化解が存在する．
\end{claim}

\begin{proof}[定理~\ref{thm:sem-benamou-brenier}の説明（形式的）]
  \textbf{$\Wass_2^2\ge\mathcal{B}$（測地線が作る流れ）．}
  $\alpha\ll\Lcal^d$ とし，$T=\nabla\phi$ を最適輸送写像
  （主張~\ref{clm:sem-brenier}）とする．各点 $x$ の質量を直線
  ...
  （第~\ref{ch:seminar-07-geodesics}章の式 $(\ast)$）に等しい．
  よって $\mathcal{B}(\alpha,\beta)\le\Wass_2^2(\alpha,\beta)$．

  \textbf{$\Wass_2^2\le\mathcal{B}$（流れが作るカップリング）．}
...
\begin{remark}{静的・動的・測地線の三位一体}{sem-bb-trinity}
  本章で，同じ対象が三つの姿で結ばれた：
...
%% ============================================================
%% 8.5 応用：Flow Matching と Rectified Flow（おまけ）
%% ============================================================
\section{応用：Flow Matching と Rectified Flow}
\label{sec:sem-bb-flow-matching}

本節は本筋（幾何）から離れ，動的定式化が近年の\textbf{生成モデル}と
どう結びつくかを述べる補足である．証明は与えず，対応関係のみを示す．

生成モデルの一つの目標は，扱いやすい基準分布 $\alpha$（標準正規分布など）から
データ分布 $\beta$ への変換を学習することである．
\textbf{連続正規化フロー}は，速度場 $v_t$ を時間に依存するニューラルネットで
表し，常微分方程式 $\dot x(t)=v_t(x(t))$ で粒子を $\alpha$ から $\beta$ へ運ぶ．
このとき粒子の分布 $\rho_t$ はまさに連続の方程式
（定義~\ref{def:sem-continuity-equation}）
$\partial_t\rho_t+\diverg(\rho_t v_t)=0$ に従う．
すなわち\textbf{生成は連続の方程式の解の構成にほかならない}．

\begin{memo*}
  \textbf{Flow Matching との対応．}
  Flow Matching は，端点 $\alpha,\beta$ をつなぐ\textbf{参照流れ} $(\rho_t,v_t)$ を
  あらかじめ固定し，ニューラル速度場 $v_t^\theta$ をその参照速度場へ
  回帰させる（$\int_0^1\E\,\norm{v_t^\theta-v_t}^2\,\d t$ を最小化）枠組みである．
  参照流れとして\textbf{ Benamou--Brenier の最小作用の流れ}（＝変位補間，
  定理~\ref{thm:sem-benamou-brenier}）を選んだものが
  \textbf{OT 流（OT-CFM）}であり，各粒子は直線
  $x_t=(1-t)x_0+t\,x_1$ を等速で進む．
  これは本章の測地線そのものであり，
  \textbf{Rectified Flow}（軌道を直線へ「整流」する手法）の幾何的な原型でもある．
  直線軌道は速度場が時間にほとんど依存せず，
  サンプリング時の数値積分が少ステップで済むという実用上の利点をもつ．
  ただし $\alpha,\beta$ から最適な対応 $\pi$（どの $x_0$ をどの $x_1$ に結ぶか）を
  得るには静的 OT を解く必要があり，ここで第~\ref{ch:seminar-04-sinkhorn}章の
