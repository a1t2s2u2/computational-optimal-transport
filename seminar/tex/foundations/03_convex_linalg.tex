...
\begin{theorem}{等式制約付き最小化の 1 階必要条件（Lagrange の未定乗数法）}{sem-lagrange-foc}
  $U \subseteq \R^N$ を開集合，$f \colon U \to \R$ を微分可能な関数とし，
  $\mathbf{A} \in \R^{r \times N}$, $\mathbf{c} \in \R^r$ による等式制約の集合
  \[
    S \defeq \{\, \mathbf{x} \in U \mid \mathbf{A}\mathbf{x} = \mathbf{c} \,\}
  \]
  を考える（各制約 $\mathbf{a}_k^\top \mathbf{x} = c_k$ は $\mathbf{x}$ について 1 次式．
  $\mathbf{a}_k^\top$ は $\mathbf{A}$ の第 $k$ 行）．
  $f$ が点 $\mathbf{x}^* \in S$ で $S$ 上の局所最小値をとるならば，
  ある乗数 $\mu \in \R^r$ が存在して
  \[
    \nabla f(\mathbf{x}^*) = \mathbf{A}^\top \mu = \sum_{k=1}^{r} \mu_k\, \mathbf{a}_k
  \]
  が成り立つ．乗数 $\mu$ は，行 $\mathbf{a}_1, \dots, \mathbf{a}_r$ が
  1 次独立でなくても存在する（ただしその場合は一意とは限らない）．
\end{theorem}

\begin{proof}
  制約を保つ方向，すなわち $\mathbf{A}\mathbf{d} = \mathbf{0}$ をみたす任意の $\mathbf{d} \in \R^N$ をとる．
  $U$ は開集合で $\mathbf{x}^* \in U$ だから，十分小さい $|t|$ では $\mathbf{x}^* + t\mathbf{d} \in U$ であり，
  $\mathbf{A}(\mathbf{x}^* + t\mathbf{d}) = \mathbf{A}\mathbf{x}^* + t\,\mathbf{A}\mathbf{d} = \mathbf{c}$ ゆえ
  $\mathbf{x}^* + t\mathbf{d} \in S$ にとどまる．
  1 変数関数 $g(t) \defeq f(\mathbf{x}^* + t\mathbf{d})$ は $t = 0$ で局所最小値をとり，
  $f$ の微分可能性と連鎖律より $t = 0$ で微分可能で
  $g'(0) = \nabla f(\mathbf{x}^*)^\top \mathbf{d}$ だから，$g'(0) = 0$，すなわち
  $\nabla f(\mathbf{x}^*)^\top \mathbf{d} = 0$ を得る．
  $\mathbf{d}$ は $\mathbf{A}\mathbf{d} = \mathbf{0}$ をみたす任意のベクトルだったから，
  $\nabla f(\mathbf{x}^*)$ は部分空間 $\{\mathbf{d} \in \R^N : \mathbf{A}\mathbf{d} = \mathbf{0}\}$ の
  すべての元と直交する．その直交補空間は $\mathbf{A}$ の行が張る空間（$\mathbf{A}^\top$ の像）に
  等しい（線形代数の基本事実；行の 1 次独立性は不要）から，
  ある $\mu \in \R^r$ により $\nabla f(\mathbf{x}^*) = \mathbf{A}^\top \mu$ と書ける．
\end{proof}
