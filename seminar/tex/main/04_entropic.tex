...
\begin{proof}
  命題~\ref{prop:sem-entropic-kl-projection} より $\mathbf{P} \defeq \mathbf{P}_\varepsilon$ は
  KL 射影 $\argmin_{\mathbf{P} \in \CouplingsD(\mathbf{a}, \mathbf{b})} \KLD(\mathbf{P}\|\mathbf{K})$ であり，
  基準核 $\mathbf{K} = \exp(-\mathbf{C}/\varepsilon)$ は全成分が正である（定義~\ref{def:sem-gibbs-kernel}）．
  「正の基準核への射影はゼロ成分を許さない」ことを示せばよい．
  ...
  \emph{背理法と線分．}
  ある成分が $P_{i_0, j_0} = 0$ であると仮定する．
  $\CouplingsD(\mathbf{a}, \mathbf{b})$ の凸性（主張~\ref{clm:sem-coupling-convex}）より，$\mathbf{P}$ から $\mathbf{Q}$ への線分
  \[
    \mathbf{P}^t \defeq (1-t)\mathbf{P} + t\mathbf{Q}
    \in \CouplingsD(\mathbf{a}, \mathbf{b})
    \qquad (t \in [0,1]),
    \qquad
    P^t_{i,j} = (1-t)P_{i,j} + t\,Q_{i,j}
  \]
  はすべて実行可能である．$t \in (0,1)$ では $P^t_{i,j} \geq t\,Q_{i,j} > 0$ が全成分で正である．
  $h(t) \defeq \KLD(\mathbf{P}^t\|\mathbf{K})$ とおくと，KL の被加数の偏微分は
  \[
    \frac{\partial}{\partial P_{i,j}}
    \!\left( P_{i,j}\log\frac{P_{i,j}}{K_{i,j}} - P_{i,j} + K_{i,j} \right)
    = \log\frac{P_{i,j}}{K_{i,j}}
  \]
  だから，$t \in (0,1)$ 上で連鎖律により
  \[
    h'(t)
    = \sum_{i,j} (Q_{i,j} - P_{i,j}) \log\frac{P^t_{i,j}}{K_{i,j}}.
  \]

  \emph{微分の発散と矛盾．}
  各項の $t \to 0+$ での挙動を $P_{i,j}$ の値で分ける（$K_{i,j} > 0$ は固定の正定数）．
  \begin{itemize}
    \item $P_{i,j} > 0$ の項：$\log(P^t_{i,j}/K_{i,j}) \to \log(P_{i,j}/K_{i,j})$（有限）．
    \item $P_{i,j} = 0$ の項（$(i_0, j_0)$ を含む）：係数 $Q_{i,j} - P_{i,j} = a_i b_j > 0$，
      かつ $\log(P^t_{i,j}/K_{i,j}) = \log(t\,a_i b_j / K_{i,j}) \to -\infty$ だから，項は $-\infty$ に発散する．
  \end{itemize}
  $+\infty$ に発散する項は一つもなく，$P_{i_0, j_0} = 0$ の項が $-\infty$ に発散するから
  ...
  よって全成分は正である．
  （直観的には，正の基準核 $\mathbf{K}$ への KL 射影は，勾配 $\log(P_{i,j}/K_{i,j})$ が
  $P_{i,j} \to 0$ で $-\infty$ となるため，$0$ 成分を決して許さないのである．）
\end{proof}
