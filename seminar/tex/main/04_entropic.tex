...
\begin{proof}
  ...
  等式制約（$\mathbf{P}$ の成分について 1 次式）つきのこの最小化に，全成分が正の最小元 $\mathbf{P}_\varepsilon$ で Lagrange の 1 階必要条件（定理~\ref{thm:sem-lagrange-foc}）を適用する．行和制約に乗数 $\alpha \in \R^n$，列和制約に乗数 $\beta \in \R^m$ を対応させると，1 階条件は
  \[
    \frac{\partial}{\partial P_{i,j}} \KLD(\mathbf{P}_\varepsilon \| \mathbf{K}) = \alpha_i + \beta_j \qquad (\forall i, j)
  \]
  である（成分 $P_{i,j}$ は第 $i$ 行和・第 $j$ 列和の二つの制約だけに現れ，その乗数 $\alpha_i, \beta_j$ を受け取る）．定義~\ref{def:sem-kl-divergence} より左辺は $\log(P_{i,j}/K_{i,j})$ だから
  \[
