...
これにより，最適解は等式制約のみの最小化問題の解とみなせ，その具体的な形が次のように定まる．
...
\begin{proof}
  補題~\ref{lem:sem-entropic-positivity} より $\mathbf{P}_\varepsilon$ は全成分が正だから，非負制約 $\mathbf{P} \geq 0$ はどの成分でも等号で効かず，制約は周辺条件の等式 $\mathbf{P}\ones_m = \mathbf{a}$, $\mathbf{P}^\top\ones_n = \mathbf{b}$（$\mathbf{P}$ の成分について 1 次式）のみとみなせる．
  二つの周辺制約に対する乗数（双対変数）$\mathbf{f} \in \R^n$, $\mathbf{g} \in \R^m$ を導入し，目的関数の Lagrangian を
  \[
  ...
  \]
  とおく．全成分が正の最小元 $\mathbf{P}_\varepsilon$ では Lagrange の 1 階条件（定理~\ref{thm:sem-lagrange-foc}）が成り立つ．
  $\inner{\mathbf{f}}{\mathbf{P}\ones_m} = \sum_{i,j} f_i P_{i,j}$, $\inner{\mathbf{g}}{\mathbf{P}^\top\ones_n} = \sum_{i,j} g_j P_{i,j}$ と
  $\partial(-\Hb)/\partial P_{i,j} = \log P_{i,j}$（注意~\ref{rem:sem-entropy-convention}）より，停留条件は
  \[
    \frac{\partial \mathcal{L}}{\partial P_{i,j}}(\mathbf{P}_\varepsilon)
    = C_{i,j} + \varepsilon\log (\mathbf{P}_\varepsilon)_{i,j} - f_i - g_j = 0
    \qquad (\forall i, j)
  \]
  となる．これを $(\mathbf{P}_\varepsilon)_{i,j}$ について解くと，$K_{i,j} = e^{-C_{i,j}/\varepsilon}$ を用いて
  \[
    (\mathbf{P}_\varepsilon)_{i,j}
    = e^{f_i/\varepsilon}\, e^{-C_{i,j}/\varepsilon}\, e^{g_j/\varepsilon}
    = e^{f_i/\varepsilon}\, K_{i,j}\, e^{g_j/\varepsilon}
  \]
  となり，$u_i \defeq e^{f_i/\varepsilon} > 0$, $v_j \defeq e^{g_j/\varepsilon} > 0$ とおけば
  \[
    (\mathbf{P}_\varepsilon)_{i,j} = u_i\, K_{i,j}\, v_j,
    \qquad\text{すなわち}\qquad
    \mathbf{P}_\varepsilon = \diag(\mathbf{u})\,\mathbf{K}\,\diag(\mathbf{v}) .
...
\begin{remark}{正則化解の指数表示と定式化非依存性}{sem-entropic-exp-form}
  停留条件 $C_{i,j} + \varepsilon\log(\mathbf{P}_\varepsilon)_{i,j} = f_i + g_j$ は，最適解を
  \[
    (\mathbf{P}_\varepsilon)_{i,j}
    = \exp\!\left(\frac{f_i + g_j - C_{i,j}}{\varepsilon}\right)
  \]
  と表す．すなわち「コストとポテンシャルの差 $f_i + g_j - C_{i,j}$ を $\varepsilon$ で割って指数化する」形である．
  同じスケーリング形は KL 射影（命題~\ref{prop:sem-entropic-kl-projection}）からも得られる：
  $\KLD(\cdot \| \mathbf{K})$ の最小化に 1 階条件を当てると，
  $\partial\KLD/\partial P_{i,j} = \log(P_{i,j}/K_{i,j})$（定義~\ref{def:sem-kl-divergence}）より
  $\log\bigl((\mathbf{P}_\varepsilon)_{i,j}/K_{i,j}\bigr) = \alpha_i + \beta_j$（$\alpha_i = f_i/\varepsilon$, $\beta_j = g_j/\varepsilon$）となり，同じ $\mathbf{P}_\varepsilon$ を与える．
  ゆえにスケーリング形は定式化の選び方に依らない．
