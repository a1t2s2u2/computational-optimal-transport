...
\begin{proof}
  ...
  \textbf{(i).}
  任意の最適解 $\mathbf{P}^* \in S^*$ をとる．$\mathbf{P}_\varepsilon$ の最適性より
  \[
    \inner{\mathbf{C}}{\mathbf{P}_\varepsilon} - \varepsilon\Hb(\mathbf{P}_\varepsilon)
    \leq
    \inner{\mathbf{C}}{\mathbf{P}^*} - \varepsilon\Hb(\mathbf{P}^*),
  \]
  また $\inner{\mathbf{C}}{\mathbf{P}^*} = \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b})
  \leq \inner{\mathbf{C}}{\mathbf{P}_\varepsilon}$
  （$\mathbf{P}_\varepsilon \in \CouplingsD(\mathbf{a},\mathbf{b})$）だから，両者を合わせて
  \[
    0
    \leq
  ...
  \]
  となり (i) を得る．
  なお上の不等式の右側と $\varepsilon > 0$ から，後で用いる
  \[
  ...
  \]
  が成り立つ．$\inner{\mathbf{C}}{\cdot}$ は有界ゆえ
  $\varepsilon_{k_j}^{-1}\inner{\mathbf{C}}{\cdot} \to 0$，$\Hb$ は連続だから，
  $j \to \infty$ の極限で
  $\Hb(\mathbf{P}^\infty) \geq \Hb(\mathbf{Q})$ が
  ...
  よって主張~\ref{clm:sem-subseq-limit-unique} から $\mathbf{P}_{\varepsilon_k} \to \mathbf{a}\mathbf{b}^\top$ であり，
  列 $(\varepsilon_k)$ は任意だったから $\mathbf{P}_\varepsilon \to \mathbf{a}\mathbf{b}^\top$（$\varepsilon \to +\infty$）を得る．
\end{proof}
