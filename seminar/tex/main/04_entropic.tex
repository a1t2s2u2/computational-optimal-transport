...
\begin{proof}
  ...
  行列変数を明示的にベクトル変数へ直す．
  以下，$q(i,j)\defeq (i-1)m+j$ とおき，行列変数 $\mathbf{P}$ を行ごとに並べたベクトルを
  \[
    \bigl(\operatorname{vec}(\mathbf{P})\bigr)_{q(i,j)} \defeq P_{i,j}
  ...
  \[
    ...
    \qquad
    U_x \defeq
    \{\mathbf{x}\in\R^{nm}\mid x_{q(i,j)}>0\ (\forall i,j)\}
  \]
  とおく．正値性より $\mathbf{x}_\varepsilon\in U_x$ であり，その十分近くでは非負制約は自動的に満たされる．
  ...
  \[
    ...
    \sum_{i,j}
    \left(
      x_{q(i,j)}\log\frac{x_{q(i,j)}}{K_{i,j}}
      - x_{q(i,j)}
      + K_{i,j}
    \right)
    \qquad (\mathbf{x}\in U_x)
  \]
  と書ける．すなわち $g(\operatorname{vec}(\mathbf{P}))=\KLD(\mathbf{P}\|\mathbf{K})$ であり，
  $g$ は $U_x$ 上で微分可能である．
  周辺条件は $\mathbf{x}$ では
  \[
    \sum_{l=1}^{m} x_{q(i,l)} = a_i
    \qquad (i=1,\dots,n),
    \qquad
    \sum_{k=1}^{n} x_{q(k,j)} = b_j
    \qquad (j=1,\dots,m)
  \]
  と書ける．したがって $\mathbf{x}_\varepsilon$ は，$U_x$ の中でこれら $n+m$ 個の 1 次等式制約の下で
  $g$ の局所最小点である．

  定理~\ref{thm:sem-lagrange-foc} を，$N=nm$，$f=g$，$\mathbf{x}^*=\mathbf{x}_\varepsilon$，
  上の $n+m$ 個の制約に対して 1 回適用する．
  定理の $\mathbf{a}_k$ は上の各制約左辺の係数ベクトルであり，ヒストグラム $\mathbf{a}$ ではない．
  定理の右辺 $c_k$ は，行制約では $a_i$，列制約では $b_j$ である．
  制約ベクトルの 1 次独立性は仮定されていないので，行和制約と列和制約の間の依存関係は問題にならない．
  上の $i$ 行目の制約と $j$ 列目の制約の係数ベクトルを，それぞれ
  $\mathbf{e}^{\mathrm{row}}_i$，$\mathbf{e}^{\mathrm{col}}_j$ と書くと，
  ある実数 $\alpha_1,\dots,\alpha_n$ と $\beta_1,\dots,\beta_m$ が存在して
  \[
  ...
  \]
  が成り立つ．この勾配等式の $q(i,j)$ 番目の成分を見ると，その座標に係数 $1$ をもつ制約は
  $i$ 行目と $j$ 列目だけなので，
  \[
    \frac{\partial g}{\partial x_{q(i,j)}}(\mathbf{x}_\varepsilon)
    =
    \alpha_i+\beta_j
    \qquad (\forall i,j).
  \]
  一方，$g$ の定義から
  \[
    \frac{\partial g}{\partial x_{q(i,j)}}(\mathbf{x})
    =
    \log\frac{x_{q(i,j)}}{K_{i,j}}
    \qquad (\mathbf{x}\in U_x)
  \]
  である．これを $\mathbf{x}=\mathbf{x}_\varepsilon$ に適用し，
  $(\mathbf{x}_\varepsilon)_{q(i,j)}=(\mathbf{P}_\varepsilon)_{i,j}$ を用いると
  \[
