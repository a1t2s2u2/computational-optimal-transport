...
\begin{proof}
  ...
  $\KLD$ の成分 $(i,j)$ ごとの被加数を，1 変数 $p$ の関数
  \[
    f_{i,j}(p) \defeq p\log\frac{p}{K_{i,j}} - p + K_{i,j},
    \qquad
    f_{i,j}'(p) = \log\frac{p}{K_{i,j}}
  \]
  と定める（$K_{i,j} > 0$ は定数）．これにより $\KLD$ と $h$ は被加数の和
  \[
    \KLD(\mathbf{P}\|\mathbf{K}) = \sum_{k,l} f_{k,l}(P_{k,l}),
    \qquad
    h(t) \defeq \KLD(\mathbf{P}^t\|\mathbf{K}) = \sum_{k,l} f_{k,l}(P^t_{k,l})
  \]
  と書ける．また線分 $P^t_{k,l} = (1-t)(\mathbf{P}_\varepsilon)_{k,l} + t\,Q_{k,l}$ より $\dfrac{d}{dt} P^t_{k,l} = Q_{k,l} - (\mathbf{P}_\varepsilon)_{k,l}$．
  よって $t \in (0,1)$（このとき $P^t_{i,j} > 0$）では，和の微分と連鎖律により
  \[
    h'(t)
    = \sum_{i,j} f_{i,j}'(P^t_{i,j}) \cdot \frac{d P^t_{i,j}}{dt}
