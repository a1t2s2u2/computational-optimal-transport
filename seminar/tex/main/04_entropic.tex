...
\begin{proof}
  ...
  目的関数を線分上に制限した $1$ 変数関数 $g(t) \defeq F(\mathbf{P}^t)$ を考える（線分に沿った $F$ の増減を $1$ 変数 $t$ の微分で追うためである）．$F$ の各項を成分 $P_{i,j}$ で偏微分すると
  \[
  ...
  \emph{微分の発散と矛盾．}
  $g'(t)$ の和を $(\mathbf{P}_\varepsilon)_{i,j}$ の正負で二つに分け，$t \to 0+$ での挙動をみる（$C_{i,j}$ は定数）．
  \begin{itemize}
  ...
  とくに十分小さい $t > 0$ では $g'(t) < 0$ だから，$g$ は $0$ の右側で減少し，そのような $t$ では
  （厳密には，$\lim_{t\to0+} g'(t) = -\infty$ より，ある $\delta \in (0,1)$ が存在して $0 < t < \delta$ で $g'(t) < 0$ となり，
  連続性と合わせて $g$ は $[0,\delta]$ 上で狭義減少する．以下この $t$ は $0 < t < \delta$ にとる）
  \[
    g(t) < g(0) = F(\mathbf{P}_\varepsilon) = \MKD_{\mathbf{C}}^\varepsilon(\mathbf{a}, \mathbf{b})
  \]
  となる．しかし $\mathbf{P}^t \in \CouplingsD(\mathbf{a}, \mathbf{b})$ は実行可能で $g(t) = F(\mathbf{P}^t)$ だから，
