...
\begin{proof}
  $\mathbf{P}_\varepsilon$ は正則化目的関数
  \[
    F(\mathbf{P})
    \defeq \inner{\mathbf{C}}{\mathbf{P}} - \varepsilon\Hb(\mathbf{P})
    = \sum_{i,j}\Bigl( C_{i,j} P_{i,j} + \varepsilon\bigl(P_{i,j}\log P_{i,j} - P_{i,j}\bigr) \Bigr)
  \]
  を $\CouplingsD(\mathbf{a}, \mathbf{b})$ 上で最小にする点である（定義~\ref{def:sem-entropic-ot}）．
  ...
  $F$ の各項を成分 $P_{i,j}$ で偏微分すると
  \[
    \frac{\partial}{\partial P_{i,j}}\!\left( C_{i,j} P_{i,j} + \varepsilon\bigl(P_{i,j}\log P_{i,j} - P_{i,j}\bigr) \right)
  ...
  $t \in (0,1)$ では全成分が $P^t_{i,j} > 0$ に留まり，各 $P^t_{i,j}$ は $t$ の $1$ 次式，$F$ の各項 $C_{i,j}\,p + \varepsilon(p\log p - p)$ は $p > 0$ で微分可能（$p = 0$ では $\log$ が発散して微分不可）だから，線分上の値 $F(\mathbf{P}^t)$ は $t$ について微分可能である．連鎖律により
  \[
  ...
  \emph{微分の発散と矛盾．}
  この微分の和を $(\mathbf{P}_\varepsilon)_{i,j}$ の正負で二つに分け，$t \to 0+$ での挙動をみる（$C_{i,j}$ は定数）．
  \begin{itemize}
    \item $(\mathbf{P}_\varepsilon)_{i,j} > 0$ の項：$P^t_{i,j} \to (\mathbf{P}_\varepsilon)_{i,j} > 0$ なので
      $C_{i,j} + \varepsilon\log P^t_{i,j}$ は有限値に収束し，係数も有限．ゆえにこれらの項の和は有界である．
    \item $(\mathbf{P}_\varepsilon)_{i,j} = 0$ の項：係数 $Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j} = a_i b_j > 0$，
      かつ $C_{i,j} + \varepsilon\log P^t_{i,j} = C_{i,j} + \varepsilon\log(t\,a_i b_j) \to -\infty$ だから，各項は $-\infty$ に発散する．
      仮定よりこのような項は少なくとも一つある．
  \end{itemize}
  前者の和は有界，後者は $-\infty$ に発散する（$+\infty$ に飛ぶ項はないので相殺は起きない）から，
  $t \to 0+$ でこの微分は $-\infty$ に発散する．
  とくに十分小さい $t > 0$ ではこの微分が負だから，$F(\mathbf{P}^t)$ は $0$ の右側で（$t$ について）減少し，そのような $t$ では
  （厳密には，$t\to0+$ でこの微分が $-\infty$ に発散することから，ある $\delta \in (0,1)$ が存在して $0 < t < \delta$ で微分が負となり，
  連続性と合わせて $F(\mathbf{P}^t)$ は $[0,\delta]$ 上で $t$ について狭義減少する．以下この $t$ は $0 < t < \delta$ にとる）
  \[
    F(\mathbf{P}^t) < F(\mathbf{P}^0) = F(\mathbf{P}_\varepsilon) = \MKD_{\mathbf{C}}^\varepsilon(\mathbf{a}, \mathbf{b})
  \]
  となる．しかし $\mathbf{P}^t \in \CouplingsD(\mathbf{a}, \mathbf{b})$ は実行可能だから，
  これは「実行可能点 $\mathbf{P}^t$ が最小値より小さい目的関数値をとる」ことになり，
  $\mathbf{P}_\varepsilon$ が $F$ を最小にすることに反する．
  ゆえにゼロ成分は存在せず，$\mathbf{P}_\varepsilon$ の全成分は正である．
  （直観的には，ある成分を $0$ からわずかに正へ開くと，勾配 $C_{i,j} + \varepsilon\log P_{i,j}$ が
  $P_{i,j} \to 0$ で $-\infty$ に落ちる（負エントロピーの勾配 $\varepsilon\log P_{i,j}$ が支配的）ため目的関数が一気に下がる．
  ゆえに $0$ 成分は最適たり得ない．）
\end{proof}
