...
\begin{proof}
  ...
  \]
  が空でない（$\mathbf{P}_\varepsilon$ が少なくとも一つのゼロ成分をもつ）と仮定する．
  ...
  \]
  だから，$t \in (0,1)$ 上で連鎖律により（$dP^t_{i,j}/dt = Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j}$）
  \[
    h'(t)
    = \sum_{i,j} \bigl(Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j}\bigr) \log\frac{P^t_{i,j}}{K_{i,j}}.
  \]
  ...
  \begin{itemize}
    ...
    \item $(\mathbf{P}_\varepsilon)_{i,j} = 0$ の項（すなわち $(i,j) \in Z$）：係数 $Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j} = a_i b_j > 0$，
      かつ $\log(P^t_{i,j}/K_{i,j}) = \log(t\,a_i b_j / K_{i,j}) \to -\infty$ だから，項は $-\infty$ に発散する．
  \end{itemize}
  $+\infty$ に発散する項は一つもなく，仮定 $Z \neq \emptyset$ よりゼロ成分の項が少なくとも一つ存在して $-\infty$ に発散するから，
