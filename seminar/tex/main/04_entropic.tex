...
\begin{proof}
  証明は 3 段からなる：最適解が領域の内部（全成分が正）にあることを示し（Step 1），
  そこで等式制約のみの 1 階条件（Lagrange の未定乗数法）を適用してスケーリング形を導き（Step 2），
  最後に一意性を確かめる（Step 3）．
  目的関数を
  \[
    F(\mathbf{P}) = \inner{\mathbf{C}}{\mathbf{P}} - \varepsilon\Hb(\mathbf{P})
    = \inner{\mathbf{C}}{\mathbf{P}} + \varepsilon\sum_{i,j}\varphi(P_{i,j}),
    \qquad \varphi(x) = x\log x - x,\quad \varphi'(x) = \log x\ (x > 0)
  \]
  とおく（$\varphi$ は注意~\ref{rem:sem-phi-continuous}）．

  \textbf{Step 1（最適解の正値性）．}
  鍵は，全成分が正である実行可能点を 1 つ手に持っておくことである．
  独立カップリング $\mathbf{P}^\circ \defeq \mathbf{a}\mathbf{b}^\top$ は
  $\CouplingsD(\mathbf{a}, \mathbf{b})$ に属し，$P^\circ_{i,j} = a_i b_j > 0$ をみたす．

  背理法で $\mathbf{P}_\varepsilon$ が零成分をもつと仮定し，その零集合
  \[
    Z \defeq \{(i, j) : (\mathbf{P}_\varepsilon)_{i,j} = 0\} \neq \emptyset
  \]
  を考える（以下 $\mathbf{P} \defeq \mathbf{P}_\varepsilon$ と略記）．
  $\mathbf{P}$ から $\mathbf{P}^\circ$ へ向かう線分
  \[
    \mathbf{P}_\theta \defeq (1 - \theta)\,\mathbf{P} + \theta\,\mathbf{P}^\circ
    \in \CouplingsD(\mathbf{a}, \mathbf{b})
    \qquad (\theta \in [0, 1])
  \]
  に沿って $g(\theta) \defeq F(\mathbf{P}_\theta)$ の挙動を調べる
  （凸集合 $\CouplingsD(\mathbf{a}, \mathbf{b})$ の 2 点を結ぶ線分なので全体が実行可能）．
  $\theta \in (0, 1]$ では $\mathbf{P}_\theta$ は全成分正だから $g$ は $(0, 1)$ 上で微分可能で，
  $\tfrac{d}{d\theta}(\mathbf{P}_\theta)_{i,j} = P^\circ_{i,j} - P_{i,j}$ より
  \[
    g'(\theta) = \inner{\mathbf{C}}{\mathbf{P}^\circ - \mathbf{P}}
    + \varepsilon\sum_{i,j} \log\!\bigl((\mathbf{P}_\theta)_{i,j}\bigr)\,
      \bigl(P^\circ_{i,j} - P_{i,j}\bigr).
  \]
  $\theta \to 0+$ での各項の挙動は次のとおり：
  \begin{itemize}
    \item $(i, j) \notin Z$：$(\mathbf{P}_\theta)_{i,j} \to P_{i,j} > 0$ なので項は有限値に収束する．
      線形項 $\inner{\mathbf{C}}{\mathbf{P}^\circ - \mathbf{P}}$ も $\theta$ によらない有限値．
    \item $(i, j) \in Z$：$(\mathbf{P}_\theta)_{i,j} = \theta\, a_i b_j$ かつ
      $P^\circ_{i,j} - P_{i,j} = a_i b_j > 0$ なので，項は
      $a_i b_j \log(\theta\, a_i b_j) \to -\infty$．
  \end{itemize}
  $Z \neq \emptyset$ より $\lim_{\theta \to 0+} g'(\theta) = -\infty$．
  一方 $g$ は $[0, 1]$ 上で連続だから（$\varphi$ は $0$ で連続かつ $\varphi(0) = 0$），
  十分小さい $\theta > 0$ で $g$ は狭義減少し，右連続性とあわせて
  $g(\theta) < g(0) = F(\mathbf{P}_\varepsilon)$ となって最適性に反する．
  ゆえに $\mathbf{P}_\varepsilon$ の全成分は正である．

  \textbf{Step 2（1 階条件）．}
  Step 1 より $\mathbf{P}_\varepsilon$ は領域 $\R_{>0}^{n \times m}$ の内点にあるので，
  非負制約 $\mathbf{P} \geq 0$ は活性化せず，制約は等式
  $\mathbf{P}\ones_m = \mathbf{a}$, $\mathbf{P}^\top\ones_n = \mathbf{b}$ のみとみなせる．
  命題~\ref{prop:sem-entropic-kl-projection} より $\mathbf{P}_\varepsilon$ は
  $\CouplingsD(\mathbf{a}, \mathbf{b})$ 上で $\KLD(\cdot \| \mathbf{K})$ を最小化し，
  この関数は $\R_{>0}^{n \times m}$ 上で微分可能で
  \[
    \frac{\partial}{\partial P_{i,j}} \KLD(\mathbf{P} \| \mathbf{K})
    = \log\frac{P_{i,j}}{K_{i,j}}
  \]
  をみたす（定義~\ref{def:sem-kl-divergence} より直接計算）．
  制約はすべてアフィンなので制約想定がみたされ，内点の最小元では Lagrange の 1 階条件が必要条件となる．
  行和・列和制約の乗数を $\boldsymbol{\alpha} \in \R^n$, $\boldsymbol{\beta} \in \R^m$ とすれば
  \[
  ...
  $(\lambda u_i)(\lambda^{-1} v_j) = u_i v_j$ より，
  $(\lambda\mathbf{u}, \lambda^{-1}\mathbf{v})$ も同じ $\mathbf{P}_\varepsilon$ を与える．
  逆にこの定数倍の自由度を除けば一意である．
  実際，全成分が正なので各 $(i, j)$ で $u_i v_j = (\mathbf{P}_\varepsilon)_{i,j} / K_{i,j} > 0$ が定まり，
  任意の $j$ について
  \[
    \frac{u_i}{u_{i'}}
    = \frac{(\mathbf{P}_\varepsilon)_{i,j} / K_{i,j}}
           {(\mathbf{P}_\varepsilon)_{i',j} / K_{i',j}}
  \]
  が（$j$ に依らず）決まる．よって $\mathbf{u}$ は正の定数倍を除いて一意であり，対応して $\mathbf{v}$ も定まる．
\end{proof}
