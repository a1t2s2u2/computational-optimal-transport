...
\begin{proof}
  ...
  まず $\KLD$ の成分 $(i,j)$ ごとの項を，1 変数 $p$ の関数として
  \[
    f_{i,j}(p) \defeq p\log\frac{p}{K_{i,j}} - p + K_{i,j},
    \qquad
    f_{i,j}'(p) = \bigl(\log p - \log K_{i,j} + 1\bigr) - 1 = \log\frac{p}{K_{i,j}}
  \]
  と定める（$K_{i,j} > 0$ は定数）．これを用いると $h(t) \defeq \KLD(\mathbf{P}^t\|\mathbf{K})$ は
  \[
    h(t) = \sum_{i,j} f_{i,j}(P^t_{i,j})
  \]
  と書ける．また線分の定義 $P^t_{i,j} = (1-t)(\mathbf{P}_\varepsilon)_{i,j} + t\,Q_{i,j}$ から，$t$ について
  \[
    \frac{d}{dt} P^t_{i,j} = Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j}.
  \]
  $t \in (0,1)$ では $P^t_{i,j} > 0$（既述）なので各 $f_{i,j}(P^t_{i,j})$ は微分可能であり，
  和の微分と連鎖律により
  \begin{align*}
    h'(t)
    &= \sum_{i,j} \frac{d}{dt}\, f_{i,j}(P^t_{i,j})
    = \sum_{i,j} f_{i,j}'(P^t_{i,j}) \cdot \frac{d}{dt} P^t_{i,j} \\
    &= \sum_{i,j} \log\frac{P^t_{i,j}}{K_{i,j}} \cdot \bigl(Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j}\bigr).
  \end{align*}

  \emph{微分の発散と矛盾．}
  各項の $t \to 0+$ での挙動を $(\mathbf{P}_\varepsilon)_{i,j}$ の値で分ける（$K_{i,j} > 0$ は固定の正定数）．
  \begin{itemize}
    \item $(\mathbf{P}_\varepsilon)_{i,j} > 0$ の項：$\log(P^t_{i,j}/K_{i,j}) \to \log((\mathbf{P}_\varepsilon)_{i,j}/K_{i,j})$（有限）．
  ...
  \end{itemize}
  $+\infty$ に発散する項は一つもなく，仮定よりゼロ成分が少なくとも一つあって，その項が $-\infty$ に発散するから，
