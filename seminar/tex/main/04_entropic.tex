...
\begin{proof}
  ...
  Lagrange の定理（定理~\ref{thm:sem-lagrange-foc}）は $\R^N$ 上のベクトル変数に対する主張なので，
  ここで行列変数を明示的にベクトル変数へ直す．
  ここでの $\mathbf{P}$ は固定された行列ではなく，最適化で動かす任意の行列変数を表す．
  行列変数 $\mathbf{P}\in\R^{n\times m}$ を行ごとに並べたベクトルを
  $\operatorname{vec}(\mathbf{P})\in\R^{nm}$ とし，
  \[
    \bigl(\operatorname{vec}(\mathbf{P})\bigr)_{(i-1)m+j} \defeq P_{i,j}
    \qquad (i=1,\dots,n,\ j=1,\dots,m)
  \]
  と定める．逆向きに，ベクトル $\mathbf{x}\in\R^{nm}$ を同じ成分をもつ $n\times m$ 行列表示へ戻す記号を
  $\operatorname{mat}(\mathbf{x})$ とし，
  \[
    \operatorname{mat}(\mathbf{x})_{i,j} \defeq x_{(i-1)m+j}
  \]
  と定める．これは新しい行列を仮定しているのではなく，$\mathbf{x}$ の座標を行列の形で書き直すための記号である．
  また
  \[
    \mathbf{x}_\varepsilon \defeq \operatorname{vec}(\mathbf{P}_\varepsilon),
    \qquad
    U_x \defeq
    \{\mathbf{x}\in\R^{nm}\mid x_{(i-1)m+j}>0\ (\forall i,j)\}
  \]
  とおく．$\mathbf{P}_\varepsilon$ は全成分が正なので，$\mathbf{x}_\varepsilon\in U_x$ であり，
  $\mathbf{x}_\varepsilon$ の十分近くでは非負制約は自動的に満たされる．
  したがって，残る制約は周辺条件だけである．
  ...
  \[
    ...
    \right)
  \]
  としてよい．定理に入れるベクトル変数の関数を
  \[
    g(\mathbf{x}) \defeq G(\operatorname{mat}(\mathbf{x}))
    \qquad (\mathbf{x}\in U_x)
  \]
  と定める．この $g$ は $U_x$ 上で微分可能である．

  次に，周辺条件を $\mathbf{x}$ の 1 次式として書く．
  行和制約用の係数ベクトル $\mathbf{e}^{\mathrm{row}}_i\in\R^{nm}$ と
  列和制約用の係数ベクトル $\mathbf{e}^{\mathrm{col}}_j\in\R^{nm}$ を
  \[
    \left(\mathbf{e}^{\mathrm{row}}_i\right)_{(p-1)m+\ell}
    \defeq
    \begin{cases}
      1 & (p=i),\\
      0 & (p\neq i),
    \end{cases}
    \qquad
    \left(\mathbf{e}^{\mathrm{col}}_j\right)_{(p-1)m+\ell}
    \defeq
    \begin{cases}
      1 & (\ell=j),\\
      0 & (\ell\neq j)
  ...
  と定める．すると周辺条件は
  \[
    \left(\mathbf{e}^{\mathrm{row}}_i\right)^\top\mathbf{x} = a_i
    \qquad (i=1,\dots,n),
    \qquad
    \left(\mathbf{e}^{\mathrm{col}}_j\right)^\top\mathbf{x} = b_j
    \qquad (j=1,\dots,m)
  \]
  である．つまり，$\mathbf{x}_\varepsilon$ は $U_x$ の中で，これら $n+m$ 個の 1 次等式制約の下で
  $g$ の局所最小点である．

  以上を定理~\ref{thm:sem-lagrange-foc} の記号へそのまま代入すると，対応は次の通りである：
  \[
    \begin{array}{ccl}
      N & = & nm,\\
      U & = & U_x,\\
      f & = & g,\\
      \mathbf{x}^* & = & \mathbf{x}_\varepsilon,\\
      r & = & n+m,\\
      \mathbf{a}_i & = & \mathbf{e}^{\mathrm{row}}_i,\quad c_i = a_i,\quad \mu_i = \alpha_i
        \qquad (i=1,\dots,n),\\
      \mathbf{a}_{n+j} & = & \mathbf{e}^{\mathrm{col}}_j,\quad c_{n+j} = b_j,\quad \mu_{n+j} = \beta_j
        \qquad (j=1,\dots,m).
    \end{array}
  \]
  この表の $\mathbf{a}_k,c_k,\mu_k$ は定理~\ref{thm:sem-lagrange-foc} 側の記号である．
  特に，定理の $\mathbf{a}_k$ は制約式左辺の係数ベクトルであり，ヒストグラム $\mathbf{a}$ ではない．
  また，定理の $c_k$ は制約式右辺の実数であり，ここでは行制約なら $a_i$，列制約なら $b_j$ である．
  ここでは $n+m$ 個の等式制約をまとめて 1 回だけ扱う．
  係数ベクトルの間には 1 本の依存関係があるが，定理~\ref{thm:sem-lagrange-foc} は
  制約ベクトルの 1 次独立性を仮定していないので，このまま適用してよい．

  定理~\ref{thm:sem-lagrange-foc} より，ある実数
  $\alpha_1,\dots,\alpha_n$ と $\beta_1,\dots,\beta_m$ が存在して
  \[
    \nabla g(\mathbf{x}_\varepsilon)
    =
    \sum_{i'=1}^{n}\alpha_{i'}\,\mathbf{e}^{\mathrm{row}}_{i'}
    +
    \sum_{j'=1}^{m}\beta_{j'}\,\mathbf{e}^{\mathrm{col}}_{j'}
  \]
  が成り立つ．定理が直接与えるのはこの $\R^{nm}$ 上の勾配等式である．
  この式の $(i-1)m+j$ 番目の成分を読むと，
  \[
    \frac{\partial g}{\partial x_{(i-1)m+j}}(\mathbf{x}_\varepsilon)
    =
    \alpha_i+\beta_j
    \qquad (\forall i,j).
  \]
  これは，その座標に現れる係数ベクトルが
  $\mathbf{e}^{\mathrm{row}}_i$ と $\mathbf{e}^{\mathrm{col}}_j$ だけだからである．
  一方，$g(\mathbf{x})=G(\operatorname{mat}(\mathbf{x}))$ の定義から
  \[
    \frac{\partial g}{\partial x_{(i-1)m+j}}(\mathbf{x})
    =
    \frac{\partial G}{\partial P_{i,j}}(\operatorname{mat}(\mathbf{x}))
    =
    \log\frac{\operatorname{mat}(\mathbf{x})_{i,j}}{K_{i,j}}
    \qquad (\mathbf{x}\in U_x)
  \]
  である．したがって
  \[
