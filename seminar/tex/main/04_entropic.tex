...
\begin{proof}
  ...
  したがって $\mathbf{P}_\varepsilon$ の十分近くでは非負制約は自動的に満たされるので，残る制約は周辺条件だけである．
  ...
  行和制約と列和制約の係数行列を
  \[
    (\mathbf{E}^{\mathrm{row}}_i)_{p,\ell} \defeq
    \begin{cases}
      1 & (p=i),\\
      0 & (p\neq i),
    \end{cases}
    \qquad
    (\mathbf{E}^{\mathrm{col}}_j)_{p,\ell} \defeq
    \begin{cases}
      1 & (\ell=j),\\
      0 & (\ell\neq j)
    \end{cases}
  ...
  と定める．周辺条件は
  \[
    \inner{\mathbf{E}^{\mathrm{row}}_i}{\mathbf{P}} = a_i,
    \qquad
    \inner{\mathbf{E}^{\mathrm{col}}_j}{\mathbf{P}} = b_j
  \]
  と書ける．よって定理~\ref{thm:sem-lagrange-foc-matrix} をこの $n+m$ 個の制約に 1 回適用すると，
  ...
  \]
  が成り立つ．この $(i,j)$ 成分を読むと
  $\mathbf{E}^{\mathrm{row}}_{i'}$ は $i'=i$ のときだけ，
  $\mathbf{E}^{\mathrm{col}}_{j'}$ は $j'=j$ のときだけ $(i,j)$ 成分が $1$ なので，
  \[
...
\begin{proof}
  ...
  これを各成分で $k=a_i b_j$ として用いると，
  \[
    \KLD(\mathbf{P}\|\mathbf{K}) \geq 0
  \]
  であり，等号は全成分で $P_{i,j}=a_i b_j$，
