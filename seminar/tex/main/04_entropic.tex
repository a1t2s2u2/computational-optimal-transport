...
\begin{proof}
  ...
  \medskip
  そこで，動き出しにおける $F$ の傾きを調べる．$g(t) \defeq F(\mathbf{P}^t)$ とおくと，$t \in (0,1]$ では全成分 $P^t_{i,j} \ge t\,a_i b_j > 0$ ゆえ $g$ は微分可能で，連鎖律より
  \[
    g'(t)
  ...
  \[
    g'(t) \xrightarrow{\ t \to 0+\ } -\infty .
  \]
  すなわち動き出しで $F$ の傾きは $-\infty$ である（負エントロピーの傾き $\log$ が境界 $0$ で $-\infty$ に落ちることが効く）．
  ゆえにある $\delta \in (0,1)$ が存在して $g'(t) < 0$（$0 < t < \delta$）．$g$ は $[0,\delta]$ で連続（$\varphi$ の連続性，注意~\ref{rem:sem-phi-continuous}）・$(0,\delta)$ で微分可能だから，平均値定理（定理~\ref{thm:sem-mvt}）より $g$ は $[0,\delta]$ で狭義減少し，
  \[
    F(\mathbf{P}^t) = g(t) < g(0) = F(\mathbf{P}_\varepsilon) \qquad (0 < t < \delta).
  \]
