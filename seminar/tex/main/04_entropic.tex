...
\begin{proof}
  \textbf{スケーリング形．}
  補題~\ref{lem:sem-entropic-positivity} より $\mathbf{P}_\varepsilon$ は $\Rpos^{n \times m}$ の内点にあり，非負制約 $\mathbf{P} \geq 0$ は不活性で，制約は等式 $\mathbf{P}\ones_m = \mathbf{a}$, $\mathbf{P}^\top\ones_n = \mathbf{b}$ のみとみなせる．
  ...
  \]
  制約はアフィンだから，内点の最小元で Lagrange の 1 階条件が成り立つ．行和・列和制約に対応する乗数 $\alpha \in \R^n$, $\beta \in \R^m$ により
  \[
  ...
  \textbf{一意性．}
  $\mathbf{P}_\varepsilon$ の一意性は命題~\ref{prop:sem-entropic-existence-unique} による．
  $(\mathbf{u}, \mathbf{v})$ については，任意の $\lambda > 0$ に対し $(\lambda u_i)(\lambda^{-1} v_j) = u_i v_j$ ゆえ $(\lambda\mathbf{u}, \lambda^{-1}\mathbf{v})$ も同じ $\mathbf{P}_\varepsilon$ を与える．
  逆に，$w_{i,j} \defeq u_i v_j = (\mathbf{P}_\varepsilon)_{i,j} / K_{i,j} > 0$ は確定し，比 $u_i / u_{i'} = w_{i,j} / w_{i',j}$ は $j$ に依らないから，$\mathbf{u}$ は正の定数倍を除いて一意で，$v_j = w_{i,j} / u_i$ も定まる．よって自由度は $\lambda$ のみである．
\end{proof}
