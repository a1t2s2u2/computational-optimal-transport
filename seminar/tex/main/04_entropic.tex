...
最適解の具体的な形を導く準備として，まず最適解が領域の内部にある（全成分が正である）ことを示す．これにより非負制約が不活性となり，等式制約のみの 1 階条件（Lagrange の未定乗数法）が適用できる．

\begin{lemma}{正則化解の正値性}{sem-entropic-positivity}
  定義~\ref{def:sem-entropic-ot} の仮定（$\mathbf{a} \in \R_{>0}^n$, $\mathbf{b} \in \R_{>0}^m$）の下で，
  $\MKD_{\mathbf{C}}^\varepsilon(\mathbf{a}, \mathbf{b})$ の最適解 $\mathbf{P}_\varepsilon$ は全成分が正である：
  \[
    (\mathbf{P}_\varepsilon)_{i,j} > 0 \qquad (\forall i, j).
  \]
  とくに $\mathbf{P}_\varepsilon$ は非負制約 $\mathbf{P} \geq 0$ を活性化しない内点である．
\end{lemma}
...
\begin{proof}
  ...
  \medskip
  \textbf{Step 1（1 階条件）．}
  補題~\ref{lem:sem-entropic-positivity} より $\mathbf{P}_\varepsilon$ は領域 $\R_{>0}^{n \times m}$ の内点にあるので，
...
\begin{remark}{双対ポテンシャルとの関係}{sem-entropic-dual-potential}
  Step 1 の乗数を $\alpha_i = f_i / \varepsilon$, $\beta_j = g_j / \varepsilon$ と書けば
  ...
  \]
  となり，同じ式が得られる（これが Step 1 を元の問題から導く「経路 A」にあたる）．
