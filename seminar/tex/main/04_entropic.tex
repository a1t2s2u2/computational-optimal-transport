...
\begin{proof}
  ...
  ここでは $N=nm$ とする．厳密には $x \in \R^{nm}$ の第 $(i-1)m+j$ 成分を
  \[
    x_{(i-1)m+j} = P_{i,j} \qquad (1\leq i\leq n,\ 1\leq j\leq m)
  \]
  と書く．以後はこの $x$ を，二重添字をもつ
  $\mathbf{P}=(P_{i,j})_{i,j}$ と表す．
  したがって $\mathbf{P}_\varepsilon$ の十分近くでは非負制約 $\mathbf{P} \geq 0$ は自動的に満たされるので，
  $\mathbf{P}_\varepsilon$ は開集合
  \[
    U \defeq \{\mathbf{P}=(P_{i,j})_{i,j} \in \R^{nm} \mid P_{i,j} > 0\ (\forall i,j)\}
  \]
