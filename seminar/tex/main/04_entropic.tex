...
\begin{proof}
  ...
  $g(t) \defeq F(\mathbf{P}^t)$ とおく．$F$ の各項を成分 $P_{i,j}$ で偏微分すると
  \[
  ...
  発散が $-\infty$ 向きであることは，$g'(t)$ が $0$ の右近傍で負になることを意味する：
  すなわち，その近傍の幅となる $\delta \in (0,1)$ が存在して
  \[
    0 < t < \delta \;\Longrightarrow\; g'(t) < 0 .
  \]
  この $\delta$ に対し，$g$ は $[0, \delta]$ で連続かつ $(0, \delta)$ で $g'(t) < 0$ だから，$[0, \delta]$ 上で狭義減少する．
  したがって $0 < t < \delta$ では
  \[
    g(t) < g(0) = F(\mathbf{P}_\varepsilon) = \MKD_{\mathbf{C}}^\varepsilon(\mathbf{a}, \mathbf{b}).
  \]
  ところが $\mathbf{P}_\varepsilon$ は $\CouplingsD(\mathbf{a}, \mathbf{b})$ 上で $F$ を最小にする点であり，
  一方 $\mathbf{P}^t \in \CouplingsD(\mathbf{a}, \mathbf{b})$ かつ $g(t) = F(\mathbf{P}^t)$ である．
  すなわち上の不等式は「実行可能点 $\mathbf{P}^t$ が最小値より小さい目的関数値をとる」ことを意味し，矛盾する．
