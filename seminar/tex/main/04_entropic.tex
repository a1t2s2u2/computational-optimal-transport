...
\begin{proof}
  ...
  \[
    ...
    \qquad (t \in [0,1])
  \]
  はすべて実行可能である．目標は，小さい $t > 0$ で $F(\mathbf{P}^t) < F(\mathbf{P}_\varepsilon)$ を示すこと（動かすと $F$ が下がること）である．

  \medskip
  これを差 $F(\mathbf{P}^t) - F(\mathbf{P}_\varepsilon)$ の符号で直接見ようとすると，$t \to 0+$ で $\mathbf{P}^t \to \mathbf{P}_\varepsilon$ ゆえ差が $0$ に潰れて符号が読めない．
  そこで「動かした量 $t$ あたりの変化」を見る：$t > 0$ ゆえこれは差と同符号で，かつ $0$ に潰れず $t \to 0+$ で確定した極限をもつ．$\inner{\mathbf{C}}{\cdot}$ の線形性より
  \[
  ...
  \]
  和の各項の $t \to 0+$ での挙動は，その成分で $\varphi$ が滑らかか，境界 $0$ にいるかで分かれる：
  \begin{align*}
    (\mathbf{P}_\varepsilon)_{i,j} > 0 \colon\quad
      \frac{\varphi(P^t_{i,j}) - \varphi\bigl((\mathbf{P}_\varepsilon)_{i,j}\bigr)}{t}
      &\xrightarrow{\ t \to 0+\ }
       \bigl(\log (\mathbf{P}_\varepsilon)_{i,j}\bigr)\bigl(a_i b_j - (\mathbf{P}_\varepsilon)_{i,j}\bigr)
       \quad (\text{有限}), \\
    (\mathbf{P}_\varepsilon)_{i,j} = 0 \colon\quad
      \frac{\varphi(P^t_{i,j}) - \varphi\bigl((\mathbf{P}_\varepsilon)_{i,j}\bigr)}{t}
      &= a_i b_j\bigl(\log(t\,a_i b_j) - 1\bigr) \xrightarrow{\ t \to 0+\ } -\infty .
  \end{align*}
  正の成分では $\varphi$ が微分可能（$\varphi'(x) = \log x$，注意~\ref{rem:sem-entropy-convention}），ゼロ成分では $\varphi(0) = 0$（注意~\ref{rem:sem-phi-continuous}）と $P^t_{i,j} = t\,a_i b_j$ を用いた．
  ゼロ成分の項が発散するのは，負エントロピーの傾き $\log$ が境界 $0$ で $-\infty$ に落ちるからであり，これが証明の要である．
  右辺第 1 項は定数，$(\mathbf{P}_\varepsilon)_{i,j} > 0$ の項の和は有限値に収束し，$(\mathbf{P}_\varepsilon)_{i,j} = 0$ の項の和は（仮定よりこれに少なくとも一項あり）$-\infty$ に発散する．
  $+\infty$ に発散する項はないので，
  \[
    \frac{F(\mathbf{P}^t) - F(\mathbf{P}_\varepsilon)}{t} \xrightarrow{\ t \to 0+\ } -\infty,
  \]
  よって十分小さい $t > 0$ で $F(\mathbf{P}^t) < F(\mathbf{P}_\varepsilon)$．これは実行可能点 $\mathbf{P}^t$ が最小値を下回ることを意味し，$\mathbf{P}_\varepsilon$ の最小性に矛盾する．ゆえに $\mathbf{P}_\varepsilon$ の全成分は正である．
\end{proof}
