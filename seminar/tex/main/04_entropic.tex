...
\begin{proof}
  ...
  \[
    \KLD(\mathbf{P}\|\mathbf{K}) = \sum_{i,j} f_{i,j}(P_{i,j}),
    \qquad
    h(t) \defeq \KLD(\mathbf{P}^t\|\mathbf{K}) = \sum_{i,j} f_{i,j}(P^t_{i,j})
  \]
  と書ける．また線分 $P^t_{i,j} = (1-t)(\mathbf{P}_\varepsilon)_{i,j} + t\,Q_{i,j}$ より $\dfrac{d}{dt} P^t_{i,j} = Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j}$．
