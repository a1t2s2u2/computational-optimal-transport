...
\begin{proof}
  ...
  $\mathbf{P}_\varepsilon$ は $F$ の最小点だから，実行可能点でこれより $F$ が小さいものが一つでも見つかれば矛盾する．これを目指す．
  背理法として，$(\mathbf{P}_\varepsilon)_{i,j} = 0$ なる成分が少なくとも一つあると仮定する．

  そのような「より小さい点」を，$\mathbf{P}_\varepsilon$ を全成分が正の実行可能点の方へ少し動かして作る．
  動かす先として独立結合 $\mathbf{Q} \defeq \mathbf{a}\mathbf{b}^\top$ をとると，$\sum_i a_i = \sum_j b_j = 1$ と $\mathbf{a} \in \Rpos^n$, $\mathbf{b} \in \Rpos^m$ より
  \[
    \mathbf{Q}\ones_m = \mathbf{a}, \qquad
    \mathbf{Q}^\top\ones_n = \mathbf{b}, \qquad
    Q_{i,j} = a_i b_j > 0 \quad (\forall i,j)
  \]
  をみたし，$\mathbf{Q} \in \CouplingsD(\mathbf{a}, \mathbf{b})$ かつ全成分が正である．凸性（主張~\ref{clm:sem-coupling-convex}）より線分
  \[
    ...
    \qquad (t \in [0,1])
  \]
  はすべて実行可能である．目標は，小さい $t > 0$ で $F(\mathbf{P}^t) < F(\mathbf{P}_\varepsilon)$（動かすと $F$ が下がること）を示すことである．

  \medskip
  そこで，動き出しにおける $F$ の傾きを調べる．$t \in (0,1]$ では全成分 $P^t_{i,j} \ge t\,a_i b_j > 0$ ゆえ $t \mapsto F(\mathbf{P}^t)$ は微分可能で，連鎖律より
  \begin{align*}
  ...
  \end{align*}
  となる（$\partial F/\partial P_{i,j}$ の計算で $\varphi'(x) = \log x$，注意~\ref{rem:sem-entropy-convention} を用いた）．
  $t \to 0+$ で各項を見ると，$(\mathbf{P}_\varepsilon)_{i,j} > 0$ の項は $\log P^t_{i,j} \to \log (\mathbf{P}_\varepsilon)_{i,j}$ ゆえ有限にとどまり，$(\mathbf{P}_\varepsilon)_{i,j} = 0$ の項は $P^t_{i,j} = t\,a_i b_j$ より
  \[
    \bigl(C_{i,j} + \varepsilon\log(t\,a_i b_j)\bigr)\,a_i b_j \xrightarrow{\ t \to 0+\ } -\infty
  \]
  （係数 $a_i b_j > 0$，$\log(t\,a_i b_j) \to -\infty$）と発散する．仮定よりこの型の項が少なくとも一つあり，$+\infty$ に飛ぶ項はないので
  \[
    \frac{d}{dt} F(\mathbf{P}^t) \xrightarrow{\ t \to 0+\ } -\infty .
  \]
  すなわち動き出しで $F$ の傾きは $-\infty$ である（負エントロピーの傾き $\log$ が境界 $0$ で $-\infty$ に落ちることが効く）．
  ゆえにある $\delta \in (0,1)$ が存在して $\dfrac{d}{dt} F(\mathbf{P}^t) < 0$（$0 < t < \delta$）．
  $t \mapsto F(\mathbf{P}^t)$ は $[0,\delta]$ で連続（$\varphi$ の連続性，注意~\ref{rem:sem-phi-continuous}）・$(0,\delta)$ で微分可能である．

  \medskip
  そこで $0 < t < \delta$ を固定し，区間 $[0,t]$ に平均値定理（定理~\ref{thm:sem-mvt}）を適用する．
  $\dfrac{d}{dt}F(\mathbf{P}^t) = \sum_{i,j}\bigl(C_{i,j} + \varepsilon\log P^t_{i,j}\bigr)\bigl(a_i b_j - (\mathbf{P}_\varepsilon)_{i,j}\bigr)$ だったから，ある $c \in (0,t)$ が存在して
  \[
    F(\mathbf{P}^t) - F(\mathbf{P}^0)
    = t \sum_{i,j}\bigl(C_{i,j} + \varepsilon\log P^c_{i,j}\bigr)\bigl(a_i b_j - (\mathbf{P}_\varepsilon)_{i,j}\bigr)
  \]
  となる（右辺は $\dfrac{d}{dt}F(\mathbf{P}^t)$ の $t = c$ での値の $t$ 倍）．
  $c \in (0,\delta)$ ゆえこの和は負，かつ $t > 0$ だから
  \[
    F(\mathbf{P}^t) < F(\mathbf{P}^0) = F(\mathbf{P}_\varepsilon) \qquad (0 < t < \delta).
  \]
  実行可能点 $\mathbf{P}^t$ が最小値を下回り，$\mathbf{P}_\varepsilon$ の最小性に矛盾する．ゆえに $\mathbf{P}_\varepsilon$ の全成分は正である．
\end{proof}
