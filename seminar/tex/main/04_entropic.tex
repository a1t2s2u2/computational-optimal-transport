...
\begin{proof}
  ...
  $h(t) \defeq \KLD(\mathbf{P}^t\|\mathbf{K})$ とおく．連鎖律の材料は次の二つである．
  \[
    \frac{\partial}{\partial P^t_{i,j}}\!\left( P^t_{i,j}\log\frac{P^t_{i,j}}{K_{i,j}} - P^t_{i,j} + K_{i,j} \right)
    = \log\frac{P^t_{i,j}}{K_{i,j}},
    \qquad
    \frac{d}{dt} P^t_{i,j} = Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j}.
  \]
  左は $\KLD$ の各項を成分で微分したもの，右は線分 $P^t_{i,j} = (1-t)(\mathbf{P}_\varepsilon)_{i,j} + t\,Q_{i,j}$ の $t$ 微分である．
  よって $t \in (0,1)$（このとき $P^t_{i,j} > 0$）では，連鎖律により
  \[
    h'(t)
    = \sum_{i,j} \frac{\partial \KLD}{\partial P^t_{i,j}} \cdot \frac{d P^t_{i,j}}{dt}
