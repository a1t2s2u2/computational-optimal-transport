...
\begin{proof}
  ...
  なお $\varphi$ は $(0,\infty)$ 上で $\varphi''(x) = 1/x > 0$ をみたすから，
  直感的には狭義凸である．ただし以下の証明は $\varphi''$ を用いず，
  導関数 $\varphi' = \log$ が狭義単調増加であることと平均値定理のみに依拠する．
  この論法は $\varphi$ が $C^1$ 級（導関数が狭義単調増加）でありさえすれば
  $C^2$ 級（$\varphi''$ の存在）を仮定せずに成り立つため，より一般的で優れている．
