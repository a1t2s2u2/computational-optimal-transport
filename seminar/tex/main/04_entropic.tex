...
\begin{proof}
  ...
  \[
    ...
    \right)
  \]
  としてよい．定理に入れるベクトル変数の関数は，この $G$ を $\mathbf{x}$ の座標で書き直した
  \[
    g(\mathbf{x})
    \defeq
    \sum_{i,j}
    \left(
      x_{(i-1)m+j}\log\frac{x_{(i-1)m+j}}{K_{i,j}}
      - x_{(i-1)m+j}
      + K_{i,j}
    \right)
    \qquad (\mathbf{x}\in U_x)
  \]
  と定める．したがって $g(\operatorname{vec}(\mathbf{P}))=G(\mathbf{P})$ であり，
  $\mathbf{P}$ を $\operatorname{vec}(\mathbf{P})$ に置き換えても目的関数の値は変わらない．
  また，この $g$ は $U_x$ 上で微分可能である．
  ...
  \[
    ...
    =
    \log\frac{x_{(i-1)m+j}}{K_{i,j}}
    \qquad (\mathbf{x}\in U_x)
  \]
  である．これを $\mathbf{x}=\mathbf{x}_\varepsilon$ に適用し，
  $(\mathbf{x}_\varepsilon)_{(i-1)m+j}=(\mathbf{P}_\varepsilon)_{i,j}$ を用いると
  \[
