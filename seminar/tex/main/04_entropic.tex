...
\begin{proof}
  ...
  行列 $\mathbf{P}\in\R^{n\times m}$ を行ごとに並べたベクトルを
  ...
  \]
  と定める．逆に，$\mathbf{x}\in\R^{nm}$ から作る行列を $\mathbf{P}(\mathbf{x})$ と書き，
  \[
    \mathbf{P}(\mathbf{x})_{i,j} \defeq x_{(i-1)m+j}
  \]
  とする．また
  \[
  ...
  \[
    g(\mathbf{x}) \defeq G(\mathbf{P}(\mathbf{x}))
  ...
  一方，$g(\mathbf{x})=G(\mathbf{P}(\mathbf{x}))$ の定義から
  \[
    \frac{\partial g}{\partial x_{(i-1)m+j}}(\mathbf{x})
    =
    \frac{\partial G}{\partial P_{i,j}}(\mathbf{P}(\mathbf{x}))
    =
    \log\frac{\mathbf{P}(\mathbf{x})_{i,j}}{K_{i,j}}
    \qquad (\mathbf{x}\in U_x)
  \]
  である．したがって
  \[
