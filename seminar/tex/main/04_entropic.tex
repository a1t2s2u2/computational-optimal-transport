...
\begin{proof}
  ...
  周辺条件を，制約関数
  \[
    h_i^{\mathrm{row}}(\mathbf{P}) \defeq \sum_{l=1}^{m} P_{i,l} - a_i \quad (i=1,\dots,n),
    \qquad
    h_j^{\mathrm{col}}(\mathbf{P}) \defeq \sum_{k=1}^{n} P_{k,j} - b_j \quad (j=1,\dots,m)
  \]
  によって
  \[
    h_i^{\mathrm{row}}(\mathbf{P}) = 0,\qquad h_j^{\mathrm{col}}(\mathbf{P}) = 0
  \]
  ...
  \[
    ...
    =
    \sum_{i'=1}^{n}\alpha_{i'}\,\nabla h_{i'}^{\mathrm{row}}(\mathbf{P}_\varepsilon)
    +
    \sum_{j'=1}^{m}\beta_{j'}\,\nabla h_{j'}^{\mathrm{col}}(\mathbf{P}_\varepsilon)
  \]
  が成り立つ．定理が直接与えるのはこの勾配等式である．
  Lagrangian の記法を使うなら，これは
  \[
    \mathcal{L}(\mathbf{P},\alpha,\beta)
    \defeq
    G(\mathbf{P})
    -
    \sum_{i'=1}^{n}\alpha_{i'} h_{i'}^{\mathrm{row}}(\mathbf{P})
    -
    \sum_{j'=1}^{m}\beta_{j'} h_{j'}^{\mathrm{col}}(\mathbf{P})
  \]
  を $\mathbf{P}$ の各成分で偏微分すると，$\mathbf{P}=\mathbf{P}_\varepsilon$ で
  すべて $0$ になる，という同値な書き換えである（上の勾配等式を左辺に移しただけ）．

  この 1 階条件を成分 $(i,j)$ で見る．まず
  \[
    \frac{\partial h_{i'}^{\mathrm{row}}}{\partial P_{i,j}}(\mathbf{P})
    =
    \begin{cases}
      1 & (i'=i),\\
      0 & (i'\neq i),
    \end{cases}
    \qquad
    \frac{\partial h_{j'}^{\mathrm{col}}}{\partial P_{i,j}}(\mathbf{P})
    =
    \begin{cases}
      1 & (j'=j),\\
      0 & (j'\neq j).
