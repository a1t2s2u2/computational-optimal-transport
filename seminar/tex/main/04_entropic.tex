...
\begin{remark}{連続版：Schr\"odinger 問題}{sem-entropic-schrodinger}
  連続版（$\X, \Y$ を Polish 空間（定義~\ref{def:sem-polish}）とする）でも対応する主張が成り立ち，最適カップリング $\pi$ は
...
非正則化 Kantorovich 問題（定義~\ref{def:sem-kantorovich}）が複数の最適解を持つとき，
エントロピー正則化はそのうちの一つ——
...
\begin{proof}
  最適解集合
  \[
    S^* = \bigl\{ \mathbf{P} \in \CouplingsD(\mathbf{a},\mathbf{b}) \bigm|
      \inner{\mathbf{C}}{\mathbf{P}} = \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b}) \bigr\}
  \]
  （主張~\ref{clm:sem-optimal-face}）を用いる．
  ...
  となり (i) を得る．
  なお上の挟み撃ちの両端から
  $0 \leq \varepsilon\bigl(\Hb(\mathbf{P}_\varepsilon) - \Hb(\mathbf{P}^*)\bigr)$
  となり，$\varepsilon > 0$ で割れば，後で用いる
  \[
  ...
  \textbf{(ii).}
  最大エントロピー最適解 $\mathbf{P}^\star \defeq \argmax_{\mathbf{P} \in S^*}\Hb(\mathbf{P})$
  はただ一つに定まる：$S^*$ は凸かつコンパクト（主張~\ref{clm:sem-optimal-face}），
  $-\Hb$ は連続で狭義凸（主張~\ref{clm:sem-neg-entropy-strict-convex}）だから，
  Weierstrass の定理（定理~\ref{thm:sem-weierstrass}）より $-\Hb$ は $S^*$ 上で最小値を達成し，
  その最小点は命題~\ref{prop:sem-strict-convex-unique-min} より一意である．

  $0$ に収束する正の列 $(\varepsilon_k)$ を任意にとり，$\mathbf{P}_{\varepsilon_k} \to \mathbf{P}^\star$ を示す．
  主張~\ref{clm:sem-subseq-limit-unique} により，
  $(\mathbf{P}_{\varepsilon_k})$ の収束する部分列の極限がすべて $\mathbf{P}^\star$ に等しいことを示せば十分である．
  そこで収束部分列 $\mathbf{P}_{\varepsilon_{k_j}} \to \mathbf{Q} \in \CouplingsD(\mathbf{a},\mathbf{b})$ をとる．
  内積の連続性と (i) より
  \[
    \inner{\mathbf{C}}{\mathbf{Q}}
    = \lim_{j\to\infty}\inner{\mathbf{C}}{\mathbf{P}_{\varepsilon_{k_j}}}
    = \MKD_{\mathbf{C}}(\mathbf{a},\mathbf{b}),
  \]
  すなわち $\mathbf{Q} \in S^*$．また (i) で得た不等式より任意の $\mathbf{P}^* \in S^*$ について
  $\Hb(\mathbf{P}_{\varepsilon_{k_j}}) \geq \Hb(\mathbf{P}^*)$ だから，
  $\Hb$ の連続性より $j\to\infty$ として $\Hb(\mathbf{Q}) \geq \Hb(\mathbf{P}^*)$．
  ゆえに $\mathbf{Q}$ は $S^*$ 上で $\Hb$ を最大化し，一意性より $\mathbf{Q} = \mathbf{P}^\star$．
  よって主張~\ref{clm:sem-subseq-limit-unique} から $\mathbf{P}_{\varepsilon_k} \to \mathbf{P}^\star$ であり，
  列 $(\varepsilon_k)$ は任意だったから $\mathbf{P}_\varepsilon \to \mathbf{P}^\star$（$\varepsilon \to 0$）を得る．
  ...
  \]
  の最小元でもある．$+\infty$ に発散する正の列 $(\varepsilon_k)$ を任意にとり，
  (ii) と同様に，$(\mathbf{P}_{\varepsilon_k})$ の収束する部分列の極限がすべて
  $\mathbf{a}\mathbf{b}^\top$ に等しいことを示す．
  収束部分列 $\mathbf{P}_{\varepsilon_{k_j}} \to \mathbf{P}^\infty \in \CouplingsD(\mathbf{a},\mathbf{b})$ をとると，
  任意の $\mathbf{Q} \in \CouplingsD(\mathbf{a},\mathbf{b})$ について最小性から
  \[
    \tfrac{1}{\varepsilon_{k_j}}\inner{\mathbf{C}}{\mathbf{P}_{\varepsilon_{k_j}}} - \Hb(\mathbf{P}_{\varepsilon_{k_j}})
    \leq
    \tfrac{1}{\varepsilon_{k_j}}\inner{\mathbf{C}}{\mathbf{Q}} - \Hb(\mathbf{Q})
  \]
  が成り立つ．ここで $j \to \infty$ とする．コスト $\inner{\mathbf{C}}{\cdot}$ は
  コンパクト集合上で有界だから，$\varepsilon_{k_j} \to +\infty$ より両辺のコスト項
  $\tfrac{1}{\varepsilon_{k_j}}\inner{\mathbf{C}}{\mathbf{P}_{\varepsilon_{k_j}}}$,
  $\tfrac{1}{\varepsilon_{k_j}}\inner{\mathbf{C}}{\mathbf{Q}}$ はともに $0$ に収束する．
  また $\Hb$ の連続性と $\mathbf{P}_{\varepsilon_{k_j}} \to \mathbf{P}^\infty$ より
  $\Hb(\mathbf{P}_{\varepsilon_{k_j}}) \to \Hb(\mathbf{P}^\infty)$．
  よって不等式は極限で
  \[
    -\Hb(\mathbf{P}^\infty) \leq -\Hb(\mathbf{Q}),
    \qquad\text{すなわち}\qquad
    \Hb(\mathbf{P}^\infty) \geq \Hb(\mathbf{Q})
  \]
  となり，これがすべての $\mathbf{Q} \in \CouplingsD(\mathbf{a},\mathbf{b})$ について成り立つ．
  ...
  以上で $(\mathbf{P}_{\varepsilon_k})$ の収束する部分列の極限がすべて
  $\mathbf{a}\mathbf{b}^\top$ に等しいとわかった．よって主張~\ref{clm:sem-subseq-limit-unique} より
  列全体が $\mathbf{P}_{\varepsilon_k} \to \mathbf{a}\mathbf{b}^\top$ と収束する．
  この列 $(\varepsilon_k)$ は $+\infty$ に発散する任意の列だったから，
