...
\begin{proof}
  ...
  周辺が正であることから，行 $i_0$ と列 $j_0$ に沿って正の成分を拾える．
  \begin{itemize}
    \item 行 $i_0$ から：$a_{i_0} = \sum_j P_{i_0, j} > 0$ より $P_{i_0, j_1} > 0$ なる $j_1$ が存在する．
      $P_{i_0, j_0} = 0$ ゆえ $j_1 \neq j_0$．
    \item 列 $j_0$ から：$b_{j_0} = \sum_i P_{i, j_0} > 0$ より $P_{i_1, j_0} > 0$ なる $i_1$ が存在する．
      $P_{i_0, j_0} = 0$ ゆえ $i_1 \neq i_0$．
  \end{itemize}
  こうして $2\times2$ の四隅のうち
