...
\begin{proof}
  ...
  \emph{方針．}
  全成分が正の実行可能点 $\mathbf{Q}$ へ向かう線分 $\mathbf{P}^t = (1-t)\mathbf{P}_\varepsilon + t\mathbf{Q}$ をとり，
  差分商 $\bigl(F(\mathbf{P}^t) - F(\mathbf{P}_\varepsilon)\bigr)/t$ を直接評価する．
  ゼロ成分に対応する項が $\varphi(t\,a_ib_j) = t\,a_ib_j(\log(t\,a_ib_j) - 1)$ を $t$ で割った形で現れ，
  $\log(t\,a_ib_j) \to -\infty$ により差分商は $t \to 0+$ で $-\infty$ に発散する（他の項は有限極限をもつ）．
  ゆえに十分小さい $t > 0$ で $F(\mathbf{P}^t) < F(\mathbf{P}_\varepsilon)$ となり，最小性に反する．
  ...
  添字集合を，$\mathbf{P}_\varepsilon$ の正負に応じて
  \[
  ...
  \emph{差分商の評価．}
  $\inner{\mathbf{C}}{\cdot}$ の線形性と $-\Hb(\mathbf{P}^t) = \sum_{i,j}\varphi(P^t_{i,j})$ より
  \[
    \frac{F(\mathbf{P}^t) - F(\mathbf{P}_\varepsilon)}{t}
    = \inner{\mathbf{C}}{\mathbf{Q} - \mathbf{P}_\varepsilon}
    + \varepsilon\sum_{i,j}\frac{\varphi(P^t_{i,j}) - \varphi\bigl((\mathbf{P}_\varepsilon)_{i,j}\bigr)}{t}
    \qquad (0 < t < 1)
  \]
  であり（第 1 項は $\inner{\mathbf{C}}{\mathbf{P}^t - \mathbf{P}_\varepsilon} = t\inner{\mathbf{C}}{\mathbf{Q} - \mathbf{P}_\varepsilon}$ を $t$ で割ったもの），
  以下この和の $t \to 0+$ での挙動を $S_+$, $S_0$ 別に調べる．

  $(i,j) \in S_+$ のとき．$p_0 \defeq (\mathbf{P}_\varepsilon)_{i,j} > 0$ とおくと，
  $P^t_{i,j} = p_0 + t\,(Q_{i,j} - p_0)$ である．
  $\varphi$ は $p_0 > 0$ で微分可能で $\varphi'(p_0) = \log p_0$（注意~\ref{rem:sem-entropy-convention}）だから，
  この差分商は，直線 $t \mapsto p_0 + t\,(Q_{i,j} - p_0)$ に沿って読んだ $p_0$ での微分係数の定義そのものであり，
  \[
  ...
  \]
  と有限極限に収束する．$S_+$ は有限集合だから，$S_+$ にわたる和も有限極限に収束し，
  とくに $0$ の近傍で有界である．

  $(i,j) \in S_0$ のとき．$\varphi\bigl((\mathbf{P}_\varepsilon)_{i,j}\bigr) = \varphi(0) = 0$（注意~\ref{rem:sem-phi-continuous}）であり，
  $P^t_{i,j} = t\,Q_{i,j} = t\,a_i b_j$ だから
  \[
  ...
  \]
  $a_i b_j > 0$ かつ $\log(t\,a_i b_j) \to -\infty$（$t \to 0+$）なので，
  この項は $-\infty$ に発散する．$S_0 \neq \emptyset$ より，こうした項は少なくとも一つ存在する．

  \emph{矛盾．}
  以上より，$t \to 0+$ のとき差分商の右辺第 1 項 $\inner{\mathbf{C}}{\mathbf{Q} - \mathbf{P}_\varepsilon}$ は定数，
  $S_+$ にわたる和は有限極限をもち（$0$ の近傍で有界），
  $S_0$ にわたる和は $-\infty$ に発散する．$+\infty$ に発散する項はないので相殺は起こらず，
  \[
    \frac{F(\mathbf{P}^t) - F(\mathbf{P}_\varepsilon)}{t}
    \xrightarrow{\ t \to 0+\ } -\infty.
  \]
  したがって，ある $\delta \in (0,1)$ が存在して，$0 < t < \delta$ なるすべての $t$ で
  $\bigl(F(\mathbf{P}^t) - F(\mathbf{P}_\varepsilon)\bigr)/t < 0$，
  すなわち $t > 0$ と合わせて
  \[
