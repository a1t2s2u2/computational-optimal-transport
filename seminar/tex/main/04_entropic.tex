...
\begin{proposition}{正則化解の形（スケーリング形）}{sem-entropic-scaling-form}
  ...
  \]
  と書ける（対角行列 $\diag(\cdot)$ は定義~\ref{def:sem-diag}）．スケーリングベクトル $(\mathbf{u}, \mathbf{v})$ は，$\lambda > 0$ による
  置き換え $(\mathbf{u}, \mathbf{v}) \mapsto (\lambda\mathbf{u}, \lambda^{-1}\mathbf{v})$ を除いて一意である．
\end{proposition}
...
\begin{proof}
  ...
  命題~\ref{prop:sem-entropic-kl-projection} より $\mathbf{P}_\varepsilon$ は $\KLD(\cdot \| \mathbf{K})$ の $\CouplingsD(\mathbf{a}, \mathbf{b})$ 上の最小元であり，定義~\ref{def:sem-kl-divergence} より
  \[
    \frac{\partial}{\partial P_{i,j}} \KLD(\mathbf{P} \| \mathbf{K}) = \log\frac{P_{i,j}}{K_{i,j}} .
  \]
  これらの等式制約は $\mathbf{P}$ の成分について 1 次式だから，全成分が正の最小元 $\mathbf{P}_\varepsilon$ で Lagrange の 1 階条件（定理~\ref{thm:sem-lagrange-foc}）が成り立つ．各成分 $P_{i,j}$ は第 $i$ 行の行和制約と第 $j$ 列の列和制約にのみ現れるので，これらに対応する乗数を $\alpha \in \R^n$, $\beta \in \R^m$ とすると，上の偏微分に対する 1 階条件は
  \[
  ...
  自由度については，任意の $\lambda > 0$ で $(\lambda u_i)(\lambda^{-1} v_j) = u_i v_j$ ゆえ $(\lambda\mathbf{u}, \lambda^{-1}\mathbf{v})$ も同じ $\mathbf{P}_\varepsilon$ を与える．
  逆に，$w_{i,j} \defeq u_i v_j = (\mathbf{P}_\varepsilon)_{i,j} / K_{i,j} > 0$ は $\mathbf{P}_\varepsilon$ から確定し，比 $u_i / u_{i'} = w_{i,j} / w_{i',j}$ は $j$ に依らないから $\mathbf{u}$ は正の定数倍を除いて一意，$v_j = w_{i,j} / u_i$ も定まる．よって自由度は $\lambda > 0$ のみである．
\end{proof}
