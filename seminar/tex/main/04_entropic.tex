...
\begin{proposition}{正則化解の形（スケーリング形）}{sem-entropic-scaling-form}
  ...
  \]
  と書ける．スケーリングベクトル $(\mathbf{u}, \mathbf{v})$ は，$\lambda > 0$ による
...
\begin{proof}
  ...
  \]
  これらの等式制約は $\mathbf{P}$ の成分について 1 次式だから，全成分が正の最小元 $\mathbf{P}_\varepsilon$ で Lagrange の 1 階条件が成り立つ．行和・列和の制約に対応する乗数 $\alpha \in \R^n$, $\beta \in \R^m$ により
  \[
