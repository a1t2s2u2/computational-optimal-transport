...
\begin{proof}
  ...
  背理法により，$(\mathbf{P}_\varepsilon)_{i,j} = 0$ なる成分が少なくとも一つあると仮定する．
  独立結合 $\mathbf{Q} \defeq \mathbf{a}\mathbf{b}^\top$ は，$\sum_i a_i = \sum_j b_j = 1$ と $\mathbf{a} \in \Rpos^n$, $\mathbf{b} \in \Rpos^m$ より
  \[
  ...
  \[
    ...
    \qquad (t \in [0,1])
  \]
  も実行可能である．$t > 0$ ならば次の量の符号は $F(\mathbf{P}^t) - F(\mathbf{P}_\varepsilon)$ の符号に等しいので，これを $t \to 0+$ で評価する：
  \begin{align*}
  ...
  \end{align*}
  第 2 行は $\varphi$ が $(\mathbf{P}_\varepsilon)_{i,j} > 0$ で微分可能（$\varphi'(x) = \log x$，注意~\ref{rem:sem-entropy-convention}）なこと，
  第 3 行は $\varphi(0) = 0$（注意~\ref{rem:sem-phi-continuous}）と $P^t_{i,j} = t\,a_i b_j$ による．
