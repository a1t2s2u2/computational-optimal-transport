...
\begin{proof}
  ...
  行和制約と列和制約の係数行列を
  \[
    (\mathbf{A}_i)_{p,\ell} \defeq
    \begin{cases}
      1 & (p=i),\\
      0 & (p\neq i),
    \end{cases}
    \qquad
    (\mathbf{B}_j)_{p,\ell} \defeq
    \begin{cases}
      1 & (\ell=j),\\
      0 & (\ell\neq j)
    \end{cases}
  ...
  と定める．周辺条件は
  \[
    \inner{\mathbf{A}_i}{\mathbf{P}} = a_i,
    \qquad
    \inner{\mathbf{B}_j}{\mathbf{P}} = b_j
  \]
  ...
  \[
    ...
    =
    \sum_{i'=1}^{n}\alpha_{i'}\,\mathbf{A}_{i'}
    +
    \sum_{j'=1}^{m}\beta_{j'}\,\mathbf{B}_{j'}
  \]
  が成り立つ．この $(i,j)$ 成分を読むと
  $\mathbf{A}_{i'}$ は $i'=i$ のときだけ，
  $\mathbf{B}_{j'}$ は $j'=j$ のときだけ $(i,j)$ 成分が $1$ なので，
  \[
