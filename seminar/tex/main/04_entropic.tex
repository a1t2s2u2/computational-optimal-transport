...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \sum_{i,j}
       \frac{\partial}{\partial P_{i,j}}\Bigl( \inner{\mathbf{C}}{\mathbf{P}} + \varepsilon\!\sum_{k,l}\varphi(P_{k,l}) \Bigr)\bigg|_{\mathbf{P} = \mathbf{P}^t}
  ...
  ゆえにある $\delta \in (0,1)$ が存在して $\dfrac{d}{dt} F(\mathbf{P}^t) < 0$（$0 < t < \delta$）．$t \mapsto F(\mathbf{P}^t)$ は $[0,\delta]$ で連続（$\varphi$ の連続性，注意~\ref{rem:sem-phi-continuous}）・$(0,\delta)$ で微分可能だから，平均値定理（定理~\ref{thm:sem-mvt}）より $[0,\delta]$ で狭義減少し，
  \[
