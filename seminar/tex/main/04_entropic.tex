...
\begin{proof}
  ...
  行列全体の空間 $\R^{n\times m}$ を，成分数 $nm$ の有限次元空間として $\R^{nm}$ と同一視して適用する．
  ただし本文では，座標を一列に並べた番号ではなく，行列成分として自然な二重添字 $P_{i,j}$ で表す．
  ...
  定理~\ref{thm:sem-lagrange-foc} の記号との対応を明示しておく．
  まず全体としては
  \[
    \begin{array}{ccl}
      N & \leftrightarrow & nm,\\
      \mathbf{x} & \leftrightarrow & \mathbf{P},\\
      f & \leftrightarrow & G,\\
      \mathbf{x}^* & \leftrightarrow & \mathbf{P}_\varepsilon.
    \end{array}
  \]
  定理の制約数 $r$ はここでは $n+m$ であり，制約関数 $r_i$ の文字 $r$ とは別である．
  制約の番号付けは，行制約を $k=i$（$i=1,\dots,n$），列制約を
  $k=n+j$（$j=1,\dots,m$）と見ればよい．
  行制約については
  \[
    \mathbf{a}_i^\top\mathbf{x} = c_i
    \quad \leftrightarrow \quad
    \sum_{l=1}^{m} P_{i,l} = a_i,
    \qquad
    \mathbf{a}_i = \nabla r_i,\quad
    c_i = a_i,\quad
    \mu_i = \alpha_i .
  ...
  列制約については
  \[
    \mathbf{a}_{n+j}^\top\mathbf{x} = c_{n+j}
    \quad \leftrightarrow \quad
    \sum_{k=1}^{n} P_{k,j} = b_j,
    \qquad
    \mathbf{a}_{n+j} = \nabla c_j,\quad
    c_{n+j} = b_j,\quad
    \mu_{n+j} = \beta_j .
  \]
  ここで左辺側の $\mathbf{a}_k, c_k, \mu_k$ は定理~\ref{thm:sem-lagrange-foc} の記号であり，
  ヒストグラム $\mathbf{a}$ や制約関数 $c_j$ とは別物である．
  1 次制約では，制約関数の勾配がその係数ベクトルそのものになる．
