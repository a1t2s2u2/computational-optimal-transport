...
\begin{proof}
  ...
  \emph{背理法と線分．}
  ある成分が $(\mathbf{P}_\varepsilon)_{i_0, j_0} = 0$ であると仮定する．
  ...
  \begin{itemize}
    ...
    \item $(\mathbf{P}_\varepsilon)_{i,j} = 0$ の項（$(i_0, j_0)$ を含む）：係数 $Q_{i,j} - (\mathbf{P}_\varepsilon)_{i,j} = a_i b_j > 0$，
      かつ $\log(P^t_{i,j}/K_{i,j}) = \log(t\,a_i b_j / K_{i,j}) \to -\infty$ だから，項は $-\infty$ に発散する．
  \end{itemize}
  $+\infty$ に発散する項は一つもなく，$(\mathbf{P}_\varepsilon)_{i_0, j_0} = 0$ の項が $-\infty$ に発散するから
