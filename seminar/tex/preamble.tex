...
%   \blockmeta{space=polish; uses=prop:foo; route.A=prop:bar; lean=ot.baz}
...
\DeclareMathOperator{\smin}{smin}          % ソフト最小値（古典双対章で使用）
\newcommand{\dHil}{d_{\mathcal{H}}}        % Hilbert 射影距離

% --- Ground 距離 ---
\newcommand{\dist}{d}
\newcommand{\distD}{\mathbf{D}}

% --- Wasserstein 幾何（測地線・曲率） ---
\newcommand{\Pp}{\mathcal{P}}                % p 次モーメント有限な測度の空間
\newcommand{\Lcal}{\mathcal{L}}              % ルベーグ測度（\Lcal^n の形で使用）
\newcommand{\Hm}{\mathrm{H}}                 % 相対エントロピー（離散版 \Hb と意味的に区別）
\DeclareMathOperator{\Ric}{Ric}              % Ricci 曲率テンソル
\newcommand{\CD}{\mathrm{CD}}                % CD(K, N) 条件
\newcommand{\Tan}{\mathrm{Tan}}              % 接空間（Otto 計算 memo で使用）
\newcommand{\diverg}{\nabla\!\cdot\!}        % 発散（連続の方程式・動的定式化で使用）
