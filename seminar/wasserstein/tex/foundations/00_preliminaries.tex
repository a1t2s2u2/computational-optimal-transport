...
\begin{claim}{実数列の部分列原理}{meas-subseq-principle}
  実数列 $(a_k)$ と $a\in\R$ について，
  $(a_k)$ の任意の部分列が，$a$ に収束するさらなる部分列をもつならば，
  $a_k\to a$ である．
\end{claim}

\begin{proof}
  $a_k\to a$ でないとすると，ある $\varepsilon>0$ と部分列 $(a_{k_j})$ で，
  すべての $j$ において $\abs{a_{k_j}-a}\geq\varepsilon$
  となるものが取れる．この部分列のどの部分列も $a$ に収束しないから，
  仮定に反する．
\end{proof}
...
\begin{proof}
  ...
  $(a_k)$ の任意の部分列 $(a_{k_j})$ に対し，tightness と
  Prokhorov の定理より $(\eta_{k_j})$ は弱収束する部分列をもち，
  その極限は仮定より $\eta$ だから，対応する $(a_{k_j})$ の部分列は
  $\int f\,\d\eta$ に収束する．
  部分列原理（主張~\ref{clm:meas-subseq-principle}）より
  $a_k\to\int f\,\d\eta$，すなわち $\eta_k\Rightarrow\eta$ である．
\end{proof}
...
\begin{lemma}{弱収束と下半連続関数}{meas-weak-lsc}
...
$\lambda_1$ は $\sigma$-有限である（$\R=\bigcup_n[-n,n]$）．
