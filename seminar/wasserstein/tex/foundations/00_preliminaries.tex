...
\begin{lemma}{下半連続関数の単調連続近似}{meas-lsc-approx}
  $S$ を距離空間，$f\colon S\to[0,+\infty]$ を下半連続関数とする．
  各 $\ell\in\N$ に対し
  \[
    f_\ell(x):=\inf_{z\in S}\bigl\{\min\{f(z),\ell\}+\ell\,d(x,z)\bigr\}
  \]
...
\begin{proof}
  ...
  連続性：任意の $x,x',z\in S$ に対し，三角不等式より
  \[
...
\begin{lemma}{周辺分布と周辺積分公式}{meas-marginal-integral}
  ...
  非負可測な $f\colon X\to[0,+\infty]$ または
  $\mu$-可積分な $f\colon X\to\R$ に対して
  \[
