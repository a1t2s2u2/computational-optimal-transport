...
行列・転置 $A^\top$・逆行列・行列式 $\det A$・
対角行列 $\diag(\lambda_1,\dots,\lambda_n)$・
固有値・固有ベクトル・固有値の重複度・階数 $\rank A$・
基底・一次独立・線形包 $\operatorname{span}$・核 $\ker A$ などの
基本概念は既知とする
（例えば齋藤正彦『線型代数入門』（東京大学出版会），
Horn--Johnson \emph{Matrix Analysis} (2nd ed., 2013) を参照）．
互いに直交する単位ベクトルの組を\textbf{正規直交系}，
それが $n$ 本そろったものを $\R^n$ の\textbf{正規直交基底}とよぶ．
また Kronecker のデルタを
$\delta_{jl}$（$j=l$ のとき $1$，それ以外のとき $0$）と書く．
...
\begin{theorem}{スペクトル分解}{la-spectral}
  ...
  Horn--Johnson (2013) \S4.1，齋藤『線型代数入門』参照．）
\end{theorem}
...
\begin{proof}
  (i)：まず各固有値 $\nu_j$ は対角成分 $\lambda_1,\dots,\lambda_n$ の
  いずれかに現れる：$Mv=\nu_jv$（$v\neq0$）なら $v=\sum_ic_iu_i$ と
  基底で展開して $\sum_ic_i(\lambda_i-\nu_j)u_i=0$，
  一次独立性から $c_i(\lambda_i-\nu_j)=0$（$\forall i$）であり，
  ある $c_i\neq0$ だから $\nu_j\in\{\lambda_1,\dots,\lambda_n\}$ である．
  $R_j$ は対称行列の和ゆえ対称で，
  $\lambda_i=\nu_j$ なる $u_i$ に対し $R_ju_i=u_i\neq0$ だから零でない．
...
\begin{proof}
  ...
  \]
  であり，$AA^\top=\sum_id_i^2\,u_iu_i^\top$ も
  $\{v_i\}$ の正規直交性から同様の計算で従う
  （したがってこの2式は，上で構成した SVD に限らず
  $A=\sum_id_iu_iv_i^\top$ の形の任意の表示について成り立つ）．
\end{proof}
...
\begin{proof}
  ...
  多項式 $p$ を取る（$\mu_j$ は相異なるので
  \[
    p(t):=\sum_{j=1}^m\sqrt{\mu_j}\prod_{l\neq j}\frac{t-\mu_l}{\mu_j-\mu_l}
  \]
  と明示的に書ける）．
