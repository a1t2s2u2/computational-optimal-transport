...
\begin{proof}
  ...
  $B$ の相異なる固有値を $\mu_1,\dots,\mu_m$（$\geq0$），
  対応する固有空間 $\ker(B-\mu_jI)$ への直交射影を $Q_j$ とすると，
  スペクトル分解を固有値ごとにまとめて
  \[
    ...
    \sum_{j=1}^mQ_j=I
  \]
  と書ける（$Q_j$ は $\lambda_i=\mu_j$ なる $u_iu_i^\top$ の和）．
  このとき
  \[
    \Sigma=B^2=\sum_{j=1}^m\mu_j^2\,Q_j
  \]
  であり，$\mu_j\geq0$ が相異なることから $\mu_j^2$ も相異なる．
  よって右辺は $\Sigma$ の「相異なる固有値と固有空間への直交射影」による
  分解であり，$\Sigma$ の固有値 $\mu_j^2$ と
  固有空間 $\ker(\Sigma-\mu_j^2I)=\operatorname{ran}Q_j$ は
  $\Sigma$ だけから定まる．
  したがって各 $Q_j$ と $\mu_j=\sqrt{\mu_j^2}$（非負の平方根）も
  $\Sigma$ だけから定まり，$B=\sum_j\mu_jQ_j$ は一意である．
\end{proof}
...
\begin{proof}
  ...
  このとき $\operatorname{ran}A^\top$ は
  $\{A^\top u_i\}_{i=1}^k=\{d_iv_i\}_{i\leq r'}\cup\{0\}$ の張る空間だから
  $\rank A=\rank A^\top=r'$，すなわち $r'=r$ である．
  最後に，$\{v_i\}$ の正規直交性から
  \[
    A^\top A
...
\begin{definition}{特異値}{la-singular}
  ...
  補題~\ref{lem:la-svd}より，非零の特異値の2乗は
  $AA^\top$ の非零固有値とも $A^\top A$ の非零固有値とも一致する．
...
\begin{proof}
  ...
  $B$ は対称で，固有値は $d_i>0$ と $0$ だから $B\succeq0$
  （主張~\ref{clm:la-psd-eigen}），かつ正規直交性より
  \[
...
\begin{proof}
  ...
  $V_1:=\operatorname{ran}\Sigma_1^{1/2}$，
  $V_2:=\operatorname{ran}\Sigma_2^{1/2}$ とおき，
  ...
  $V_1$ の正規直交基底 $e_1,\dots,e_m$ を取り，$b\in V_2$ に対し
  \[
  ...
  \]
  より $\norm{c(b)}\leq\norm{b}$ である．
  $P$ を $V_2$ への直交射影として $C:=c\circ P$
  （$w\mapsto c(Pw)$ を表す行列）とおけば，
  $\norm{Cw}=\norm{c(Pw)}\leq\norm{Pw}\leq\norm{w}$ より
  $\norm{C}_{\mathrm{op}}\leq1$ である．

  最後に $K=\Sigma_1^{1/2}C\Sigma_2^{1/2}$ を確かめる．
  任意の $u,v$ に対し，$\Sigma_2^{1/2}v\in V_2$ と
  $\Sigma_1^{1/2}u\in V_1$ から
  \[
...
\begin{proof}
  $\Sigma$ の相異なる固有値を $\mu_1,\dots,\mu_m\geq0$，
  固有空間への直交射影を $Q_j$ とし，
  $\Sigma=\sum_j\mu_jQ_j$（$Q_jQ_l=\delta_{jl}Q_j$，$\sum_jQ_j=I$）と書く．
