...
さらに，$\Wass_p$ が $\Pp{p}(\mathcal{X})$ の弱収束を距離化し，
$(\Pp{p}(\mathcal{X}),\Wass_p)$ が Polish 空間になること
（同 Theorems 6.9, 6.18）を示し，
応用として，Talagrand の不等式から
標準 Gaussian 測度の次元によらない集中不等式を導く
（同 Chapter 22）．
主たる文献は Villani
...
\begin{itemize}
  ...
  \item 第\ref{ch:topology}章：
    $\Pp{p}(\mathcal{X})$ における弱収束を定義し，
    $\Wass_p$ がそれを距離化すること，および
    $(\Pp{p}(\mathcal{X}),\Wass_p)$ が Polish 空間になることを証明する．
