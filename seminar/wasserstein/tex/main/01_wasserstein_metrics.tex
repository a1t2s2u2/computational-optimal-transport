...
\begin{remark}{記法と well-definedness}{wp-welldef}
  距離関数 $d\colon X\times X\to[0,\infty)$ は
  $\pi\in\Prob(X\times X)$ 上の可測関数だから，
  $L^p$ ノルム（定義~\ref{def:meas-lp-norm}）を用いて
  \[
    \Wass_p(\mu,\nu)
    =\inf_{\pi\in\Couplings(\mu,\nu)}\norm{d}_{L^p(\pi)}
  \]
...
\begin{definition}{Lipschitz 関数}{wp-lipschitz-function}
  ...
  \]
  が成り立つことをいう．
  さらに上限（定義~\ref{def:meas-sup-inf}）の意味で $\sup_{x\in X}\abs{f(x)}<\infty$ であるとき，
...
\begin{proof}
  ...
  を任意に取る．補題~\ref{lem:wp-gluing}により，$X^3$ 上の確率測度
