...
\begin{definition}{Coupling 集合}{wp-coupling}
  ...
  すなわち射影の像測度として
  $\mathrm{pr}_1(\pi)=\mu$，$\mathrm{pr}_2(\pi)=\nu$
...
\begin{lemma}{接着補題（gluing lemma）}{wp-gluing}
  ...
  $\mathrm{pr}_{12}(\gamma)$ のことをいう（$(y,z)$ も同様．
...
\begin{proof}
  ...
  $s(\sigma)$ の第2周辺分布は $\sigma$ の第1周辺分布，
  すなわち $\nu$ である．
  再び分解定理を $s(\sigma)$ に適用して，Markov 核 $K_\sigma$ で
  \[
    \sigma(B\times C)
    =(s(\sigma))(C\times B)
  ...
  $\mathrm{pr}_{12}(\gamma)=\pi$ である．
  同様に $K_\pi(y,X)=1$ より
  \[
    (\mathrm{pr}_{23}(\gamma))(B\times C)
    =\gamma(X\times B\times C)
    =\int_B K_\pi(y,X)\,K_\sigma(y,C)\,\d\nu(y)
    =\int_B K_\sigma(y,C)\,\d\nu(y)
    =\sigma(B\times C)
  \]
  だから $\mathrm{pr}_{23}(\gamma)=\sigma$ である．
\end{proof}
...
\begin{proof}
  ...
  $\pi^*:=\iota(\mu)$ は，$\iota^{-1}(A\times B)=A\cap B$ より
  \[
  ...
  $\mathrm{pr}_{12}(\gamma)=\pi$，
  $\mathrm{pr}_{23}(\gamma)=\sigma$
  なるものが取れる．
  $\tau:=\mathrm{pr}_{13}(\gamma)$ とおくと，
  \[
