...
\begin{remark}{$\R^n$ は Polish 空間}{gauss-rn-polish}
  ...
  実数の完備性（注意~\ref{rem:meas-real-facts}）から各座標が収束するので
  全体も収束する．
  ...
  \]
  とする．
  なお直積 $\R^n\times\R^n$ に付録Aの $\max$ 距離を入れたものと
  $\R^{2n}$ の Euclid 距離は，
  $\max\{\norm{x},\norm{y}\}\leq\norm{(x,y)}\leq\sqrt{2}\max\{\norm{x},\norm{y}\}$
  より開集合が一致し，Borel $\sigma$-代数も一致する
  （$\Borel(\R^n\times\R^n)=\Borel(\R^{2n})$）．
  以下この同一視を断りなく用いる．
  また本章では，行列 $A\in\R^{k\times n}$ を線形写像 $x\mapsto Ax$ と
  同一視し，その像測度（定義~\ref{def:meas-pushforward}）を
  $\gamma_n\circ A^{-1}$ のように書く．
  ここで $A^{-1}(B)$ は\textbf{逆像}であり，$A$ の正則性は仮定しない
  （$A$ が正則なら逆行列による逆像と一致するので，記号に矛盾はない）．
\end{remark}
...
\begin{claim}{平均・共分散の基本性質}{gauss-cov-basic}
  ...
  \begin{enumerate}
    ...
    \item （アフィン像の平均・共分散）$L\in\R^{k\times n}$，$b\in\R^k$，
      $\ell(x):=Lx+b$ とすると
      （この形の写像を\textbf{アフィン写像}とよぶ），
      $\nu:=\mu\circ\ell^{-1}\in\Pp{2}(\R^k)$ で
      \[
        m_\nu=Lm_\mu+b,
        \qquad
        \Sigma_\nu=L\Sigma_\mu L^\top.
...
\begin{proof}
  ...
  $\lambda_2=\lambda_1\otimes\lambda_1$ と
  Fubini--Tonelli の定理（いずれも定理~\ref{thm:meas-product}），
  および極座標変換（定理~\ref{thm:meas-polar}）から
  \[
    I^2
  ...
  次に $\abs{t}\varphi(t)$ は偶関数で，
  $\int_0^\infty t\varphi(t)\,\d t=\varphi(0)$ だから
  $\int_\R\abs{t}\varphi(t)\,\d\lambda_1\leq2\varphi(0)<\infty$，
  すなわち $t\varphi(t)$ は可積分である．
  鏡映 $r(t):=-t$ は $\abs{\det(-1)}=1$ のアフィン写像なので
  $\lambda_1\circ r^{-1}=\lambda_1$（定理~\ref{thm:meas-affine-cov}），
...
\begin{definition}{標準 Gaussian 測度}{gauss-standard}
  ...
  （$\lambda_n=\lambda_1\otimes\cdots\otimes\lambda_1$ と
...
\begin{claim}{$\gamma_n$ の直積構造とモーメント}{gauss-standard-moments}
  \begin{enumerate}
    \item $\gamma_n=\gamma_1\otimes\cdots\otimes\gamma_1$（$n$ 個の直積）であり，
      $1\leq r\leq n$ のとき，最初の $r$ 座標への射影
      $\mathrm{pr}_r(x_1,\dots,x_n):=(x_1,\dots,x_r)$ について
      $\gamma_n\circ\mathrm{pr}_r^{-1}=\gamma_r$ である
      （$r=n$ では $\mathrm{pr}_n$ は恒等写像で自明）．
...
\begin{proof}
  ...
  （注意~\ref{rem:meas-borel-product}）から，
  一致定理（定理~\ref{thm:meas-uniqueness}）より
  $\gamma_n=\gamma_1^{\otimes n}$ である．
  射影については，$r=n$ なら $\mathrm{pr}_n$ は恒等写像で自明．
  $r<n$ のときは同様に $\gamma_n=\gamma_r\otimes\gamma_{n-r}$ だから，
  直積測度の周辺分布（主張~\ref{clm:meas-product-marginals}）より
  \[
    (\gamma_n\circ\mathrm{pr}_r^{-1})(A)
  ...
  $i\neq j$ なら，$\abs{x_ix_j}\leq\frac{1}{2}(x_i^2+x_j^2)$ と
  前半より $x_ix_j$ は可積分なので，Fubini の定理で
  $\int x_ix_j\,\d\gamma_n
  =\bigl(\int t\varphi\,\d\lambda_1\bigr)^2=0$ となり，
  共分散は $I_n$ である．
\end{proof}

\begin{lemma}{直交不変性}{gauss-rotation}
  $U\in\R^{n\times n}$ が直交行列ならば
  $\gamma_n\circ U^{-1}=\gamma_n$ である．
\end{lemma}
...
\begin{proof}
  ...
  右辺は $\gamma_n(U^{-1}B)=(\gamma_n\circ U^{-1})(B)$ である．
\end{proof}

\begin{lemma}{標準 Gaussian の線形像（鍵補題）}{gauss-linear-pf}
  任意の $A\in\R^{k\times n}$ に対し
  \[
    \gamma_n\circ A^{-1}=\gamma_k\circ\bigl((AA^\top)^{1/2}\bigr)^{-1}.
  \]
\end{lemma}
...
\begin{proof}
  ...
  $r=0$（$A=O$）のときは，$O^2=O$ と平方根の一意性
  （定理~\ref{thm:la-sqrt}）から $(AA^\top)^{1/2}=O^{1/2}=O$ であり，
  両辺とも定値写像 $0$ による像測度で一致するから，$r\geq1$ とする．
  正規直交系 $\{v_i\}_{i\leq r}$，$\{u_i\}_{i\leq r}$ をそれぞれ
  $\R^n$，$\R^k$ の正規直交基底 $\{v_i\}_{i=1}^n$，$\{u_i\}_{i=1}^k$ に
  延長し（主張~\ref{clm:la-onb-extension}；$r=n$ や $r=k$ のときは
  延長は不要），
  $V:=[v_1,\dots,v_n]$，$U:=[u_1,\dots,u_k]$ とおく．
  列の正規直交性から $V^\top V=I$ で，$V$ は正方だから
  $V^{-1}=V^\top$，よって $VV^\top=I$ も成り立ち，
  $V$（同様に $U$）は直交行列である．
  ...
  また $B:=\sum_{i=1}^rd_iu_iu_i^\top$ は対称で，
  $x^\top Bx=\sum_id_i\inner{u_i}{x}^2\geq0$ より半正定値，
  かつ $B^2=\sum_id_i^2u_iu_i^\top=AA^\top$
  （補題~\ref{lem:la-svd}）だから，平方根の一意性
  （定理~\ref{thm:la-sqrt}）より $(AA^\top)^{1/2}=B$ であり，
  同じ計算で $B=U\circ D'\circ U^\top$ である．

  像測度の合成則（主張~\ref{clm:meas-pf-compose}）と
  直交不変性（補題~\ref{lem:gauss-rotation}；$V^\top,U^\top$ も直交行列）より
  \[
    \gamma_n\circ A^{-1}
    =\gamma_n\circ(V^\top)^{-1}\circ D^{-1}\circ U^{-1}
    =\gamma_n\circ D^{-1}\circ U^{-1},
    \qquad
    \gamma_k\circ B^{-1}
    =\gamma_k\circ(D')^{-1}\circ U^{-1}
  \]
  だから（1つ目と3つ目の等号は合成則の繰り返し．
  3つ目ではさらに直交不変性 $\gamma_k\circ(U^\top)^{-1}=\gamma_k$ を，
  2つ目では $\gamma_n\circ(V^\top)^{-1}=\gamma_n$ を使った），
  $\gamma_n\circ D^{-1}=\gamma_k\circ(D')^{-1}$ を示せばよい．
  ...
  最初の $r$ 座標への射影）と書ける．
  主張~\ref{clm:gauss-standard-moments}(i)（$r\leq n$，$r\leq k$）より
  $\gamma_n\circ\mathrm{pr}_r^{-1}=\gamma_r=\gamma_k\circ(\mathrm{pr}_r')^{-1}$
  だから，合成則より
  \[
    \gamma_n\circ D^{-1}
    =\gamma_n\circ\mathrm{pr}_r^{-1}\circ D_r^{-1}\circ\iota^{-1}
    =\gamma_r\circ D_r^{-1}\circ\iota^{-1}
    =\gamma_k\circ(\mathrm{pr}_r')^{-1}\circ D_r^{-1}\circ\iota^{-1}
    =\gamma_k\circ(D')^{-1}.  \qedhere
  \]
\end{proof}

\begin{definition}{Gaussian 測度}{gauss-def}
  $m\in\R^n$ と $\Sigma\succeq0$ に対し，
  アフィン写像 $\ell(x):=m+\Sigma^{1/2}x$（連続ゆえ可測）による像測度
  \[
    \Normal(m,\Sigma):=\gamma_n\circ\ell^{-1}
  \]
...
\begin{lemma}{Gaussian 測度のアフィン像}{gauss-affine}
  $L\in\R^{k\times n}$，$b\in\R^k$ とし，
  アフィン写像 $g(x):=Lx+b$ をとると
  \[
    \Normal(m,\Sigma)\circ g^{-1}
...
\begin{proof}
  像測度の合成則（主張~\ref{clm:meas-pf-compose}）より
  \[
    \Normal(m,\Sigma)\circ g^{-1}
    =(\gamma_n\circ\ell^{-1})\circ g^{-1}
    =\gamma_n\circ(g\circ\ell)^{-1},
    \qquad
    (g\circ\ell)(x)=L(m+\Sigma^{1/2}x)+b=(Lm+b)+Ax
  \]
  である（$A:=L\Sigma^{1/2}$ とおいた）．
  平行移動 $t(y):=(Lm+b)+y$ を使うと $g\circ\ell=t\circ A$ と分解でき，
  $AA^\top=L\Sigma L^\top$ だから，
  合成則と鍵補題（補題~\ref{lem:gauss-linear-pf}）より
  \begin{align*}
    \gamma_n\circ(t\circ A)^{-1}
    &=(\gamma_n\circ A^{-1})\circ t^{-1}
    =\bigl(\gamma_k\circ((L\Sigma L^\top)^{1/2})^{-1}\bigr)\circ t^{-1}\\
    &=\gamma_k\circ\bigl(t\circ(L\Sigma L^\top)^{1/2}\bigr)^{-1}
    =\Normal(Lm+b,\;L\Sigma L^\top).
  \end{align*}
  最後の等号は，$t\circ(L\Sigma L^\top)^{1/2}$ が
  $x\mapsto(Lm+b)+(L\Sigma L^\top)^{1/2}x$，すなわち
  $\Normal(Lm+b,L\Sigma L^\top)$ を定義する写像
  そのものであることによる．
\end{proof}
...
\begin{proof}
  ...
  $\Normal((m_1,m_2),A)\circ\mathrm{pr}_1^{-1}=\Normal(m_1,\Sigma_1)$．
  第2周辺も同様である．
\end{proof}
...
\begin{proof}
  ...
  $\Sigma^{1/2}I_n\Sigma^{1/2}=\Sigma$
  （平方根の定義：定理~\ref{thm:la-sqrt}）である．
  主張文中の $\int\norm{x}^2\,\d\Normal(m,\Sigma)=\norm{m}^2+\tr\Sigma$ は
  主張~\ref{clm:gauss-cov-basic}(ii) による．
\end{proof}
...
\begin{memo*}
  ...
  像測度による定義（定義~\ref{def:gauss-def}）の利点は，
...
\begin{lemma}{平均の分離}{gauss-centering}
  ...
  $\tilde\mu:=\mu\circ(x\mapsto x-m_\mu)^{-1}$，
  $\tilde\nu:=\nu\circ(y\mapsto y-m_\nu)^{-1}$ とおく
...
\begin{proof}
  $T(x,y):=(x-m_\mu,\,y-m_\nu)$ とおく（アフィンゆえ可測）．
  $\pi\in\Couplings(\mu,\nu)$ に対し
  $\pi\circ T^{-1}\in\Couplings(\tilde\mu,\tilde\nu)$ である：実際
  \[
    (\pi\circ T^{-1})(A\times\R^n)
  ...
  =\int\norm{x'-y'}^2\,\d(\pi\circ T^{-1})$ である．よって
  \[
    \int\norm{x-y}^2\,\d\pi
    =\int\norm{x'-y'}^2\,\d(\pi\circ T^{-1})
    +\norm{m_\mu-m_\nu}^2.
  \]
  対応 $\pi\mapsto \pi\circ T^{-1}$ は全単射だから，
  両辺で下限を取り，
  $\Wass_2(\mu,\nu)^2=\inf_\pi\int\norm{x-y}^2\,\d\pi$
  （下限と2乗の交換：注意~\ref{rem:wp-welldef}）を使えば
  主張の等式を得る．
\end{proof}
...
\begin{proof}
  ...
  \]
  $\pi$ について下限を取れば
  （$\Wass_2^2=\inf_\pi\int\norm{x-y}^2\,\d\pi$：注意~\ref{rem:wp-welldef}）
  結論を得る．
\end{proof}
...
\begin{proof}
  ...
  \[
    \int\norm{x-y}^2\,\d\gamma^*
    =\int\norm{z}^2\,\d\bigl(\gamma^*\circ L^{-1}\bigr)(z),
    \qquad
    \gamma^*\circ L^{-1}
...
\begin{remark}{最適輸送写像}{gauss-optimal-map}
  ...
  （逆平方根 $\Sigma_1^{-1/2}$ は注意~\ref{rem:la-inv-sqrt}．
  $\Sigma_1^{-1/2}$ は対称だから $T$ は合同変換の形で
  $T\succeq0$：主張~\ref{clm:la-congruence}），
  ...
  \begin{itemize}
    \item $T\Sigma_1T
      ...
      アフィン像（補題~\ref{lem:gauss-affine}）より
      $\Normal(m_1,\Sigma_1)\circ\phi^{-1}=\Normal(m_2,\Sigma_2)$．
    \item $(\Id,\phi)(x):=(x,\phi(x))$ もアフィン写像で，
      $\pi_\phi:=\Normal(m_1,\Sigma_1)\circ(\Id,\phi)^{-1}$ は
...
\begin{example}{点測度：特異な場合の検算}{gauss-dirac}
  ...
  これは直接の計算とも合う：
  $\pi\in\Couplings(\delta_{m_1},\delta_{m_2})$ なら
  $\pi(\{m_1\}\times\R^n)=1$ かつ $\pi(\R^n\times\{m_2\})=1$ で，
  全測度をもつ2つの集合の共通部分 $\{(m_1,m_2)\}$ も全測度をもつ
  （補集合は零集合2つの合併）から，
  $\pi=\delta_{(m_1,m_2)}$ に限る．
  このとき被積分関数 $\norm{x-y}^2$ は $\pi$-a.e. で定数
  $\norm{m_1-m_2}^2$ に等しく，コストはその値である．
