...
\begin{proof}
  ...
  $r(\lambda_1)=\lambda_1$（定理~\ref{thm:meas-affine-cov}），
...
\begin{claim}{$\gamma_n$ の直積構造とモーメント}{gauss-standard-moments}
  \begin{enumerate}
    \item $\gamma_n=\gamma_1\otimes\cdots\otimes\gamma_1$（$n$ 個の直積）であり，
      ...
      $\mathrm{pr}_r(\gamma_n)=\gamma_r$ である
...
\begin{proof}
  ...
  \[
    A(\gamma_n)
    =U\bigl(D(V^\top(\gamma_n))\bigr)
    =U(D(\gamma_n)),
    \qquad
    B(\gamma_k)
    =U(D'(\gamma_k))
  \]
  だから，$D(\gamma_n)=D'(\gamma_k)$ を示せばよい．
  ...
  $\mathrm{pr}_r(\gamma_n)=\gamma_r=\mathrm{pr}_r'(\gamma_k)$
  だから，合成則より
  \[
    D(\gamma_n)
    =\iota\bigl(D_r(\gamma_r)\bigr)
    =D'(\gamma_k).  \qedhere
  \]
\end{proof}
...
\begin{proof}
  像測度の合成則（主張~\ref{clm:meas-pf-compose}）より，左辺は
  ...
  \begin{align*}
    (x\mapsto(Lm+b)+Ax)(\gamma_n)
    &=(y\mapsto(Lm+b)+y)\bigl(A(\gamma_n)\bigr)\\
    &=(y\mapsto(Lm+b)+y)
      \bigl((L\Sigma L^\top)^{1/2}(\gamma_k)\bigr)\\
    &=\bigl(y\mapsto(Lm+b)+(L\Sigma L^\top)^{1/2}y\bigr)(\gamma_k)
...
\begin{proof}
  ...
  $\mathrm{pr}_1(\Normal((m_1,m_2),A))=\Normal(m_1,\Sigma_1)$．
  第2周辺も同様である．
\end{proof}
...
\begin{proof}
  $T(x,y):=(x-m_\mu,\,y-m_\nu)$ とおく（アフィンゆえ可測）．
  $\pi\in\Couplings(\mu,\nu)$ に対し
  $T(\pi)\in\Couplings(\tilde\mu,\tilde\nu)$ である：実際
  \[
    (T(\pi))(A\times\R^n)
  ...
  =\int\norm{x'-y'}^2\,\d(T(\pi))$ である．よって
  \[
    \int\norm{x-y}^2\,\d\pi
    =\int\norm{x'-y'}^2\,\d(T(\pi))
    +\norm{m_\mu-m_\nu}^2.
  \]
  対応 $\pi\mapsto T(\pi)$ は全単射だから，
  両辺で下限を取り，
  $\Wass_2(\mu,\nu)^2=\inf_\pi\int\norm{x-y}^2\,\d\pi$
...
\begin{remark}{最適輸送写像}{gauss-optimal-map}
  ...
  \begin{itemize}
    \item $T\Sigma_1T
      ...
      アフィン像（補題~\ref{lem:gauss-affine}）より
      $\phi(\Normal(m_1,\Sigma_1))=\Normal(m_2,\Sigma_2)$．
    \item $(\Id,\phi)(x):=(x,\phi(x))$ もアフィン写像で，
      $\pi_\phi:=(\Id,\phi)(\Normal(m_1,\Sigma_1))$ は
