...
\begin{lemma}{$W_p$ の順序単調性（Villani Rem 6.6）}{wt-order}
...
\begin{lemma}{$W_p$ の Cauchy 列は tight（Villani Lem 6.14）}{wt-cauchy-tight}
...
\begin{definition}{$\Pp{p}(\mathcal{X})$ における弱収束（Villani Def 6.8）}{wt-weak-pp}
...
\begin{lemma}{$\Pp{p}$ 弱収束の同値な言い換え（Villani Def 6.8 の同値性）}{wt-weak-pp-equiv}
...
\begin{lemma}{$W_p$ の弱収束に関する下半連続性（Villani Rem 6.12）}{wt-wp-lsc}
...
\begin{theorem}{最適性の弱極限での保存（Villani Thm 5.20）}{wt-stability}
  $\Pp{p}(\mathcal{X})$ の列 $(\mu_k)$，$(\nu_k)$ と
  $\mu,\nu\in\Pp{p}(\mathcal{X})$ が
  $\mu_k\Rightarrow\mu$，$\nu_k\Rightarrow\nu$（弱収束）をみたし，
...
\begin{theorem}{$W_p$ は $\Pp{p}$ の弱収束を距離化する（Villani Thm 6.9）}{wt-metrize}
  $(\mu_k)$ を $\Pp{p}(\mathcal{X})$ の列，
...
\begin{theorem}{Wasserstein 空間の位相的性質（Villani Thm 6.18）}{wt-polish}
...
\begin{remark}{文献との対応}{wt-references}
  ...
  特別な場合）は証明なしで認めた．
  Corollary 6.11（$W_p$ の $2$ 変数連続性）は
  本資料では用いないため扱わない．
\end{remark}
