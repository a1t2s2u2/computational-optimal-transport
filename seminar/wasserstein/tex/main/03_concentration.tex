...
\begin{definition}{$T_p$ 不等式}{tc-tp}
  ...
  \]
  が成り立つことをいう．
  この形の不等式は\textbf{輸送コスト不等式}あるいは
  \textbf{Talagrand 型不等式}とよばれる
  （後者の呼称は $p=2$ の場合に限ることもある）．
\end{definition}

\begin{remark}{$p$ についての強さ}{tc-tp-order}
  $1\leq p\leq q<\infty$ のとき $\Wass_p\leq\Wass_q$
  （補題~\ref{lem:wt-order}）だから，
  $\nu\in\Pp{q}(\mathcal{X})$ が $T_q(\lambda)$ をみたせば，
  任意の $\mu\in\Pp{q}(\mathcal{X})$ で
  $\Wass_p(\mu,\nu)\leq\sqrt{2H_\nu(\mu)/\lambda}$ も成り立つ．
  すなわち $T_p$ 不等式は $p$ が大きいほど強い
  （Villani (2009) Remark 22.2）．
\end{remark}
...
\begin{theorem}{$T_p$ 不等式は集中を導く}{tc-concentration}
...
\begin{corollary}{Lipschitz 関数の集中}{tc-lipschitz-concentration}
...
\begin{remark}{$\R^N$ は Polish 空間}{tc-rn-polish}
  ...
  よって $\R^N$ は Polish 空間であり，
  第\ref{ch:wasserstein-metrics}章--第\ref{ch:topology}章と
  本章前節までの理論がそのまま適用できる．
...
\begin{theorem}{Talagrand の不等式}{tc-talagrand}
  ...
  Villani (2009) では，Ricci 曲率条件 $\mathrm{CD}(K,\infty)$ を
  みたす測度が $T_2(K)$ をみたすという Theorem 22.14 の帰結として，
  Example 22.15 で述べられている．）
\end{theorem}
...
\begin{remark}{文献との対応}{tc-references}
  ...
  定理~\ref{thm:tc-talagrand}は本資料で唯一証明を省略した主結果であり，
  出典は定理中に記した．
