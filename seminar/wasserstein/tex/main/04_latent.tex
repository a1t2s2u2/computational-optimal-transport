...
\begin{definition}{デコーダ・エンコーダ・集約事後分布}{lt-vae}
  ...
  $K$（各 $z$ で $K(z,\cdot)\in\Prob(\mathcal{X})$，
  各 $B$ で $z\mapsto K(z,B)$ が可測）のことをいい，
...
\begin{proof}
  ...
  （注意~\ref{rem:tc-entropy-welldef}と同様），
...
\begin{definition}{曲線の長さと内在距離}{lt-length}
  集合 $M$ 上の擬距離 $\rho$ と写像 $\gamma\colon[0,1]\to M$，
  ...
  \]
  とおき，$L(\gamma)$ を $\gamma$ の\textbf{長さ}という．
  また
  \[
    \hat\rho(z,z')
    :=\inf\bigl\{L(\gamma)\bigm|
    \gamma\colon[0,1]\to M,\ \gamma(0)=z,\ \gamma(1)=z'\bigr\}
  \]
...
\begin{claim}{長さの基本性質}{lt-length-props}
  \begin{enumerate}
    \item 分割 $\Delta'$ が $\Delta$ の細分ならば
      ...
      特に，任意の分割列に沿った $S(\gamma;\cdot)$ の値は
      つねに $L(\gamma)$ 以下である．
...
Roy--Hauberg の式 (3)(7) は，一様分割
$\Delta_K\colon t_k=k/K$ に沿った和 $S(\gamma;\Delta_K)$ の
$K\to\infty$ での極限として潜在空間の曲線の「長さ」を定め，
その最小化元（3次スプラインでパラメータ付けして数値最適化する）を
\textbf{測地線}とよぶ．主張~\ref{clm:lt-length-props} (1) より，
これらの和はつねに本定義の長さ $L(\gamma)$ を下から近似する．
...
\begin{remark}{文献との対応と展望}{lt-references}
  ...
  集約事後分布も同論文にはない本章の補足概念である．
  系~\ref{cor:lt-elbo-w2}は同論文にはない補足であり，
  ELBO の正則化項が潜在空間の輸送的ずれを制御することを，
  第\ref{ch:concentration}章の Talagrand の不等式から導いた．
