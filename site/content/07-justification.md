...
\(\varepsilon\to0\) のとき、正則化解 \(\mathbf{P}_{\varepsilon}\) は最大エントロピー最適解へ収束する。特に
...
:::details-embedded Cuturi/Peyre における収束の見方
収束解析では Hilbert 射影距離を使う。Sinkhorn 反復は正ベクトル全体 \(\R_{++}^n\), \(\R_{++}^m\) の上で Birkhoff の縮小定理を利用するため、スケーリングベクトルは定数倍の自由度を除いて一意な極限へ収束する。
:::
:::
...
\(\varepsilon>0\) のとき、\(\mathrm{MK}_{\mathbf{C}}^\varepsilon(\mathbf{a},\mathbf{b})\) は入力ヒストグラムに関して微分可能である。最適な双対ポテンシャルを \((\mathbf{f}^{\star},\mathbf{g}^{\star})\) とすると、定数倍の不定性を除いて
